...
SocketServer::SocketServer(const std::string& uds_path, int connections, SocketMode mode) : Socket(uds_path)
{
#ifdef IS_WINDOWS
    if(!uds_path.empty()){
        m_error = std::string("unix socket not supported");
...
SocketClient::SocketClient(const std::string& host, int port, int connect_timeout_msec, long readTimeout) : Socket(std::string(), readTimeout)
{
    sockaddr_in addr;
...
int SocketSelect::selectSocket()
{
    ...
    for (unsigned i = 0; i < m_socketVec.size(); i++) {
        ...
        if (max_s < m_socketVec[i]->m_socket) max_s = (int)m_socketVec[i]->m_socket;
    }
