...
#include "../../include/beerocks/bcl/network/swap.h"
...
#ifdef BEEROCKS_UGW
    mac = (uint64_t(mac_address[0]) << 56) |
          (uint64_t(mac_address[1]) << 48) |
          (uint64_t(mac_address[2]) << 40) |
          (uint64_t(mac_address[3]) << 32) |
          (uint64_t(mac_address[4]) << 24) |
          (uint64_t(mac_address[5]) << 16);
#elif BEEROCKS_BRCM
    mac = (uint64_t(mac_address[0]) << 16) |
          (uint64_t(mac_address[1]) << 24) |
          (uint64_t(mac_address[2]) << 32) |
          (uint64_t(mac_address[3]) << 40) |
          (uint64_t(mac_address[4]) << 48) |
          (uint64_t(mac_address[5]) << 56);
#endif
...
{
    std::string ipv4_str;
    std::for_each(ipv4, ipv4 + IP_ADDR_LEN, [&](const uint8_t& oct) { 
...
uint32_t network_utils::uint_ipv4_from_array(void* ip)
{
    ...
    ret = (
            ((uint32_t(*(((char*)ip) + 0)) << 24) & 0xFF000000) |
...
uint32_t network_utils::uint_ipv4_from_string(const std::string& ip_str)
{
    ...
    uint32_t ret = ( (uint32_t(ip_arr[0]) << 24) |
                     (uint32_t(ip_arr[1]) << 16) |
                     (uint32_t(ip_arr[2]) << 8 ) |
                     (uint32_t(ip_arr[3])      ) );
...
bool network_utils::get_raw_iface_info(const std::string& iface_name, raw_iface_info& info)
{
    ...
    std::copy_n((uint8_t*)&(*(sockaddr_in*)&ifr.ifr_broadaddr).sin_addr, 4, (uint8_t*)&info.bcast);
...
static int readNlSock(int fd, char* msg, uint32_t seq, uint32_t pid)
{
    ...
    for (;;) {
        ...
        if (((header->nlmsg_flags & NLM_F_MULTI) == 0) ||
             (header->nlmsg_seq != seq) || (header->nlmsg_pid != pid)) {
            break;
        }
    }
...
static int parseRoutes(struct nlmsghdr *nlHdr, std::shared_ptr<route_info> rtInfo)
{
    ...
    if (rtMsg->rtm_family != AF_INET) return (0);
...
bool network_utils::linux_remove_iface_from_bridge(const std::string& bridge, const std::string& iface)
{
...
bool network_utils::linux_iface_ctrl(const std::string& iface, bool up, std::string ip, const std::string& netmask)
{
    bool ret = true;
    int fd;
    struct ifreq ifr;
    ...
    LOG(TRACE) << "linux_iface_ctrl iface=" << iface << (up?" up":" down") << " ip=" << ip << " netmask=" << netmask;
    ...
    while (up) {
        ...
        if (!netmask.empty()) {
            ((struct sockaddr_in *) &ifr.ifr_netmask)->sin_addr.s_addr = network_utils::uint_ipv4_from_string(netmask);
...
{
    std::ifstream phy_device_file("/sys/class/net/" + iface + "/phy80211/device/device", std::ios::in);
    ...
    std::ifstream phy_vendor_file("/sys/class/net/" + iface + "/phy80211/device/vendor", std::ios::in);
    ...
    for (; NLMSG_OK(nlMsg, uint32_t(len)); nlMsg = NLMSG_NEXT(nlMsg, len)) {
        ...
        if (rtInfo_ret == 1) { // GW address
            ...
            if (ioctl(fd, SIOCGIFHWADDR, &ifr) == -1) {
                ...
                ip_info.mac = network_utils::mac_to_string((uint8_t*)(ifr.ifr_ifru.ifru_hwaddr.sa_data));
                std::transform(ip_info.mac.begin(), ip_info.mac.end(), ip_info.mac.begin(), ::tolower);
            }
...
#ifdef __ANDROID__
#else // Linux
bool network_utils::arp_send(const std::string& iface, const std::string& dst_ip, const std::string& src_ip, 
                                sMacAddr dst_mac, sMacAddr src_mac, int count, int arp_socket)
{
    int tx_len;
    arp_hdr arphdr;
...
bool network_utils::icmp_send(const std::string& ip, uint16_t id, int count, int icmp_socket)
{
    ...
    for(int i = 0; i < count ; i++){
        ...
        pkt->checksum = 0; //keep cheksum = 0 before calling icmp_checksum -> needed for checksum calculation
        pkt->checksum = htons(icmp_checksum((uint16_t*)pkt, sizeof(packet_buffer)));
        ssize_t bytes = sendto(icmp_socket, packet_buffer, sizeof(packet_buffer), 0, (sockaddr *)&pingaddr, sizeof(sockaddr_in));
        if (bytes < 0) {
            LOG(ERROR) << "writeBytes() failed: " << strerror(errno);
            return false;
        } else if(bytes < (ssize_t)sizeof(packet_buffer)) {
            LOG(ERROR) << "ICMP Failed to send, sent " << int(bytes) << " out of " << int(sizeof(packet_buffer));
            return false;
        }
    }
    return true;
