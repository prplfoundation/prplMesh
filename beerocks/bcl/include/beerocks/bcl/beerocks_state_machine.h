...
class beerocks_fsm_transition
{
// Public methods:
public:
	
	// Constructor
	beerocks_fsm_transition(
		const TState& source, const TState& destination, const TEvent& event)
	...
	bool change_destination(const TState& dest) {
		LOG(INFO) << "'" << m_source
			<< "': Overriding destination for '" << m_event
			<< "' from '" << m_destination << "' to '" << dest << "'";
...
public:
	...
	// Constructor
	beerocks_fsm_event(const TEvent& event,
		const std::list<TState>& target_states, const TCondition& cond)
	...
	bool check_condition(TTransition& transition, const void* args = nullptr) {
		...
		if (!m_cond) return true;

		TState org_dest = transition.destination();
		
		// Evaluate the condition
		if (!m_cond(transition, args)) return false;
...
public:
	...
	// Constructor
	beerocks_fsm_state(const TState& state) 
		: m_state(state) {}
	...
	void add_event(const std::shared_ptr<TEventHandler>& event_handler) {
		...
		if (m_events.find(event_handler->event()) != m_events.end()) {
			LOG(FATAL) << "Event '" << event_handler->event() 
					   << "' for State '" << m_state << "' already exists!";
		}
...
public:
	...
	beerocks_fsm_config& on(const TEvent& event, const std::list<TState>& states, const TTransAction& cond = {}) {
		if (!m_curr_state) { 
			LOG(FATAL) << "Invalid state handler!";
		}
	...
	beerocks_fsm_config& on(const TEvent& trigger, const TState& state, const TTransAction& cond = {}) {
...
public:
	...
	bool fire(const TEvent& event, const void* args = nullptr) {
		...
		if (!state_handler->can_handle(event)) {
			LOG(WARNING) << "Current state '" << m_curr_state << "' unable to handle event '" << event << "'";
			return false;
		}
