...
const uint32_t &cACTION_HEADER::magic() { return (const uint32_t &)(*m_magic); }

const uint8_t &cACTION_HEADER::version() { return (const uint8_t &)(*m_version); }

eAction &cACTION_HEADER::action() { return (eAction &)(*m_action); }

uint8_t &cACTION_HEADER::action_op() { return (uint8_t &)(*m_action_op); }

uint8_t &cACTION_HEADER::direction() { return (uint8_t &)(*m_direction); }
...
uint8_t &cACTION_HEADER::last() { return (uint8_t &)(*m_last); }

uint16_t &cACTION_HEADER::id() { return (uint16_t &)(*m_id); }

uint16_t &cACTION_HEADER::length() { return (uint16_t &)(*m_length); }
...
bool cACTION_HEADER::init()
{
    ...
    if (!m_parse__)
        *m_magic = beerocks::message::MESSAGE_MAGIC;
    m_buff_ptr__ += sizeof(uint32_t) * 1;
    m_version = (uint8_t *)m_buff_ptr__;
    if (!m_parse__)
        *m_version = beerocks::message::MESSAGE_VERSION;
