...
public:
    ...
    {
        return (eActionOp_PLATFORM)(
            ACTION_PLATFORM_SON_SLAVE_BACKHAUL_CONNECTION_COMPLETE_NOTIFICATION);
    }
...
public:
    cACTION_PLATFORM_SON_SLAVE_REGISTER_RESPONSE(uint8_t *buff, size_t buff_len, bool parse = false,
                                                 bool swap_needed = false);
    cACTION_PLATFORM_SON_SLAVE_REGISTER_RESPONSE(std::shared_ptr<BaseClass> base,
...
public:
    cACTION_PLATFORM_POST_INIT_CONFIG_REQUEST(uint8_t *buff, size_t buff_len, bool parse = false,
                                              bool swap_needed = false);
    cACTION_PLATFORM_POST_INIT_CONFIG_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false,
                                              bool swap_needed = false);
...
public:
    cACTION_PLATFORM_POST_INIT_CONFIG_RESPONSE(uint8_t *buff, size_t buff_len, bool parse = false,
                                               bool swap_needed = false);
    cACTION_PLATFORM_POST_INIT_CONFIG_RESPONSE(std::shared_ptr<BaseClass> base, bool parse = false,
                                               bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WLAN_PARAMS_CHANGED_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                      bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WLAN_PARAMS_CHANGED_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                      bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_CHANGE_MODULE_LOGGING_LEVEL(uint8_t *buff, size_t buff_len, bool parse = false,
                                                 bool swap_needed = false);
    cACTION_PLATFORM_CHANGE_MODULE_LOGGING_LEVEL(std::shared_ptr<BaseClass> base,
                                                 bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_BEEROCKS_CREDENTIALS_UPDATE_REQUEST(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_CREDENTIALS_SET_REQUEST(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_CREDENTIALS_SET_REQUEST(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_CREDENTIALS_SET_RESPONSE(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_CREDENTIALS_SET_RESPONSE(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_ADVERTISE_SSID_FLAG_UPDATE_REQUEST(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_PLATFORM_ADVERTISE_SSID_FLAG_UPDATE_REQUEST(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
...
public:
    cACTION_PLATFORM_ADVERTISE_SSID_FLAG_UPDATE_RESPONSE(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
...
public:
    cACTION_PLATFORM_GET_WLAN_READY_STATUS_REQUEST(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_GET_WLAN_READY_STATUS_REQUEST(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_GET_WLAN_READY_STATUS_RESPONSE(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_GET_WLAN_READY_STATUS_RESPONSE(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_CREDENTIALS_GET_REQUEST(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_CREDENTIALS_GET_REQUEST(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_CREDENTIALS_GET_RESPONSE(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_CREDENTIALS_GET_RESPONSE(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_ADMIN_CREDENTIALS_GET_REQUEST(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_ADMIN_CREDENTIALS_GET_REQUEST(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_ADMIN_CREDENTIALS_GET_RESPONSE(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_ADMIN_CREDENTIALS_GET_RESPONSE(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_SET_IFACE_STATE_REQUEST(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_SET_IFACE_STATE_REQUEST(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_SET_IFACE_STATE_RESPONSE(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_SET_IFACE_STATE_RESPONSE(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_SET_RADIO_TX_STATE_REQUEST(uint8_t *buff, size_t buff_len,
                                                     bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_SET_RADIO_TX_STATE_REQUEST(std::shared_ptr<BaseClass> base,
                                                     bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_SET_RADIO_TX_STATE_RESPONSE(uint8_t *buff, size_t buff_len,
                                                      bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_WIFI_SET_RADIO_TX_STATE_RESPONSE(std::shared_ptr<BaseClass> base,
                                                      bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_VERSION_MISMATCH_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_PLATFORM_VERSION_MISMATCH_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_PLATFORM_MASTER_SLAVE_VERSIONS_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_PLATFORM_MASTER_SLAVE_VERSIONS_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
...
public:
    cACTION_PLATFORM_GET_MASTER_SLAVE_VERSIONS_REQUEST(uint8_t *buff, size_t buff_len,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
    cACTION_PLATFORM_GET_MASTER_SLAVE_VERSIONS_REQUEST(std::shared_ptr<BaseClass> base,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
...
public:
    cACTION_PLATFORM_GET_MASTER_SLAVE_VERSIONS_RESPONSE(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_PLATFORM_GET_MASTER_SLAVE_VERSIONS_RESPONSE(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_INTERFACE_STATUS_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_PLATFORM_WIFI_INTERFACE_STATUS_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_CONFIGURATION_UPDATE_REQUEST(uint8_t *buff, size_t buff_len,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
    cACTION_PLATFORM_WIFI_CONFIGURATION_UPDATE_REQUEST(std::shared_ptr<BaseClass> base,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
...
public:
    cACTION_PLATFORM_WIFI_CONFIGURATION_UPDATE_RESPONSE(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_PLATFORM_WIFI_CONFIGURATION_UPDATE_RESPONSE(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
