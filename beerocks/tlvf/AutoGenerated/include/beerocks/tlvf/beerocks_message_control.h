...
public:
    cACTION_CONTROL_SLAVE_JOINED_4ADDR_MODE_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
...
public:
    cACTION_CONTROL_VERSION_MISMATCH_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_VERSION_MISMATCH_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_PLATFORM_OPERATIONAL_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                      bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_PLATFORM_OPERATIONAL_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                      bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_BACKHAUL_DL_RSSI_REPORT_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_TX_ON_REQUEST(uint8_t *buff, size_t buff_len, bool parse = false,
                                         bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_TX_ON_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false,
                                         bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_TX_OFF_REQUEST(uint8_t *buff, size_t buff_len, bool parse = false,
                                          bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_TX_OFF_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false,
                                          bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_TX_ON_RESPONSE(uint8_t *buff, size_t buff_len, bool parse = false,
                                          bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_TX_ON_RESPONSE(std::shared_ptr<BaseClass> base, bool parse = false,
                                          bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_CSA_ERROR_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_CSA_ERROR_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_ACS_ERROR_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_ACS_ERROR_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_CHANNEL_SWITCH_ACS_START(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_CHANNEL_SWITCH_ACS_START(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_CHANNEL_SWITCH_REQUEST(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_CHANNEL_SWITCH_REQUEST(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_STATS_MEASUREMENT_REQUEST(uint8_t *buff, size_t buff_len,
                                                     bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_STATS_MEASUREMENT_REQUEST(std::shared_ptr<BaseClass> base,
                                                     bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_STATS_MEASUREMENT_RESPONSE(uint8_t *buff, size_t buff_len,
                                                      bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_STATS_MEASUREMENT_RESPONSE(std::shared_ptr<BaseClass> base,
                                                      bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_LOAD_MEASUREMENT_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_SET_NEIGHBOR_11K_REQUEST(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_SET_NEIGHBOR_11K_REQUEST(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_REMOVE_NEIGHBOR_11K_REQUEST(uint8_t *buff, size_t buff_len,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_REMOVE_NEIGHBOR_11K_REQUEST(std::shared_ptr<BaseClass> base,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_ACTIVITY_NOTIFICATION(uint8_t *buff, size_t buff_len, bool parse = false,
                                                 bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_ACTIVITY_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                 bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_VAPS_LIST_UPDATE_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_AP_DISABLED_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_AP_DISABLED_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_HOSTAP_AP_ENABLED_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_HOSTAP_AP_ENABLED_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_START_MONITORING_REQUEST(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_START_MONITORING_REQUEST(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_STOP_MONITORING_REQUEST(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_STOP_MONITORING_REQUEST(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_RX_RSSI_MEASUREMENT_REQUEST(uint8_t *buff, size_t buff_len,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
    cACTION_CONTROL_CLIENT_RX_RSSI_MEASUREMENT_REQUEST(std::shared_ptr<BaseClass> base,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_CONTROL_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_NO_ACTIVITY_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_NO_ACTIVITY_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_NO_RESPONSE_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_NO_RESPONSE_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_ASSOCIATED_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_ASSOCIATED_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_DISCONNECTED_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                     bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_DISCONNECTED_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                     bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_DISALLOW_REQUEST(uint8_t *buff, size_t buff_len, bool parse = false,
                                            bool swap_needed = false);
    cACTION_CONTROL_CLIENT_DISALLOW_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false,
                                            bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_ALLOW_REQUEST(uint8_t *buff, size_t buff_len, bool parse = false,
                                         bool swap_needed = false);
    cACTION_CONTROL_CLIENT_ALLOW_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false,
                                         bool swap_needed = false);
...
class cACTION_CONTROL_CLIENT_BSS_STEER_REQUEST : public BaseClass {
public:
    cACTION_CONTROL_CLIENT_BSS_STEER_REQUEST(uint8_t *buff, size_t buff_len, bool parse = false,
                                             bool swap_needed = false);
    cACTION_CONTROL_CLIENT_BSS_STEER_REQUEST(std::shared_ptr<BaseClass> base, bool parse = false,
                                             bool swap_needed = false);
    ~cACTION_CONTROL_CLIENT_BSS_STEER_REQUEST();

    static eActionOp_CONTROL get_action_op()
    {
        return (eActionOp_CONTROL)(ACTION_CONTROL_CLIENT_BSS_STEER_REQUEST);
    }
...
public:
    cACTION_CONTROL_CLIENT_BSS_STEER_RESPONSE(uint8_t *buff, size_t buff_len, bool parse = false,
                                              bool swap_needed = false);
    cACTION_CONTROL_CLIENT_BSS_STEER_RESPONSE(std::shared_ptr<BaseClass> base, bool parse = false,
                                              bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_DHCP_COMPLETE_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                      bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_DHCP_COMPLETE_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                      bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_ARP_MONITOR_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_ARP_MONITOR_NOTIFICATION(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_CHANNEL_LOAD_11K_REQUEST(uint8_t *buff, size_t buff_len,
                                                    bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_CHANNEL_LOAD_11K_REQUEST(std::shared_ptr<BaseClass> base,
                                                    bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_CHANNEL_LOAD_11K_RESPONSE(uint8_t *buff, size_t buff_len,
                                                     bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_CHANNEL_LOAD_11K_RESPONSE(std::shared_ptr<BaseClass> base,
                                                     bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_STATISTICS_11K_REQUEST(uint8_t *buff, size_t buff_len,
                                                  bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_STATISTICS_11K_REQUEST(std::shared_ptr<BaseClass> base,
                                                  bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_STATISTICS_11K_RESPONSE(uint8_t *buff, size_t buff_len,
                                                   bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_CLIENT_STATISTICS_11K_RESPONSE(std::shared_ptr<BaseClass> base,
                                                   bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_CLIENT_LINK_MEASUREMENT_11K_REQUEST(uint8_t *buff, size_t buff_len,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
    cACTION_CONTROL_CLIENT_LINK_MEASUREMENT_11K_REQUEST(std::shared_ptr<BaseClass> base,
                                                        bool parse       = false,
                                                        bool swap_needed = false);
...
public:
    cACTION_CONTROL_STEERING_CLIENT_SET_GROUP_REQUEST(uint8_t *buff, size_t buff_len,
                                                      bool parse = false, bool swap_needed = false);
    cACTION_CONTROL_STEERING_CLIENT_SET_GROUP_REQUEST(std::shared_ptr<BaseClass> base,
                                                      bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_STEERING_CLIENT_SET_GROUP_RESPONSE(uint8_t *buff, size_t buff_len,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
    cACTION_CONTROL_STEERING_CLIENT_SET_GROUP_RESPONSE(std::shared_ptr<BaseClass> base,
                                                       bool parse       = false,
                                                       bool swap_needed = false);
...
public:
    cACTION_CONTROL_STEERING_CLIENT_SET_RESPONSE(uint8_t *buff, size_t buff_len, bool parse = false,
                                                 bool swap_needed = false);
    cACTION_CONTROL_STEERING_CLIENT_SET_RESPONSE(std::shared_ptr<BaseClass> base,
                                                 bool parse = false, bool swap_needed = false);
...
public:
    cACTION_CONTROL_STEERING_EVENT_SNR_XING_NOTIFICATION(uint8_t *buff, size_t buff_len,
                                                         bool parse       = false,
                                                         bool swap_needed = false);
