...
ap_wlan_hal_brcm::ap_wlan_hal_brcm(
    ...
    base_wlan_hal(bwl::HALType::AccessPoint, iface_name, IfaceType::Broadcom, acs_enabled, callback),
    base_wlan_hal_brcm(bwl::HALType::AccessPoint, iface_name, acs_enabled, IfaceType::Broadcom, callback)
{
}
...
HALState ap_wlan_hal_brcm::attach(bool block)
{
    ...
    int mac_mode = 1;
    int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_SET_MACMODE, &mac_mode, sizeof(mac_mode));
    if (ret < 0) {
...
{ 
    ...
    if (mac.empty()) {
        ...
        maclist->count = 0;
        int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_SET_MACLIST, maclist, sizeof(int));
        if (ret < 0) {
    ...
    maclist->count = max;
    int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_GET_MACLIST, maclist, sizeof(wl_ioctl_buf));
    if (ret < 0) {
    ...
    for (uint i = 0; i < maclist->count; i++) {
        if (!memcmp(&sta_mac_raw, &maclist->ea[i], ETHER_ADDR_LEN)) {
            ...
            if (i == maclist->count - 1) {
                ...
                memmove(&maclist->ea[i], &maclist->ea[i + 1], (ETHER_ADDR_LEN * (maclist->count - i)));
            }
...
{ 
    ...
    int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_SCB_DEAUTHENTICATE, &scb_val.ea, ETHER_ADDR_LEN);
    if (ret < 0) {
...
bool ap_wlan_hal_brcm::sta_bss_steer(const std::string& mac, const std::string& bssid, int chan, int disassoc_timer, int valid_int)
{
    LOG(TRACE) << __func__ << " mac: " << mac;
...
bool ap_wlan_hal_brcm::sta_unassoc_rssi_measurement(const std::string& mac, int chan, int bw, int vht_center_frequency, int delay, int window_size)
{
    return true;
}

bool ap_wlan_hal_brcm::switch_channel(int chan, int bw, int vht_center_frequency) 
{ 
    return true;
}

bool ap_wlan_hal_brcm::set_antenna_mode(AntMode mode) 
{ 
    return true;
}

bool ap_wlan_hal_brcm::wds_set_mode(WDSMode mode)
{
    return true;    
}

bool ap_wlan_hal_brcm::wds_add_sta(const std::string& mac)
{
    return true;    
}

bool ap_wlan_hal_brcm::wds_del_sta(const std::string& mac)
{
    return true;
}

bool ap_wlan_hal_brcm::wds_clear_list()
{
    return true;
}
...
bool ap_wlan_hal_brcm::failsafe_channel_get(int& chan, int& bw)
{
    return true;
}

bool ap_wlan_hal_brcm::restricted_channels_set(char* channel_list)
{
    return true;
}

bool ap_wlan_hal_brcm::restricted_channels_get(char* channel_list)
{
    return true;
}
...
{ 
    ...
    int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_GET_VALID_CHANNELS, chan_buf, sizeof(chan_buf));
    ...
    if (!list->count) {
        ...
        for (int channel_idx = 0; channel_idx < int(list->count); channel_idx++) {
            ...
            if(channel_idx < beerocks::message::SUPPORTED_CHANNELS_LENGTH) {
                if(channel > 14) m_radio_info.is_5ghz = 1;
...
std::string ap_wlan_hal_brcm::get_radio_driver_version()
{
    return "brcm";
}
...
bool ap_wlan_hal_brcm::get_vap_enable(const std::string& iface_name, bool& enable)
{
    return true;
}
...
bool ap_wlan_hal_brcm::process_ext_events()
{
    ...
    maclist->count = max;
    int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_GET_ASSOCLIST, maclist, sizeof(wl_ioctl_buf));
    ...
    for (int i = 0; i < int(maclist->count) && i < max; i++, ea++) {
        ...
        if (m_assoc_list.find(mac) == m_assoc_list.end()) {
            ...
            auto msg_buff = ALLOC_SMART_BUFFER(sizeof(beerocks::message::sACTION_APMANAGER_CLIENT_ASSOCIATED_NOTIFICATION));
            auto msg = reinterpret_cast<beerocks::message::sACTION_APMANAGER_CLIENT_ASSOCIATED_NOTIFICATION*>(msg_buff.get());
            LOG_IF(!msg, FATAL) << "Memory allocation failed!";
                
            // Initialize the message
            memset(msg_buff.get(), 0, sizeof(beerocks::message::sACTION_APMANAGER_CLIENT_ASSOCIATED_NOTIFICATION));
    ...
    for (auto sta = m_assoc_list.begin(); sta != m_assoc_list.end();) {
        ...
        if (sta->second < now) {
            ...
            auto msg_buff = ALLOC_SMART_BUFFER(sizeof(beerocks::message::sACTION_APMANAGER_CLIENT_DISCONNECTED_NOTIFICATION));
            auto msg = reinterpret_cast<beerocks::message::sACTION_APMANAGER_CLIENT_DISCONNECTED_NOTIFICATION*>(msg_buff.get());
            LOG_IF(!msg, FATAL) << "Memory allocation failed!";
                
            // Initialize the message
            memset(msg_buff.get(), 0, sizeof(beerocks::message::sACTION_APMANAGER_CLIENT_DISCONNECTED_NOTIFICATION));
