...
bool base_wlan_hal_brcm::refresh_radio_info()
{
    ...
    int ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_GET_VAR, wl_ioctl_buf, sizeof(wl_ioctl_buf));
    if (ret < 0) {
    ...
    uint32_t tx_enabled;
    ret = bwl_wl_ioctl((char*)m_radio_info.iface_name.c_str(), WLC_GET_RADIO, &tx_enabled, sizeof(tx_enabled));
    if (ret < 0) {
    ...
    network_utils::linux_iface_get_mac(m_radio_info.iface_name, m_radio_info.available_vaps[beerocks::IFACE_VAP_ID_MIN].mac);
...
void base_wlan_hal_brcm::refresh_vaps_info(int id)
{
    return true;
}
