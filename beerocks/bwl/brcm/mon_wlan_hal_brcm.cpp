...
mon_wlan_hal_brcm::mon_wlan_hal_brcm(std::string iface_name, hal_event_cb_t callback)
...
bool mon_wlan_hal_brcm::detach() { return true; }

bool mon_wlan_hal_brcm::process_ext_events() { return true; }
...
bool mon_wlan_hal_brcm::update_stations_stats(const std::string vap_iface_name,
                                              const std::string sta_mac, SStaStats &sta_stats)
{
    ...
    ret = bwl_wl_ioctl((char *)vap_iface_name.c_str(), WLC_GET_VAR, wl_ioctl_buf,
                       sizeof(wl_ioctl_buf));
    if (ret < 0) {
