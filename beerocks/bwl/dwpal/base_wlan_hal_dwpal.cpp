...
base_wlan_hal_dwpal::base_wlan_hal_dwpal(
...
bool base_wlan_hal_dwpal::fsm_setup()
{
		...
		[&](const void *args) -> bool
	{
		...
		// Handle "Attach" event
		.on(dwpal_fsm_event::Attach, { dwpal_fsm_state::Attach, dwpal_fsm_state::Detach, dwpal_fsm_state::GetRadioInfo },
		[&](TTransition& transition, const void *args) -> bool
	{
		...
		if (m_dwpal_ctx == nullptr) {
			int result = dwpal_hostap_interface_attach(&m_dwpal_ctx, m_radio_info.iface_name.c_str(), nullptr); // return values: 0 on success, -1 on failure
			if ((result == 0) && m_dwpal_ctx) {
				attached = true;
				LOG(DEBUG) << "dwpal_hostap_interface_attach() success for: " << m_radio_info.iface_name;
			} else if (result != 0) {
				LOG(ERROR) << "dwpal_hostap_interface_attach() failed for: " << m_radio_info.iface_name;
			} else {
				LOG(ERROR) << "dwpal_hostap_interface_attach() returns 0 for: " << m_radio_info.iface_name << " but context is NULL!";
			}
		} else {
			//Already attached
			attached = true;
		...
		[&](TTransition& transition, const void *args) -> bool
	{
		...
		.on(dwpal_fsm_event::Attach, { dwpal_fsm_state::Operational, dwpal_fsm_state::Delay, dwpal_fsm_state::Detach },
		[&](TTransition& transition, const void *args) -> bool
	{
		...
		if (m_dwpal_ctx != nullptr) {
			if (dwpal_hostap_event_fd_get(m_dwpal_ctx, &m_fd_ext_events)) {
				LOG(ERROR) << "dwpal_hostap_event_fd_get() failed for: " << m_radio_info.iface_name;
				return (transition.change_destination(dwpal_fsm_state::Detach));
			}
		} else {
...
HALState base_wlan_hal_dwpal::attach(bool block)
{
	while (true) {
		...
		if (m_last_attach_state != attach_state) {
			LOG(DEBUG) << "DWPAL FSM " << m_radio_info.iface_name << " Attach State: " << attach_state;
...
bool base_wlan_hal_dwpal::refresh_radio_info()
{
	...
	if (get_type() == HALType::Station) {
		...
		FieldsToParse fieldsToParse[] =
		{
			...
			{ (void *)&m_radio_info.conducted_power,   &numOfValidArgs[1], DWPAL_INT_PARAM, "TxPower=",                  0 },
			{ (void *)&m_radio_info.bandwidth,         &numOfValidArgs[2], DWPAL_INT_PARAM, "OperatingChannelBandwidt=", 0 },
			{ (void *)&m_radio_info.vht_center_freq,   &numOfValidArgs[3], DWPAL_INT_PARAM, "Cf1=",                      0 },
			{ (void *)&m_radio_info.wifi_ctrl_enabled, &numOfValidArgs[4], DWPAL_INT_PARAM, "HostapdEnabled=",           0 },
...
bool base_wlan_hal_dwpal::refresh_vap_info(int vap_id)
{
	...
	if (ifname.empty()) {
		LOG(ERROR) << "Failed to get vap_iface from radio " << m_radio_info.iface_name << " and vap id " << vap_id;
		return false;
	}
...
bool base_wlan_hal_dwpal::process_ext_events()
{
	...
	if (status == DWPAL_FAILURE) {
		...
		// No pending messages
		LOG(WARNING) << "base_wlan_hal_dwpal::process_ext_events() called but there are no pending messages...";
		return false;
	}

	/* Silencing unhandled multiple events */
	if (!strncmp(opCode, "WPS-ENROLLEE-SEEN", sizeof(opCode))) {
		LOG_EVERY_N(UNHANDLED_EVENTS_LOGS, DEBUG) << "DWPAL unhandled event opcode recieved: " << opCode;
		return true;
