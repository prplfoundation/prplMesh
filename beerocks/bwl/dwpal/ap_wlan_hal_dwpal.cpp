	...
	bool is_5ghz)
{
	...
	if (!caps_strings.ht_cap.empty()) {
		...
		sta_caps.ht_sm_power_save = (ht_cap & HT_CAP_INFO_SMPS_MASK) >> 2; // 0=static, 1=dynamic, 2=reserved, 3=disabled
		sta_caps.ht_low_bw_short_gi = (ht_cap & HT_CAP_INFO_SHORT_GI20MHZ) >> 5; // 20 long == 0 / short == 1
	...
	if (is_5ghz && (!caps_strings.vht_cap.empty())) {
		...
		for (i = 4; i > 0; i--){ // 4ss
			vht_mcs_temp = (vht_mcs_rx >> (2 * (i - 1))) & 0x03; // 0 indicates support for VHT-MCS 0-7 for n spatial streams
	...
	LOG(DEBUG)
		...
		<< std::endl << "ht_sm_power_save = "	  << (sta_caps.ht_sm_power_save ? "true" : "false")
		...
		<< std::endl << "link_meas = "				<< int(sta_caps.link_meas)
		<< std::endl << "beacon_report_passive = " 	<< int(sta_caps.beacon_report_passive)
		<< std::endl << "beacon_report_active = " 	<< int(sta_caps.beacon_report_active)
		<< std::endl << "beacon_report_table = " 	<< int(sta_caps.beacon_report_table)
		<< std::endl << "lci_meas = " 				<< int(sta_caps.lci_meas)
		<< std::endl << "fmt_range_report = " 		<< int(sta_caps.fmt_range_report);
...
ap_wlan_hal_dwpal::ap_wlan_hal_dwpal(
	...
	base_wlan_hal_dwpal(bwl::HALType::AccessPoint, iface_name, acs_enabled, callback, HOSTAPD_TO_DWPAL_MSG_LENGTH)
{
}
...
bool ap_wlan_hal_dwpal::sta_bss_steer(const std::string& mac, const std::string& bssid, int chan, int disassoc_timer, int valid_int)
{
	// Build command string
	std::string cmd =
...
bool ap_wlan_hal_dwpal::sta_unassoc_rssi_measurement(const std::string& mac, int chan, int bw, int vht_center_frequency, int delay, int window_size)
{
...
bool ap_wlan_hal_dwpal::sta_softblock_add(const std::string& vap_name, const std::string& client_mac, uint8_t reject_error_code, 
...
bool ap_wlan_hal_dwpal::sta_softblock_remove(const std::string& vap_name, const std::string& client_mac) 
{
	// Build command string
...
bool ap_wlan_hal_dwpal::read_acs_report()
{
	...
	for (uint16_t i = 0; i < sizeof(numOfValidArgs) / sizeof(size_t); i++) {
		if ((numOfValidArgs[i] == 0) || (numOfValidArgs[i] != numOfValidArgs[0])) {
			...
			LOG(ERROR) << "Should be the same as numOfValidArgs[0]= " << (int)numOfValidArgs[0] << " ==> Abort";
			return false;
		}
	}
	...
	for (uint16_t i = 0; (i < numOfValidArgs[0]) && (i < MAX_SUPPORTED_CHANNELS); i++) {
		LOG(DEBUG) << "Ch=" << (int)acs_report[i].Ch << " BW=" << (int)acs_report[i].BW << " DFS=" << acs_report[i].DFS << " bss=" << acs_report[i].bss;
...
bool ap_wlan_hal_dwpal::process_dwpal_event(char *buffer, int bufLen, const std::string& opcode)
{
	...
	case Event::CSA_Finished:
	{
        ...
        FieldsToParse fieldsToParse[] =
        {
            ...
            { (void *)&m_radio_info.bandwidth,          &numOfValidArgs[3], DWPAL_INT_PARAM,  "OperatingChannelBandwidt=", 0 },
            { (void *)&m_radio_info.channel_ext_above,  &numOfValidArgs[4], DWPAL_INT_PARAM,  "ExtensionChannel=",         0 },
            { (void *)&m_radio_info.vht_center_freq,    &numOfValidArgs[5], DWPAL_INT_PARAM,  "cf1=",                      0 },
            { (void *)&m_radio_info.is_dfs_channel,     &numOfValidArgs[6], DWPAL_BOOL_PARAM, "dfs_chan=",                 0 },
        ...
        LOG(DEBUG) << "numOfValidArgs[2]= " << numOfValidArgs[2] << " " << channelStr << m_radio_info.channel;
        LOG(DEBUG) << "numOfValidArgs[3]= " << numOfValidArgs[3] << " OperatingChannelBandwidt= " << (int)m_radio_info.bandwidth;
        LOG(DEBUG) << "numOfValidArgs[4]= " << numOfValidArgs[4] << " ExtensionChannel= " << (int)m_radio_info.channel_ext_above;
        LOG(DEBUG) << "numOfValidArgs[5]= " << numOfValidArgs[5] << " cf1= " << (int)m_radio_info.vht_center_freq;
        LOG(DEBUG) << "numOfValidArgs[6]= " << numOfValidArgs[6] << " dfs_chan= " << (int)m_radio_info.is_dfs_channel;
	...
	case Event::STA_Connected:
    {
        ...
        FieldsToParse fieldsToParse[] =
        {
            ...
            { (void *)supportedRates,                          		&numOfValidArgs[3], DWPAL_STR_PARAM,  "SupportedRates=", sizeof(supportedRates) },
            ...
            { (void *)&msg->params.capabilities.btm_supported, 		&numOfValidArgs[8], DWPAL_CHAR_PARAM, "btm_supported=",  0 },
            { (void *)&msg->params.capabilities.cell_capa,     		&numOfValidArgs[9],DWPAL_CHAR_PARAM, "cell_capa=",      0 },
			{ (void *)&msg->params.capabilities.band_2g_capable,	&numOfValidArgs[10],DWPAL_CHAR_PARAM, "band_2g_capable=",      0 },
			{ (void *)&msg->params.capabilities.band_5g_capable,	&numOfValidArgs[11],DWPAL_CHAR_PARAM, "band_5g_capable=",      0 },
			{ (void *)&msg->params.capabilities.rrm_supported, 		&numOfValidArgs[12],DWPAL_CHAR_PARAM, "rrm_supported=",  0 },
			{ (void *)RRM_CAP,  							   		&numOfValidArgs[13],DWPAL_STR_PARAM,  "rrm_cap=",   	sizeof(RRM_CAP) },
			{ (void *)&msg->params.capabilities.max_ch_width,  		&numOfValidArgs[14],DWPAL_CHAR_PARAM, "max_ch_width=",   0 },
			{ (void *)&msg->params.capabilities.max_streams,   		&numOfValidArgs[15],DWPAL_CHAR_PARAM, "max_streams=",	0 },
			{ (void *)&msg->params.capabilities.phy_mode,  	   		&numOfValidArgs[16],DWPAL_CHAR_PARAM, "phy_mode=",       0 },
			{ (void *)&msg->params.capabilities.max_mcs,  	   		&numOfValidArgs[17],DWPAL_CHAR_PARAM, "max_mcs=",   		0 },
			{ (void *)&msg->params.capabilities.max_tx_power,  		&numOfValidArgs[18],DWPAL_CHAR_PARAM, "max_tx_power=",   0 },
			{ (void *)&msg->params.capabilities.mumimo_supported, 	&numOfValidArgs[19],DWPAL_CHAR_PARAM, "mu_mimo=",      0 },
        ...
        LOG(DEBUG) << "numOfValidArgs[3]= "  << numOfValidArgs[3]  << " supportedRates= " << supportedRates;
        ...
        LOG(DEBUG) << "numOfValidArgs[8]= "  << numOfValidArgs[8]  << " btm_supported= " << (int)msg->params.capabilities.btm_supported;
        LOG(DEBUG) << "numOfValidArgs[9]= "  << numOfValidArgs[9]  << " cell_capa= " << (int)msg->params.capabilities.cell_capa;
		LOG(DEBUG) << "numOfValidArgs[10]= " << numOfValidArgs[10] << " band_2g_capable= " << (int)msg->params.capabilities.band_2g_capable;
		LOG(DEBUG) << "numOfValidArgs[11]= " << numOfValidArgs[11] << " band_5g_capable= " << (int)msg->params.capabilities.band_5g_capable;
		LOG(DEBUG) << "numOfValidArgs[12]= " << numOfValidArgs[12] << " rrm_supported= " << (int)msg->params.capabilities.rrm_supported;
		LOG(DEBUG) << "numOfValidArgs[13]= " << numOfValidArgs[13] << " RRM_CAP= " << RRM_CAP;
		LOG(DEBUG) << "numOfValidArgs[14]= " << numOfValidArgs[14] << " max_ch_width= " << (int)msg->params.capabilities.max_ch_width;
		LOG(DEBUG) << "numOfValidArgs[15]= " << numOfValidArgs[15] << " max_streams= " << (int)msg->params.capabilities.max_streams;
		LOG(DEBUG) << "numOfValidArgs[16]= " << numOfValidArgs[16] << " phy_mode= " << (int)msg->params.capabilities.phy_mode;
		LOG(DEBUG) << "numOfValidArgs[17]= " << numOfValidArgs[17] << " max_mcs= " << (int)msg->params.capabilities.max_mcs;
		LOG(DEBUG) << "numOfValidArgs[18]= " << numOfValidArgs[18] << " max_tx_power= " << (int)msg->params.capabilities.max_tx_power;
		LOG(DEBUG) << "numOfValidArgs[19]= " << numOfValidArgs[19] << " mumimo_supported= " << (int)msg->params.capabilities.mumimo_supported;
        /* End of TEMP: Traces... */

        for (uint8_t i = 0; i < (sizeof(numOfValidArgs)/sizeof(size_t)); i++) {
            if (numOfValidArgs[i] == 0) {
                LOG(ERROR) << "Failed reading parsed parameter " << (int)i << " ==> Continue with default values";
            }
        }
	...
	case Event::STA_Disconnected:
    {
        ...
        LOG(DEBUG) << "numOfValidArgs[1]= " << numOfValidArgs[1] << " vap_id= " << VAP;
        LOG(DEBUG) << "numOfValidArgs[2]= " << numOfValidArgs[2] << " MACAddress= " << MACAddress;
		LOG(DEBUG) << "numOfValidArgs[3]= " << numOfValidArgs[3] << " reason= " << int(msg->params.reason);
		LOG(DEBUG) << "numOfValidArgs[4]= " << numOfValidArgs[4] << " source= " << int(msg->params.source);
		LOG(DEBUG) << "numOfValidArgs[5]= " << numOfValidArgs[5] << " type= " <<   int(msg->params.type);
	...
	case Event::STA_Softblock_Drop:
    {
        ...
        FieldsToParse fieldsToParseForMsgType[] =
        {
            ...
            { (void*)&message_type,			&numOfValidArgsForMsgType[1], DWPAL_UNSIGNED_CHAR_PARAM,  "msg_type=", 0	},
            /* Must be at the end */
            { NULL, 						NULL, 						  DWPAL_NUM_OF_PARSING_TYPES, NULL, 	   0 	}
        };

        if (dwpal_string_to_struct_parse(buffer, bufLen, fieldsToParseForMsgType) == DWPAL_FAILURE) {
            LOG(ERROR) << "DWPAL parse error ==> Abort";
            return false;
        }

        /* TEMP: Traces... */
        LOG(DEBUG) << "numOfValidArgs[1]=" << numOfValidArgsForMsgType[1] << ", message_type=" << (int)message_type;

		if (message_type == WLAN_FC_STYPE_PROBE_REQ) {

			auto msg_buff = ALLOC_SMART_BUFFER(sizeof(sACTION_APMANAGER_STEERING_EVENT_PROBE_REQ_NOTIFICATION));
			auto msg = reinterpret_cast<sACTION_APMANAGER_STEERING_EVENT_PROBE_REQ_NOTIFICATION*>(msg_buff.get());
			LOG_IF(!msg, FATAL) << "Memory allocation failed!";
			// Initialize the message
			memset(msg_buff.get(), 0, sizeof(sACTION_APMANAGER_STEERING_EVENT_PROBE_REQ_NOTIFICATION));
			...
			FieldsToParse fieldsToParse[] =
			{
				...
				{ (void*)vap_bssid,  			&numOfValidArgs[1], DWPAL_STR_PARAM,           	"VAP_BSSID=",   sizeof(vap_bssid) 	},
				{ (void*)client_mac,  			&numOfValidArgs[2], DWPAL_STR_PARAM,           	"addr=",        sizeof(client_mac) 	},
				{ (void*)&msg->params.rx_snr,   &numOfValidArgs[3], DWPAL_UNSIGNED_CHAR_PARAM,  "snr=",       	0, 					},
				{ (void*)&msg->params.blocked,  &numOfValidArgs[4], DWPAL_UNSIGNED_CHAR_PARAM,  "blocked=",     0, 					},
				{ (void*)&msg->params.broadcast,&numOfValidArgs[5], DWPAL_UNSIGNED_CHAR_PARAM,  "broadcast=",   0, 					},
				/* Must be at the end */
				{ NULL, 						NULL, 				DWPAL_NUM_OF_PARSING_TYPES, NULL, 			0 					}
			};
			...
			LOG(DEBUG) << "numOfValidArgs[3]=" << numOfValidArgs[3] << ", rx_snr=" 	  	<< (int)msg->params.rx_snr;
			LOG(DEBUG) << "numOfValidArgs[4]=" << numOfValidArgs[4] << ", blocked="   	<< (int)msg->params.blocked;
			LOG(DEBUG) << "numOfValidArgs[5]=" << numOfValidArgs[5] << ", broadcast=" 	<< (int)msg->params.broadcast;
			...
			auto msg_buff = ALLOC_SMART_BUFFER(sizeof(sACTION_APMANAGER_STEERING_EVENT_AUTH_FAIL_NOTIFICATION));
			auto msg = reinterpret_cast<sACTION_APMANAGER_STEERING_EVENT_AUTH_FAIL_NOTIFICATION*>(msg_buff.get());
			LOG_IF(!msg, FATAL) << "Memory allocation failed!";
			// Initialize the message
			memset(msg_buff.get(), 0, sizeof(sACTION_APMANAGER_STEERING_EVENT_AUTH_FAIL_NOTIFICATION));
			...
			FieldsToParse fieldsToParse[] =
			{
				...
				{ (void*)&msg->params.rx_snr,   &numOfValidArgs[3], DWPAL_UNSIGNED_CHAR_PARAM,  "snr=",       	0, 					},
				{ (void*)&msg->params.blocked,  &numOfValidArgs[4], DWPAL_UNSIGNED_CHAR_PARAM,  "blocked=",     0, 					},
				{ (void*)&msg->params.reject,   &numOfValidArgs[5], DWPAL_UNSIGNED_CHAR_PARAM, 	"rejected=",    0, 					},
				{ (void*)&msg->params.reason,   &numOfValidArgs[6], DWPAL_UNSIGNED_CHAR_PARAM,  "reason=",   	0, 					},
				/* Must be at the end */
				{ NULL, 						NULL, 				DWPAL_NUM_OF_PARSING_TYPES, NULL, 			0 					}
			};
			...
			LOG(DEBUG) << "numOfValidArgs[0]=" << numOfValidArgs[0] << ", VAP="			<< vap_name;
			LOG(DEBUG) << "numOfValidArgs[1]=" << numOfValidArgs[1] << ", VAP_BSSID="	<< vap_bssid;
			LOG(DEBUG) << "numOfValidArgs[2]=" << numOfValidArgs[2] << ", client_mac="	<< client_mac;
			LOG(DEBUG) << "numOfValidArgs[3]=" << numOfValidArgs[3] << ", rx_snr=" 	  	<< (int)msg->params.rx_snr;
			LOG(DEBUG) << "numOfValidArgs[4]=" << numOfValidArgs[4] << ", blocked="   	<< (int)msg->params.blocked;
			LOG(DEBUG) << "numOfValidArgs[5]=" << numOfValidArgs[5] << ", rejected=" 	<< (int)msg->params.reject;
			LOG(DEBUG) << "numOfValidArgs[6]=" << numOfValidArgs[6] << ", reason=" 		<< (int)msg->params.reason;
	...
	case Event::BSS_TM_Response:
    {
        ...
        /* TEMP: Traces... */
        LOG(DEBUG) << "numOfValidArgs[2]= " << numOfValidArgs[2] << " MACAddress= " << MACAddress;
		LOG(DEBUG) << "numOfValidArgs[3]= " << numOfValidArgs[3] << " status_code= " << int(status_code);
	...
	case Event::DFS_CAC_Completed:
    {
        ...
        /* TEMP: Traces... */
        LOG(DEBUG) << "numOfValidArgs[2]= " << numOfValidArgs[2] << " success= " << (int)msg->params.success;
        LOG(DEBUG) << "numOfValidArgs[3]= " << numOfValidArgs[3] << " freq= " << msg->params.frequency;
        LOG(DEBUG) << "numOfValidArgs[4]= " << numOfValidArgs[4] << " timeout= " << msg->params.timeout;
        LOG(DEBUG) << "numOfValidArgs[5]= " << numOfValidArgs[5] << " chan_width= " << (int)chan_width;
	...
	case Event::DFS_NOP_Finished:
    {
        ...
        FieldsToParse fieldsToParse[] =
        {
            ...
            { (void *)&msg->params.vht_center_frequency, &numOfValidArgs[3], DWPAL_SHORT_INT_PARAM, "cf1=",        0 },
        ...
        /* TEMP: Traces... */
        LOG(DEBUG) << "numOfValidArgs[2]= " << numOfValidArgs[2] << " freq= " << (int)msg->params.frequency;
        LOG(DEBUG) << "numOfValidArgs[3]= " << numOfValidArgs[3] << " cf1= " << (int)msg->params.vht_center_frequency;
        LOG(DEBUG) << "numOfValidArgs[4]= " << numOfValidArgs[4] << " chan_width= " << (int)chan_width;
