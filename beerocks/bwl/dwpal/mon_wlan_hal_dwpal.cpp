...
bool mon_wlan_hal_dwpal::update_radio_stats(SRadioStats& radio_stats)
{
	...
	FieldsToParse fieldsToParse[] =
	{
		...
		{ (void *)&BytesReceived,               &numOfValidArgs[1], DWPAL_LONG_LONG_INT_PARAM, "BytesReceived=",   0 },
		{ (void *)&PacketsSent,                 &numOfValidArgs[2], DWPAL_LONG_LONG_INT_PARAM, "PacketsSent=",     0 },
		{ (void *)&PacketsReceived,             &numOfValidArgs[3], DWPAL_LONG_LONG_INT_PARAM, "PacketsReceived=", 0 },
		...
		{ (void *)&radio_stats.errors_received, &numOfValidArgs[6], DWPAL_INT_PARAM,           "ErrorsReceived=",  0 },
	...
	calc_curr_traffic((uint64_t)PacketsReceived, radio_stats.rx_packets_cnt, radio_stats.rx_packets);
...
bool mon_wlan_hal_dwpal::update_vap_stats(const std::string vap_iface_name, SVapStats& vap_stats)
{
	...
	FieldsToParse fieldsToParse[] =
	{
		...
		{ (void *)&vap_stats.errors_received, &numOfValidArgs[6], DWPAL_INT_PARAM,           "ErrorsReceived=",  0 },
		/* Must be at the end */
		{ NULL, NULL, DWPAL_NUM_OF_PARSING_TYPES, NULL, 0 }
	};
...
bool mon_wlan_hal_dwpal::update_stations_stats(const std::string vap_iface_name, const std::string sta_mac, SStaStats& sta_stats)
{
	char *reply = nullptr;
	...
	uint64_t BytesSent=0, BytesReceived=0, PacketsSent=0, PacketsReceived=0, LastDataDownlinkRate=0, LastDataUplinkRate=0;
	...
	FieldsToParse fieldsToParse[] =
	{
		...
		{ (void *)ShortTermRSSIAverage,     &numOfValidArgs[5], DWPAL_STR_PARAM,           "ShortTermRSSIAverage=", sizeof(ShortTermRSSIAverage) },
		{ (void *)&LastDataDownlinkRate,    &numOfValidArgs[6], DWPAL_LONG_LONG_INT_PARAM, "LastDataDownlinkRate=", 0 },
		{ (void *)&LastDataUplinkRate,      &numOfValidArgs[7], DWPAL_LONG_LONG_INT_PARAM, "LastDataUplinkRate=",   0 },
...
bool mon_wlan_hal_dwpal::sta_beacon_11k_request(const SBeaconRequest11k& req, int& dialog_token)
{
	...
	case SBeaconRequest11k::MeasurementMode::Passive:
		measurement_mode = "passive"; break;
	case SBeaconRequest11k::MeasurementMode::Active:
		measurement_mode = "active"; break;
	case SBeaconRequest11k::MeasurementMode::Table:
		measurement_mode = "table"; break;
	default:
		LOG(WARNING) << "Invalid measuremetn mode: " << int(req.measurement_mode) << ", using PASSIVE...";
...
bool mon_wlan_hal_dwpal::process_dwpal_event(char *buffer, int bufLen, const std::string& opcode)
{
	...
	case Event::RRM_Beacon_Response:
	{
		...
		LOG(DEBUG) << "numOfValidArgs[4]= " << numOfValidArgs[4] << " Retransmissions= " << (int)resp->dialog_token;
		LOG(DEBUG) << "numOfValidArgs[5]= " << numOfValidArgs[5] << " measurement_rep_mode= " << (int)resp->rep_mode;
		LOG(DEBUG) << "numOfValidArgs[6]= " << numOfValidArgs[6] << " op_class= " << (int)resp->op_class;
		LOG(DEBUG) << "numOfValidArgs[7]= " << numOfValidArgs[7] << " duration= " << (int)resp->duration;
