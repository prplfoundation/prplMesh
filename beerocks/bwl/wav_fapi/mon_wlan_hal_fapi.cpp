...
static void wav_fapi_debug_obj(const std::shared_ptr<ObjList> &debug_obj, const std::string &label)
{
...
mon_wlan_hal_fapi::mon_wlan_hal_fapi(std::string iface_name, hal_event_cb_t callback)
...
bool mon_wlan_hal_fapi::update_radio_stats(SRadioStats &radio_stats)
{
    ...
    if (fapi_wlan_radio_info_get(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
    ...
    // RX Bytes
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.Stats", 0, "BytesReceived", tmpBuff) !=
        UGW_SUCCESS) {
    ...
    // TX Packets
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.Stats", 0, "PacketsSent", tmpBuff) !=
        UGW_SUCCESS) {
    ...
    // RX Packets
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.Stats", 0, "PacketsReceived", tmpBuff) !=
        UGW_SUCCESS) {
    ...
    // RX Errors
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.Stats", 0, "ErrorsReceived", tmpBuff) !=
        UGW_SUCCESS) {
...
bool mon_wlan_hal_fapi::update_vap_stats(const std::string vap_iface_name, SVapStats &vap_stats)
{
    ...
    // RX Bytes
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.SSID.Stats", 0, "BytesReceived", tmpBuff) !=
        UGW_SUCCESS) {
        LOG(ERROR) << "BytesReceived is missing from the FAPI object!";
        return false;
    }
    ...
    // RX Packets
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.SSID.Stats", 0, "PacketsReceived", tmpBuff) !=
        UGW_SUCCESS) {
        LOG(ERROR) << "PacketsReceived is missing from the FAPI object!";
        return false;
    }
    ...
    // RX Errors
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.SSID.Stats", 0, "ErrorsReceived", tmpBuff) !=
        UGW_SUCCESS) {
        LOG(ERROR) << "ErrorsReceived is missing from the FAPI object!";
        return false;
    }
    ...
    // Retransmissions
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.SSID.Stats", 0, "RetransCount", tmpBuff) !=
        UGW_SUCCESS) {
...
bool mon_wlan_hal_fapi::update_stations_stats(const std::string vap_iface_name,
                                              const std::string sta_mac, SStaStats &sta_stats)
{
    ...
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", "MACAddress",
                          sta_mac.c_str(), 0, 0);

    // Read STA statistics
    if (fapi_wlan_sta_measurement_get(vap_iface_name.c_str(), wlObj.get(), 0) == UGW_FAILURE) {
        LOG(ERROR) << "fapi_wlan_sta_measurement_get failed! sta_mac = " << sta_mac
                   << " vap = " << vap_iface_name;
        return false;
    }

    // Short Term RSSI
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", 0, "ShortTermRSSIAverage",
                       tmpBuff) != UGW_SUCCESS) {
    ...
    // Short Term SNR
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", 0, "SNR", tmpBuff) !=
        UGW_SUCCESS) {
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", 0, "LastDataDownlinkRate",
                       tmpBuff) != UGW_SUCCESS) {
    ...
    // Last Uplink (RX) Rate
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", 0, "LastDataUplinkRate",
                       tmpBuff) != UGW_SUCCESS) {
    ...
    // TX Bytes
    if (UGW_READ_VALUE(wlObj, "Device.WIFI.AccessPoint.AssociatedDevice.Stats", 0, "BytesSent",
                       tmpBuff) != UGW_SUCCESS) {
        LOG(ERROR) << "BytesSent is missing from the FAPI object!";
        return false;
    }
    ...
    // RX Bytes
    if (UGW_READ_VALUE(wlObj, "Device.WIFI.AccessPoint.AssociatedDevice.Stats", 0, "BytesReceived",
                       tmpBuff) != UGW_SUCCESS) {
        LOG(ERROR) << "BytesReceived is missing from the FAPI object!";
        return false;
    }
    ...
    // TX Packets
    if (UGW_READ_VALUE(wlObj, "Device.WIFI.AccessPoint.AssociatedDevice.Stats", 0, "PacketsSent",
                       tmpBuff) != UGW_SUCCESS) {
        LOG(ERROR) << "PacketsSent is missing from the FAPI object!";
        return false;
    }
    ...
    // RX Packets
    if (UGW_READ_VALUE(wlObj, "Device.WIFI.AccessPoint.AssociatedDevice.Stats", 0,
                       "PacketsReceived", tmpBuff) != UGW_SUCCESS) {
        LOG(ERROR) << "PacketsReceived is missing from the FAPI object!";
        return false;
    }
    ...
    // Retranmission Count
    if (UGW_READ_VALUE(wlObj, "Device.WIFI.AccessPoint.AssociatedDevice.Stats", 0, "RetransCount",
                       tmpBuff) != UGW_SUCCESS) {
        LOG(ERROR) << "RetransCount is missing from the FAPI object!";
        return false;
    }
...
bool mon_wlan_hal_fapi::sta_channel_load_11k_request(const SStaChannelLoadRequest11k &req)
{
    ...
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "dst",
                          beerocks::net::network_utils::mac_to_string(req.sta_mac.oct).c_str(), 0,
                          0);
    ...
    if (req.use_optional_ch_load_rep == true) {
        auto ch_load_rep =
            std::to_string(req.ch_load_rep_first) + "," + std::to_string(req.ch_load_rep_second);
    ...
    if (req.use_optional_wide_band_ch_switch == true) {
        auto wide_band_ch_switch = std::to_string(req.new_ch_width) + "," +
    ...
    if (fapi_wlan_channel_load_request(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool mon_wlan_hal_fapi::sta_beacon_11k_request(const SBeaconRequest11k &req, int &dialog_token)
{
    ...
    // Operating Class
    UGW_OBJLIST_EDIT_NODE(
        wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "op_class",
        std::to_string(((req.op_class < 0) ? GET_OP_CLASS(get_radio_info().channel) : req.op_class))
            .c_str(),
        0, 0);
    ...
    // ???
    if (req.use_optional_wide_band_ch_switch == true) {
        std::string wide_band_ch_switch = std::to_string(req.new_ch_width) + "," +
                                          std::to_string(req.new_ch_center_freq_seg_0) + "," +
                                          std::to_string(req.new_ch_center_freq_seg_1);
    ...
    // Execute the FAPI command
    if (fapi_wlan_beacon_request(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "dialog_token",
                       tmpBuff) != UGW_SUCCESS) {
...
bool mon_wlan_hal_fapi::sta_statistics_11k_request(const SStatisticsRequest11k &req)
{
    ...
    // Peer MAC Address
    UGW_OBJLIST_EDIT_NODE(
        wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "peer_mac_addr",
    ...
    if (req.use_optional_trig_rep_sta_counters) {
        std::string trig_rep_sta_counters =
            std::to_string(req.measurement_count_1) + "," + std::to_string(req.trigger_timeout_1) +
        ...
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor",
                              "trig_rep_sta_counters", trig_rep_sta_counters.c_str(), 0, 0);
    }

    if (req.use_optional_trig_rep_qos_sta_counters) {
        std::string trig_rep_qos_sta_counters =
            std::to_string(req.measurement_count_2) + "," + std::to_string(req.trigger_timeout_2) +
        ...
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor",
                              "trig_rep_qos_sta_counters", trig_rep_qos_sta_counters.c_str(), 0, 0);
    }

    if (req.use_optional_trig_rep_rsna_counters) {
        std::string trig_rep_rsna_counters =
            std::to_string(req.measurement_count_3) + "," + std::to_string(req.trigger_timeout_3) +
        ...
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor",
                              "trig_rep_rsna_counters", trig_rep_rsna_counters.c_str(), 0, 0);
    }

    // Execute the FAPI command
    if (fapi_wlan_sta_statistics_request(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool mon_wlan_hal_fapi::sta_link_measurements_11k_request(const std::string &sta_mac)
{
    ...
    // STA MAC Address
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "dst", sta_mac.c_str(), 0,
                          0);

    // Execute the FAPI function
    if (fapi_wlan_sta_link_measurements_request(get_radio_info().iface_name.c_str(), wlObj.get(),
                                                0) == UGW_FAILURE) {
...
bool mon_wlan_hal_fapi::parse_fapi_event(const std::string &opcode, std::shared_ptr<void> obj)
{
    ...
    if (event == mon_wlan_hal::Event::Invalid) {
        return true;
    }
    ...
    case Event::RRM_Channel_Load_Response: {
        ...
        // STA MAC
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", 0, "MACAddress",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0,
                           "measurement_rep_mode", tmpBuff) != UGW_SUCCESS) {
        ...
        // Operating Class
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "op_class",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "start_time",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "duration",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // Channel Load
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "channel_load",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "wide_band_ch_switch",
                           tmpBuff) == UGW_SUCCESS) {
    ...
    case Event::RRM_Beacon_Response: {
        ...
        // Frame information
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "frame_info",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // RCPI
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "rcpi", tmpBuff) !=
            UGW_SUCCESS) {
        ...
        // RSNI
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "rsni", tmpBuff) !=
            UGW_SUCCESS) {
        ...
        // BSSID
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "bssid", tmpBuff) !=
            UGW_SUCCESS) {
        ...
        // Antenna ID
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "antenna_id",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // Parent TSF
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "parent_tsf",
                           tmpBuff) != UGW_SUCCESS) {
    ...
    case Event::RRM_STA_Statistics_Response: {
        ...
        // Group Identity
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "group_identity",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0,
                           "statistics_group_data", tmpBuff) != UGW_SUCCESS) {
        ...
        // Response Reason?
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "rep_reason",
                           tmpBuff) != UGW_SUCCESS) {
    ...
    case Event::RRM_Link_Measurement_Response: {
        ...
        // TPC Report
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "tpc_report",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // RX Antenna ID
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "rx_ant_id",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // TX Antenna ID
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "tx_ant_id",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // ???
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "dmg_link_margin",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        // ???
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "dmg_link_adapt_ack",
                           tmpBuff) != UGW_SUCCESS) {
