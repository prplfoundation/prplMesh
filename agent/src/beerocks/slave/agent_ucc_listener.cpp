                                              ...
                                              std::string &value)
{
    ...
    if (parameter == "alid") {
        ...
        auto db = AgentDB::get();
        auto radio =
            db->get_mac_parent_radio(tlvf::mac_from_string(ruid), AgentDB::eMacType::RADIO);
        if (!radio) {
                                                       ...
                                                       std::string &err_string)
{
    ...
    auto radio_found = db->get_mac_parent_radio(radio_mac);
    if (!radio_found) {
        LOG(ERROR) << "radio_mac " + radio_mac_str + " not found";
        err_string = "Failed to get radio for " + band;
        return false;
    }
    ...
    return m_backhaul_manager_ctx.start_wps_pbc(radio_mac);
}
