                                                     ...
                                                     ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    case beerocks_message::ACTION_BACKHAUL_CLIENT_ASSOCIATED_NOTIFICATION: {
        ...
        s.radio_mac  = msg->iface_mac();
        s.bssid      = msg->bssid();
        s.time_stamp = std::chrono::steady_clock::now();
                                                      ...
                                                      const std::string &src_mac)
{
    ...
    if (associated_client_found) {
        ...
        auto assoc_    = s.assoc_req;
        auto assoc_req = std::string(assoc_, assoc_ + s.asso_len);

        auto assoc_frame_body = string_utils::hex_to_char_string(assoc_req);

        LOG(DEBUG) << "******************************************************************";
        LOG(DEBUG) << "          sAssociatedClients.asso_len / 2 = " << s.asso_len / 2;
        LOG(DEBUG) << "          assoc_req.len = " << assoc_req.length() / 2;
        LOG(DEBUG) << "          assoc_frame_body.len = " << assoc_frame_body.length();

        LOG(DEBUG) << "******************************************************************";
        client_capability_report_tlv->alloc_association_frame(s.asso_len / 2);
        std::copy(&assoc_frame_body[0], &assoc_frame_body[0] + s.asso_len / 2,
    ...
    for (const auto &slave : m_radio_info_map) {
        ...
        for (const auto &client : associated_clients_map) {
            if (client.second.find(client_mac) != client.second.end()) {
                m_radio_info_map.erase(client_mac);
            }
        }
    }
}
