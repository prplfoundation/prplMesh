    ...
    for (auto it = slaves_sockets.begin(); it != slaves_sockets.end();) {
        ...
        if (soc->slave == sd) {
            ...
            if (m_eFSMState >= EState::CONNECT_TO_MASTER) {
                ...
                send_cmdu_to_bus(cmdu_tx, network_utils::MULTICAST_1905_MAC_ADDR,
...
bool backhaul_manager::backhaul_fsm_main(bool &skip_select)
{
    ...
    case EState::ENABLED: {
        ...
        if (db->device_conf.local_controller && db->device_conf.local_gw) {
            ...
            if ((wired_link_state == wan_monitor::ELinkState::eUp) &&
                ...
                auto selected_ruid = db->get_mac_parent_radio(
                    tlvf::mac_from_string(selected_backhaul), AgentDB::eMacType::RADIO);
    ...
    case EState::MASTER_DISCOVERY: {
        ...
        if (!bus_connect(beerocks_temp_path, db->device_conf.local_controller)) {
...
bool backhaul_manager::send_1905_topology_discovery_message(const std::string &iface_name)
{
    ...
    LOG(DEBUG) << "send_1905_topology_discovery_message, bridge_mac=" << db->bridge.mac
               << ", iface=" << iface_name;
    return send_cmdu_to_bus(cmdu_tx, network_utils::MULTICAST_1905_MAC_ADDR,
...
bool backhaul_manager::handle_cmdu(Socket *sd, ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    if (from_bus(sd)) {
        ...
        if (dst_mac == network_utils::ZERO_MAC_STRING) {
            ...
            cmdu_rx.swap(); //swap back before forwarding
            send_cmdu_to_bus(cmdu_rx, dst_mac, tlvf::mac_to_string(db->bridge.mac), length);
        }
    }
                                                     ...
                                                     ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    case beerocks_message::ACTION_BACKHAUL_CLIENT_ASSOCIATED_NOTIFICATION: {
        ...
        auto radio = db->get_mac_parent_radio(msg->bssid(), AgentDB::eMacType::BSSID);
        if (!radio) {
    ...
    case beerocks_message::ACTION_BACKHAUL_ASSOCIATED_STA_LINK_METRICS_RESPONSE: {
        ...
        for (size_t i = 0; i < response_out->bssid_info_list_length(); ++i) {
            ...
            auto radio = db->get_mac_parent_radio(client_mac, AgentDB::eMacType::CLIENT);
            if (!radio) {
                LOG(ERROR) << "radio for client mac " << client_mac << " not found";
                return false;
            }

            // If get_mac_parent_radio() found the radio, it means that 'client_mac' is on the radio
        ...
        send_cmdu_to_bus(cmdu_tx, controller_bridge_mac, tlvf::mac_to_string(db->bridge.mac));
        break;
    }
    default: {
                                                             ...
                                                             const std::string &src_mac)
{
    ...
    return send_cmdu_to_bus(cmdu_tx, src_mac, tlvf::mac_to_string(db->bridge.mac));
}
                                                                ...
                                                                const std::string &src_mac)
{
    ...
    auto radio = db->get_mac_parent_radio(mac->sta_mac(), AgentDB::eMacType::CLIENT);
    if (!radio) {
    ...
    const auto &bssid = radio->associated_clients.at(mac->sta_mac()).bssid;
                                                      ...
                                                      const std::string &src_mac)
{
    ...
    auto radio =
        db->get_mac_parent_radio(client_info_tlv_r->client_mac(), AgentDB::eMacType::CLIENT);
    if (!radio) {
                                                        ...
                                                        const std::string &src_mac)
{
    ...
    if (0 == mid) {
        uint16_t length = message_com::get_uds_header(cmdu_rx)->length;
        cmdu_rx.swap(); //swap back before forwarding
        return send_cmdu_to_bus(cmdu_rx, controller_bridge_mac, tlvf::mac_to_string(db->bridge.mac),
                                length);
    }
    ...
    return send_cmdu_to_bus(cmdu_tx, controller_bridge_mac, tlvf::mac_to_string(db->bridge.mac));
}
                                                        ...
                                                        Socket *&forward_to)
{
    ...
    auto db    = AgentDB::get();
    auto radio = db->get_mac_parent_radio(requested_sta_mac, AgentDB::eMacType::CLIENT);
    if (!radio) {
    ...
    auto radio_info = get_radio(radio->front.iface_mac);
    if (!radio_info) {
        LOG(ERROR) << "radio_info is nullptr";
        return false;
    }
    ...
    send_cmdu_to_bus(cmdu_tx, src_mac, tlvf::mac_to_string(db->bridge.mac));
...
const std::string backhaul_manager::freq_to_radio_mac(eFreqType freq) const
{
    ...
    LOG(ERROR) << "couldn't find radio for freq " << int(freq);
    return std::string();
}
