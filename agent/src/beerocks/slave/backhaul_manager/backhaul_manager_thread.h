...
public:
     ...
     */
    const std::string freq_to_radio_mac(enum eFreqType freq) const;

    /**
     * @brief start WPS PBC
     * 
     * @param radio_mac radio mac of the radio on which to start WPS
     * @param bssid bssid on which to start WPS
     * @return true on success, false on failure
     */
    bool start_wps_pbc(const sMacAddr &radio_mac, const sMacAddr &bssid);
