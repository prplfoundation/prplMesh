...
static mon_wlan_hal_create_t s_obj_create   = nullptr;
static mon_wlan_hal_destroy_t s_obj_destroy = nullptr;
                                       ...
                                       bwl::base_wlan_hal::hal_event_cb_t cb)
{
    ...
    if (!s_sharedLib) {
        ...
        // Creator function
        s_obj_create     = (mon_wlan_hal_create_t)(dlsym(s_sharedLib.get(), "mon_wlan_hal_create"));
        auto dlsym_error = dlerror();
        if (!s_obj_create || dlsym_error) {
            LOG(ERROR) << "Failed loading 'mon_wlan_hal_create': " << dlsym_error;
            return nullptr;
        }

        // Destroy function
        s_obj_destroy = (mon_wlan_hal_destroy_t)(dlsym(s_sharedLib.get(), "mon_wlan_hal_destroy"));
        dlsym_error   = dlerror();
        if (!s_obj_destroy || dlsym_error) {
            LOG(ERROR) << "Failed loading 'mon_wlan_hal_destroy': " << dlsym_error;
            return nullptr;
        }
    }
...
{
    if (!s_obj_destroy) {
        LOG(FATAL) << "Attempted destroy of 'mon_wlan_hal' object without creation!";
    } else {
        // Destroy the object
        s_obj_destroy(obj);
    }
