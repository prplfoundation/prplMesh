...
public:
    ...
    enum neighbor_type { NEIGHBOR_1905, NEIGHBOR_NON_1905 };
    ...
    struct interface {
        ...
        bool
            transport; /*!< TODO Bit value indicating if interface is transport interface connecting to other devices in network */
        int status;
    ...
    struct interface_list_item {
        ...
        bool operator==(const interface_list_item &interface2) const
        {
            ...
            if (memcmp(interface2.instance.mac, testmac, ETH_ALEN) != 0 &&
                memcmp(this->instance.mac, testmac, ETH_ALEN) != 0) {
