...
int TopologyDiscoveryAgentLocalDB::HandleInterfaceEvent(struct interface interface_instance)
{
	...
	std::copy_n((uint8_t*)&interface_instance, sizeof(interface_instance), (uint8_t*)&search_interface.instance);
	auto it = std::find(interface_list.begin(),interface_list.end(),search_interface);
	if(it != interface_list.end()) {
		if((it->instance.status != search_interface.instance.status) && (interface_instance.status == 0)) {
			// delete operation
...
void TopologyDiscoveryAgentLocalDB::ShowInterfaceList()
{
	...
	for(auto it : interface_list) {
		MAPF_DBG(it.instance.index << "\t"
			<< it.instance.parent_bridge_index << "\t"
 ...
 */
int TopologyDiscoveryAgentLocalDB::UpdateNeighborEntry(int index, uint8_t *mac, uint8_t *neigh_mac, int type, int oper)
{
	int return_value = 0;
	struct interface_list_item search_interface(index, mac);
	struct neighbor search_neighbor(neigh_mac);
	auto interface_entry = std::find(interface_list.begin(),interface_list.end(),search_interface);

	MAPF_DBG("search for interface entry with index " << index);
	if(interface_entry != interface_list.end()) {
	MAPF_DBG("interface entry found");
		auto neighbor_entry = std::find(interface_entry->neighbor_list.begin(),interface_entry->neighbor_list.end(),search_neighbor);
...
void TopologyDiscoveryAgentLocalDB::ShowNeighborList(std::list<struct interface_list_item>::iterator it1)
{
	...
	for(auto it2 : it1->neighbor_list) {
		MAPF_DBG("Neighbor : MAC - " << std::hex << std::setfill('0') << std::setw(2) << (unsigned)it2.mac[0] 
					...
					<< ", type - " << it2.type
					<< ", bridge exists - " << it2.bridge_exists);
	}
}
