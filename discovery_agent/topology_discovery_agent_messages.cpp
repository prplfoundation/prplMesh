		...
		if (revents & MAPF_POLLIN) {
			...
			if(auto message2 = dynamic_cast<CmduRxMessage *>(message1.get())) {
				...
				if(message2->metadata()->ether_type == ETH_P_LLDP) {
					...
					struct topology_notification_format *topology_notification = (struct topology_notification_format *)(message2->data() + 8);
					HandleTopologyNotify(topology_notification);
				}
			} else if(auto message3 = dynamic_cast<InterfaceConfigurationIndicationMessage *>(message1.get())) {
				...
				if(message->topic().compare(da_topics[0]) == 0) {
					HandleTopologyNotify((struct topology_notification_format *)message->frame().data());
					...
					HandleClientAssociationEvent((struct associated_client_list *)message->frame().data());
				} else {
...
int DiscAgent::HandleLocalBusInterfaceIndication(mapf::InterfaceConfigurationIndicationMessage::Metadata *interface_metadata)
{
	...
	for(i=0; i < int(interface_metadata->numInterfaces); i++) {
		...
		interface_instance.transport = interface_metadata->interfaces[i].flags & Flags::ENABLE_IEEE1905_TRANSPORT;
		interface_instance.ieee802_1bridge = interface_metadata->interfaces[i].flags & Flags::IS_BRIDGE;
		...
		if(return_value & (1<<2)) {
			...
			std::copy_n((uint8_t*)&interface_instance, sizeof(interface_instance), (uint8_t*)&interface_list_instance.instance);
...
{
	uint8_t	higher_layer_response_tlv_buffer[MAX_DATA_SIZE] = {0}, ipv4_address[MAX_IPv4_ADDR_LEN] = {0};
	uint8_t	ipv6_local_address[MAX_IPv6_ADDR_LEN];
	struct Ieee1905CmduHeader ieee1905_header(HIGHER_LAYER_RESPONSE_MESSAGE, CMDU_NO_FRAGMENT_INDICATOR);
	...
	auto url = "http://" + std::to_string(ipv4_address[0]) + '.'
						 + std::to_string(ipv4_address[1]) + '.'
						 + std::to_string(ipv4_address[2]) + '.'
						 + std::to_string(ipv4_address[3]) + '/';
	...
	copied_bytes = sizeof(struct higher_layer_response_format) - MAX_URL_LEN + higher_layer_response.control_url_tlv.length;
	std::copy_n((uint8_t*)&higher_layer_response, copied_bytes, (uint8_t*)&higher_layer_response_tlv_buffer[higher_layer_response_tlv_size]);
	higher_layer_response_tlv_size += copied_bytes;

	higher_layer_response_tlv_buffer[higher_layer_response_tlv_size++] = 1;
	std::copy_n(almac_, ETH_ALEN, (uint8_t*)&higher_layer_response_tlv_buffer[higher_layer_response_tlv_size]);
	...
	std::copy_n(ipv4_address, MAX_IPv4_ADDR_LEN, (uint8_t*)&higher_layer_response_tlv_buffer[higher_layer_response_tlv_size]); // IP address in octet format
	higher_layer_response_tlv_size += MAX_IPv4_ADDR_LEN;
	memset((uint8_t*)&higher_layer_response_tlv_buffer[higher_layer_response_tlv_size], 0, MAX_IPv4_ADDR_LEN); // DHCP server unknown:all 0's
	higher_layer_response_tlv_size += MAX_IPv4_ADDR_LEN;

	// TODO assume last 3 octets of AL MAC as local IPv6 IP, until mAP provides this in separate message
	higher_layer_response_tlv_buffer[higher_layer_response_tlv_size++] = 1;

	std::copy_n(std::array<uint8_t,16>({0xfe, 0x80, 0x00, 0x00, 
										0x00, 0x00, 0x00, 0x00, 
										0x00, 0x9a, 0x96, 0xff, 
										0xfe, 0xf3, 0x58, 0x30}).data() , 13, ipv6_local_address); // array contain 16, but we copy only 13
	...
	higher_layer_response_message.metadata()->length = sizeof(ieee1905_header) + higher_layer_response_tlv_size + 3; // end of message tlv : 3

	std::copy_n((uint8_t*)&ieee1905_header, sizeof(ieee1905_header), higher_layer_response_message.data());
	std::copy_n(higher_layer_response_tlv_buffer, higher_layer_response_tlv_size, higher_layer_response_message.data() + sizeof(ieee1905_header));

	memset(higher_layer_response_message.data() + sizeof(ieee1905_header) + higher_layer_response_tlv_size, 0, 3);
...
{
	int	return_value = 0;
	struct topology_discovery_format *topology_discovery = (struct topology_discovery_format *)(rxmsg->data() + 8);

	return_value = UpdateNeighborEntry(rxmsg->metadata()->if_index, NULL, topology_discovery->al_mac_tlv.almac, NEIGHBOR_1905, 1); // oper is add
...
int DiscAgent::HandleLldpDiscovery(CmduRxMessage *rxmsg)
{
	...
	return_value = UpdateNeighborEntry(rxmsg->metadata()->if_index, NULL, lldp_discovery->port_id.id, NEIGHBOR_1905, 2); // oper is update
...
{
	int	return_value = 0;
	struct Ieee1905CmduHeader ieee1905_header(TOPOLOGY_DISCOVERY_MESSAGE, CMDU_NO_FRAGMENT_INDICATOR);
	...
	std::copy_n(std::array<uint8_t,16>({0x01, 0x80, 0xc2, 0x00, 0x00, 0x0e}).data(), ETH_ALEN, lldp_discovery_message.metadata()->dst);
	...
	std::copy_n(std::array<uint8_t,16>({0x01, 0x80, 0xc2, 0x00, 0x00, 0x13}).data(), ETH_ALEN, topology_discovery_message.metadata()->dst);
	std::copy_n(interfacelistitem.instance.mac, ETH_ALEN, topology_discovery_message.metadata()->src);
	...
	topology_discovery_message.metadata()->length = sizeof(ieee1905_header) + sizeof(topology_discovery) + 3; // end of message tlv : 3
	...
	std::copy_n((uint8_t*)&topology_discovery, sizeof(topology_discovery), topology_discovery_message.data() + 8);
	memset(topology_discovery_message.data() + sizeof(ieee1905_header) + sizeof(topology_discovery), 0, 3);
...
int DiscAgent::HandleTopologyQuery(CmduRxMessage *rxmsg)
{
	...
	struct Ieee1905CmduHeader ieee1905_header(TOPOLOGY_RESPONSE_MESSAGE, CMDU_NO_FRAGMENT_INDICATOR);
	// TODO optimize stack size
	char	neighbor_1905_tlv[MAX_DATA_SIZE] = {0}, neighbor_non_1905_tlv[MAX_DATA_SIZE] = {0};
	uint8_t	topology_response_tlv[MAX_DATA_SIZE] = {0}, ap_operational_bss_tlv[MAX_DATA_SIZE] = {0}, associated_clients_tlv[MAX_DATA_SIZE] = {0};	
	uint8_t type, interface_count, mac_buffer[MAX_DATA_SIZE] = {0}, mac_buffer_size = 0;
	char	ssid_length;
	uint16_t neighbor_1905_tlv_size, neighbor_non_1905_tlv_size, ap_operational_bss_tlv_size, associated_clients_tlv_size;
	...
	std::copy_n((uint8_t*)&tlv_length, sizeof(tlv_length), topology_response_tlv + topology_response_tlv_size);
	...
	std::copy_n((uint8_t*)&interface_count, sizeof(interface_count), topology_response_tlv + topology_response_tlv_size);
	topology_response_tlv_size += 1;
	i = 0;
	for(auto it : interface_list) {
		std::copy_n(it.instance.mac, ETH_ALEN, topology_response_tlv + topology_response_tlv_size);
		topology_response_tlv_size += ETH_ALEN;
		std::copy_n((uint8_t*)&it.instance.media_type, 2, topology_response_tlv + topology_response_tlv_size);
		topology_response_tlv_size += 2;
	...
	topology_response_tlv_size += 1;
	topology_response_tlv[topology_response_tlv_size] = interface_count; // skip 1 octet for number of MAC addresses in bridging tuple
	topology_response_tlv_size += 1;
	...
	topology_response_tlv[topology_response_tlv_size++] = 6; // non-1905 neighbor device list tlv type
	std::copy_n((uint8_t*)&neighbor_non_1905_tlv_size, sizeof(neighbor_non_1905_tlv_size), &topology_response_tlv[topology_response_tlv_size]); // non-1905 neighbor device list tlv data length
	topology_response_tlv_size += 2;
	std::copy_n(neighbor_non_1905_tlv, neighbor_non_1905_tlv_size, &topology_response_tlv[topology_response_tlv_size]); // tlv data
	...
	std::copy_n((uint8_t*)&neighbor_1905_tlv_size, sizeof(neighbor_1905_tlv_size), &topology_response_tlv[topology_response_tlv_size]); // 1905 neighbor device list tlv data length
	topology_response_tlv_size += 2;
	std::copy_n(neighbor_1905_tlv, neighbor_1905_tlv_size, &topology_response_tlv[topology_response_tlv_size]); // tlv data
	...
	std::copy_n((uint8_t*)&number_of_map_services, sizeof(number_of_map_services), &topology_response_tlv[topology_response_tlv_size]); // 3 octets as put below
	topology_response_tlv_size += 2;
	...
	for(i=0; i<ap_bss_info_.ap_count; i++) {
		std::copy_n(ap_bss_info_.ap_list[i].apmac, ETH_ALEN, &ap_operational_bss_tlv[ap_operational_bss_tlv_size]);
		...
		for(j=0; j<ap_bss_info_.ap_list[i].bss_count; j++) {
			std::copy_n(ap_bss_info_.ap_list[i].bss_list[j].bssmac, ETH_ALEN, &ap_operational_bss_tlv[ap_operational_bss_tlv_size]);
			ap_operational_bss_tlv_size += ETH_ALEN;
			...
			ap_operational_bss_tlv_size += 1;
			std::copy_n(ap_bss_info_.ap_list[i].bss_list[j].ssid, ssid_length, &ap_operational_bss_tlv[ap_operational_bss_tlv_size]);
			ap_operational_bss_tlv_size += ssid_length;
		}
	}
	topology_response_tlv[topology_response_tlv_size++] = 0x83; // AP Operational BSS TLV type
	std::copy_n((uint8_t*)&ap_operational_bss_tlv_size, sizeof(ap_operational_bss_tlv_size), &topology_response_tlv[topology_response_tlv_size]); // AP Operational BSS TLV data length
	topology_response_tlv_size += 2;
	std::copy_n(ap_operational_bss_tlv, ap_operational_bss_tlv_size, &topology_response_tlv[topology_response_tlv_size]); // tlv data
	...
	for(i=0; i<associated_client_info_.bss_count; i++) {
		std::copy_n(associated_client_info_.bss_list[i].bssmac, ETH_ALEN, &associated_clients_tlv[associated_clients_tlv_size]);
		associated_clients_tlv_size += ETH_ALEN;
		std::copy_n((uint8_t*)&associated_client_info_.bss_list[i].client_count, sizeof(associated_client_info_.bss_list[i].client_count), &associated_clients_tlv[associated_clients_tlv_size]);
		associated_clients_tlv_size += 2;
		std::copy_n((uint8_t*)associated_client_info_.bss_list[i].client_list, associated_client_info_.bss_list[i].client_count * sizeof(associated_client_info_.bss_list[i]), &associated_clients_tlv[associated_clients_tlv_size]);
		associated_clients_tlv_size += associated_client_info_.bss_list[i].client_count * sizeof(associated_client_info_.bss_list[i].client_list[0]);
	}
	topology_response_tlv[topology_response_tlv_size++] = 0x84; // Associated Clients TLV type
	std::copy_n((uint8_t*)&associated_clients_tlv_size, sizeof(associated_clients_tlv_size), &topology_response_tlv[topology_response_tlv_size]); // Associated Clients TLV data length
	topology_response_tlv_size += 2;
	std::copy_n(associated_clients_tlv, associated_clients_tlv_size, &topology_response_tlv[topology_response_tlv_size]);  // tlv data
	...
	topology_response_message.metadata()->if_index = rxmsg->metadata()->if_index; // TODO check if correct or to use almac_ ?

	topology_response_message.metadata()->length = sizeof(ieee1905_header) + topology_response_tlv_size + 3; // end of message tlv : 3

	std::copy_n((uint8_t*)&ieee1905_header, sizeof(ieee1905_header), topology_response_message.data());
	std::copy_n(topology_response_tlv, topology_response_tlv_size, topology_response_message.data() + sizeof(ieee1905_header));

	memset(topology_response_message.data() + sizeof(ieee1905_header) + topology_response_tlv_size, 0, 3);
...
int DiscAgent::HandleWlanApEvent(struct ap_operational_bss *rx_ap_bss_info)
{
	...
	std::copy_n((uint8_t*)&rx_ap_bss_info->ap_list, rx_ap_bss_info->ap_count * sizeof(ap_bss_info_.ap_list[0]), (uint8_t*)&ap_bss_info_.ap_list);

	return 0;
}

int DiscAgent::HandleClientAssociationEvent(struct associated_client_list *rx_associated_client_info)
{
	// TODO invoke member function of TopologyDiscoveryAgentLocalDB so that it's stored in that class
	associated_client_info_.bss_count = rx_associated_client_info->bss_count;
	std::copy_n((uint8_t*)&rx_associated_client_info->bss_list, rx_associated_client_info->bss_count * sizeof(rx_associated_client_info->bss_list[0]), (uint8_t*)&associated_client_info_.bss_list);
...
int DiscAgent::HandleTopologyNotify(struct topology_notification_format *topology_notification)
{
	...
	if (memcmp(neighbor_mac, almac_, ETH_ALEN) == 0) {
		return_value = UpdateNeighborEntry(-1, topology_notification->client_association_tlv.bssid, neighbor_mac, NEIGHBOR_1905, (topology_notification->client_association_tlv.flags>> 7));
