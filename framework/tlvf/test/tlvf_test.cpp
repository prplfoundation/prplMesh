...
int test_parser()
{
    ...
    //auto tlv3   = msg.addClass<tlvVendorSpecific>();
    auto tlv4 = msg.addClass<tlvWscM1>();
    ...
    if (!tlv1_)
        errors++;
    // auto tlv2_ = received_message.getClass<tlvVendorSpecific>();
    // if (!tlv2_)
    //     errors++;
    auto tlv3_ = received_message.getClass<tlvLinkMetricQuery>();
    if (!tlv3_)
        errors++;
    auto tlv4_ = received_message.getClass<tlvWscM1>();
    if (!tlv4_)
        errors++;
