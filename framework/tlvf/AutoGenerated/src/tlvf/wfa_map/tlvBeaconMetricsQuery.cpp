...
bool tlvBeaconMetricsQuery::init()
{
    ...
    m_reporting_detail_value = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    ...
    m_elemnt_id_list_length = reinterpret_cast<uint8_t*>(m_buff_ptr__);
...
bool cApChannelReports::init()
{
    ...
    m_ap_channel_report_list_length = reinterpret_cast<uint8_t*>(m_buff_ptr__);
