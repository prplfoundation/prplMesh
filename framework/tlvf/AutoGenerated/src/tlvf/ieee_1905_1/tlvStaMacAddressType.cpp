...
const eTlvType& tlvStaMacAddressType::type() {
...
sMacAddr& tlvStaMacAddressType::mac() {
...
bool tlvStaMacAddressType::init()
{
    ...
    if (!m_parse__) *m_type = eTlvType::TLV_STA_MAC_ADDRESS_TYPE;
    ...
    if (m_parse__) {
        if (*m_type != eTlvType::TLV_STA_MAC_ADDRESS_TYPE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: " << int(eTlvType::TLV_STA_MAC_ADDRESS_TYPE) << ", received value: " << int(*m_type);
            return false;
        }
    }
    return true;
