        ...
        if param_length_type == None:
            ...
            if is_int_len or is_const_len:
                ...
                if obj_meta.is_tlv_class:
                    lines_cpp.append( "if(m_length && !m_parse__){ (*m_length) += (sizeof(%s) * %s); }" % ( param_type, param_length) )
                if TypeInfo(param_type).type == TypeInfo.STRUCT:
            ...
            if (param_val_const or param_val):
                if is_int_len or is_const_len:
                    ...
                    lines_cpp += ["if (!m_parse__) {",
                                 "%sfor (size_t i = 0; i < %s; i++){" % (self.getIndentation(1), str(param_length)),
