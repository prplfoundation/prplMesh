...
int main()
{
    ...
    uint8_t m1_nonce[192];
    uint8_t m2_nonce[192];
    mapf::encryption::create_nonce(m1_nonce, sizeof(m1_nonce));
    mapf::encryption::create_nonce(m2_nonce, sizeof(m2_nonce));
    uint8_t mac[6] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66};
    uint8_t authkey1[32];
    uint8_t keywrapkey1[16];
    check(errors,
          wps_calculate_keys(m1, m2.pubkey(), m2.pubkey_length(), m1_nonce, mac, m2_nonce, authkey1,
                             keywrapkey1),
          "WPS calculate keys");
    uint8_t authkey2[32];
    uint8_t keywrapkey2[16];
    check(errors,
          wps_calculate_keys(m2, m1.pubkey(), m1.pubkey_length(), m1_nonce, mac, m2_nonce, authkey2,
                             keywrapkey2),
          "WPS calculate keys");
