...
diffie_hellman::diffie_hellman(bool force_small_key) : m_dh(nullptr), m_pubkey(nullptr)
{
    ...
    do {
        ...
        MAPF_DBG("Got key length ") << m_pubkey_length;
        if (!force_small_key || m_pubkey_length != 192) {
            break;
        }
        DH_free(m_dh);
    } while (true);
