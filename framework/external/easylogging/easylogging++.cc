...
base::FileStreamPtr TypedConfigurations::sharedFileStream(Level level) {
  return getConfigByRef<base::FileStreamPtr>(level, &m_fileStreamMap, "fileStream");
}
...
bool TypedConfigurations::unsafeValidateFileRolling(Level level, const PreRollOutCallback& preRollOutCallback) {
  // Use shared_ptr to prevent file closing in the middle of operation
  auto fs = unsafeGetConfigByRef(level, &m_fileStreamMap, "fileStream");
  if (fs == nullptr) {
    return true;
  }
...
void DefaultLogDispatchCallback::dispatch(base::type::string_t&& logLine, base::type::string_t&& syslogLine) {
  if (m_data->dispatchAction() == base::DispatchAction::NormalLog) {
    if (m_data->logMessage()->logger()->m_typedConfigurations->toFile(m_data->logMessage()->level())) {
      // Use shared_ptr to prevent file closing in the middle of operation
      auto fs = m_data->logMessage()->logger()->m_typedConfigurations->sharedFileStream(m_data->logMessage()->level());
      if (fs != nullptr) {
