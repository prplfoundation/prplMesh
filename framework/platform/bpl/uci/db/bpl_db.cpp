...
bool db_has_entry(const std::string &entry_type, const std::string &entry_name)
{
    ...
    return db::uci_entry_exists(DB_FILE, entry_type, entry_name);
}
                  ...
                  const std::unordered_map<std::string, std::string> &params)
{
    ...
    // Check if entry of the same name and type exist.
    if (!db::uci_entry_exists(DB_FILE, entry_type, entry_name)) {
        LOG(DEBUG) << "Entry " << entry_name << " does not exist! Create new one.";
        if (!db::uci_add_entry(DB_FILE, entry_type, entry_name)) {
    ...
    return db::uci_set_entry(DB_FILE, entry_type, entry_name, params);
}
                  ...
                  std::unordered_map<std::string, std::string> &params)
{
    ...
    if (params.size() == 0) {
        ...
        return db::uci_get_entry(DB_FILE, entry_type, entry_name, params);
    }
    for (auto &param : params) {
        // params map is not empty, getting selected parameters
        if (!db::uci_get_param(DB_FILE, entry_type, entry_name, param.first, param.second)) {
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, std::string>> &nested_params)
{
    ...
    if (!db::uci_get_all_entries(DB_FILE, entry_type, entries)) {
        LOG(ERROR) << "Failed to get all entries";
        return false;
    }
    for (auto &entry_name : entries) {
        std::unordered_map<std::string, std::string> params;
        if (!db::uci_get_entry(DB_FILE, entry_type, entry_name, params)) {
            LOG(ERROR) << "Failed to get entry " << entry_name;
            return false;
        }
        nested_params[entry_name] = params;
    }
...
bool db_remove_entry(const std::string &entry_type, const std::string &entry_name)
{
    ...
    return db::uci_delete_entry(DB_FILE, entry_type, entry_name);
}
