...
cal_object_list::iterator &cal_object_list::iterator::
operator=(const cal_object_list::iterator &src)
{
    if (this != &src) {
        (*m_it) = (*src.m_it);
    }
...
cal_object_list::iterator &cal_object_list::iterator::operator++()
{
    ...
    m_it->m_obj = reinterpret_cast<ObjList *>(reinterpret_cast<char *>(m_it->m_obj->xOlist.next) -
                                              offsetof(ObjList, xOlist));
    return *this;
}
...
cal_object_list::iterator &cal_object_list::iterator::operator--()
{
    ...
    m_it->m_obj = reinterpret_cast<ObjList *>(reinterpret_cast<char *>(m_it->m_obj->xOlist.prev) -
                                              offsetof(ObjList, xOlist));
    return *this;
}
...
cal_object_list::iterator::pointer cal_object_list::iterator::operator->() { return m_it.get(); }

cal_object_list::cal_object_list(ObjList *head, const std::string &name, uint32_t obj_op,
                                 uint16_t sid, uint32_t sub_op)
...
cal_object_list::cal_object_list(cal_object_list &list_obj, const std::string &name,
                                 uint32_t obj_op, uint16_t sid)
...
cal_object_list::cal_object_list(cal_object_list &&src) noexcept
    : m_head(src.m_head), m_own_head(src.m_own_head), m_obj(src.m_obj), m_sub_op(src.m_sub_op)
{
...
cal_object_list cal_object_list::add_set_object(const std::string &name, uint32_t set_op,
                                                uint16_t sid, uint32_t sub_op)
{
...
cal_object_list::iterator cal_object_list::find_object(const std::string &name)
{
    return find_if(begin(), end(),
                   [&name](const cal_object_list &obj) { return obj.is_name(name); });
}
...
std::string cal_object_list::get_name() { return std::string(m_obj->sObjName); }

uint32_t cal_object_list::get_sub_op() { return m_sub_op; }

uint32_t cal_object_list::get_obj_op() { return m_obj->unObjOper; }

uint16_t cal_object_list::get_sid() { return m_obj->unSid; }

ObjList *cal_object_list::get() { return m_obj; }

cal_object_list::operator ObjList *() { return m_obj; }
