...
void Broker::Bind(BrokerSocket socket_type, const std::string& endpoint)
{
	...
	case BrokerSocket::FRONTEND:
		if (config_.security.enable) {
			rc = zmq_setsockopt(frontend_->sd_, ZMQ_CURVE_SERVERKEY, config_.security.pub_public.c_str(), config_.security.key_len);
			errno_assert(rc == 0);
			rc = zmq_setsockopt(frontend_->sd_, ZMQ_CURVE_PUBLICKEY, config_.security.xsub_public.c_str(), config_.security.key_len);
			errno_assert(rc == 0);
			rc = zmq_setsockopt(frontend_->sd_, ZMQ_CURVE_SECRETKEY, config_.security.xsub_secret.c_str(), config_.security.key_len);
			errno_assert(rc == 0);
		}
		if (config_.verbose)
	...
	case BrokerSocket::BACKEND:
		if (config_.security.enable) {
			...
			errno_assert(rc == 0);
			rc = zmq_setsockopt(backend_->sd_, ZMQ_CURVE_SECRETKEY, config_.security.xpub_secret.c_str(), config_.security.key_len);
			errno_assert(rc == 0);
		}
		if (config_.verbose)
