...
public:
	...
	virtual std::unique_ptr<Message> Create(const std::string &topic, std::initializer_list<Message::Frame> frames) const = 0;
	virtual const std::string& topic_prefix() const = 0;
	virtual ~MessageMakerBase() {}
};

template<typename T>
class MessageMaker : public MessageMakerBase
{
public:
	MessageMaker(const std::string& topic_prefix)
	...
	Create(const std::string& topic) const
	{
		return std::unique_ptr<T>{new T(topic)};
	}

	virtual std::unique_ptr<Message>
	Create(const std::string& topic, std::initializer_list<Message::Frame> frames) const
	{
