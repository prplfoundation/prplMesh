...
public:
	...
	BrokerSecurity(bool enable, unsigned int key_len, std::string xsub_secret,
				   std::string xsub_public, std::string pub_secret,
				   std::string pub_public, std::string xpub_secret,
				   std::string xpub_public, std::string sub_secret,
				   std::string sub_public)
		: enable(enable), key_len(key_len)
		, xsub_secret(xsub_secret), xsub_public(xsub_public)
		, pub_secret(pub_secret), pub_public(pub_public)
		, xpub_secret(xpub_secret), xpub_public(xpub_public)
		, sub_secret(sub_secret), sub_public(sub_public) {}
