...
SHARE_EASYLOGGINGPP(el::Helpers::storage()) //use easylogging resources of the main process (assuming this process use INITIALIZE_EASYLOGGINGPP)
...
static void* watch_log_file(void* args)
{
	...
	ret = inotify_add_watch(fd, std::string(CONF_FILE_PATH).c_str(), IN_MODIFY | IN_CLOSE_WRITE | IN_IGNORED | IN_DELETE_SELF);
	if (ret < 0)
	{
		return nullptr;
	}
		...
		else
		{
			while (i<length)
			{
				...
				if (fileEvent->mask)
				{
					...
					{
						inotify_add_watch(fd, std::string(CONF_FILE_PATH).c_str(), IN_MODIFY | IN_CLOSE_WRITE | IN_IGNORED | IN_DELETE_SELF);
					}
					sleep(1);
...
{
	LoggerInit(DEFAULT_LOGGER_NAME);
}
...
void Logger::LoggerInit(const char *logger_name)
{
	...
	if (!init_performed)
	{
		...
		syslogConf.parseFromText(std::string(std::string("*GLOBAL:\n FORMAT = ")+std::string(kSyslogMessageFormat)).c_str());
		el::Loggers::reconfigureLogger("syslog", syslogConf);
	#endif //#ifdef ELPP_SYSLOG
	}
}

void Logger::LoggerConfig()
{
	LoggerConfig(DEFAULT_LOGGER_NAME);
}
