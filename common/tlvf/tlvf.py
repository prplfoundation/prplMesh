        ...
        for key, value in dict.items():
            if key.startswith(MetaData.META_PREFIX):
                if key == MetaData.KEY_TYPE:
                    ...
                    if value.find("32") != -1:
                        self.abort("bit field of more than 8 bit is not supported")
                    elif value.find("16") != -1:
                        self.abort("bit field of more than 8 bit is not supported")
                    elif value.find("8") != -1:
                        self.bit_field_pos = 7
                    else:
...
class TlvF:
    def __init__(self, src_path, yaml_path, out_path, conf_path, print_dependencies, print_outputs):
        ...
        if print_dependencies or print_outputs:
            logConsoleDisable = True
        self.logger = None
        self.yaml_file_list = []
    ...
    def generateCode(self):
        ...
        for filename in self.yaml_file_list:
            ...
            for (fname, obj_name), dict_value in self.db.items():
                if fname != self.yaml_fname or obj_name.startswith(MetaData.META_PREFIX):
                    continue
            ...
            for db_item_idx in range(db_items_list_len):
                ...
                if obj_meta.error:
                    self.abort(obj_meta.error)
                if obj_meta.type == None:
                    self.abort("%s.yaml --> '_type' not defined" % (self.yaml_fname))
                ...
                if self.root_obj_meta == None:
                    self.root_obj_meta = obj_meta
    ...
    def processDeceleration(self, obj_name, dict_value):
        if obj_name == MetaData.DECELERATION_NAMESPACE:
            ...
            for inc_value in inc_list:
                self.declared_include_list.append('"' + inc_value + '"')
        ...
        for param_name, param_dict in list(dict_value.items()):
            ...
            if (type(param_dict) is OrderedDict):
                ...
                if param_type_real.type == TypeInfo.ENUM:
                    try:
                        value = self.db_enum_storage_type[(
                            self.yaml_fname, param_type_real.type_str)]
                    except:
                        value = self.db_enum_storage_type[(
            ...
            if param_type != None:
                if param_type_info.type == TypeInfo.ERROR:
                    self.abort("%s.yaml --> bad type: %s" % (self.yaml_fname, param_type))
                    ...
                    if (param_type_info.type == TypeInfo.ENUM or
                        ...
                        if (self.db_yaml_paths.__contains__(param_type)):
                            self.include_list.append(
                                '"' + self.db_yaml_paths[param_type] + "/" + param_type + ".h" + '"')
                        else:
                            self.include_list.append(
                                '"' + self.yaml_path + "/" + param_type + ".h" + '"')

            if obj_meta.type == MetaData.TYPE_ENUM or obj_meta.type == MetaData.TYPE_ENUM_CLASS:
                if param_meta:
                    self.abort("%s.yaml --> metadata not supported on enum values" %
                               self.yaml_fname)
                try:
                    self.insertLineH(obj_meta.name, self.CODE_ENUM_INSERT,
                                     "%s = %s," % (param_name, hex(param_type)))
                except:
                    self.abort("%s.yaml --> bad param_type=%s" % (self.yaml_fname, str(param_type)))
                ...
                if obj_meta.bit_field:
                    self.abort("%s.yaml --> _bit_field not allowed in class " % self.yaml_fname)
                self.addClassParam(obj_meta, param_name, param_type, param_type_info, param_meta)
            else:
                self.abort("%s.yaml --> unknown obj_meta.type:%s" %
                           (self.yaml_fname, obj_meta.type))

            if obj_meta.type == MetaData.TYPE_ENUM_CLASS:
                self.insertLineH(obj_meta.name, self.CODE_ENUM_VALIDATION_INSERT,
                                 "case %s:" % (hex(param_type)))
    ...
    def addStructParam(self, obj_meta, param_name, param_type, param_type_info, param_meta):
        ...
        if param_meta == None:
            ...
            if param_type_info.swap_needed:
                t_name = ("&" if not param_type_info.swap_is_func else "") + \
                    param_name + ("." if param_type_info.swap_is_func else "")
                swap_func_lines.append("%s%s%s;" % (
                    param_type_info.swap_prefix, t_name, param_type_info.swap_suffix))
            if TypeInfo(param_type).type == TypeInfo.STRUCT:
                self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT,
                                 "%s.%s;" % (param_name, TypeInfo.STRUCT_INIT_FUNCTION_NAME))
        else:
            if param_meta.value_const != None:
                self.abort("%s.yaml --> value_const not supported in struct" % self.yaml_fname)
            if obj_meta.bit_field:
                if len(param_meta.bit_range) != 2:
                    self.abort("%s.yaml --> bit_range error for: %s" %
                               (self.yaml_fname, param_name))
                ...
                if param_meta.value != None:
                    self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT, "%s = %s;" % (
                        param_name, MetaData.getFormattedValue(param_meta.value)))
                line = "%s %s : %d;" % (bit_field_type, param_name, bits)
            else:
                bit_field_type = None
                if param_meta.type == None:
                    self.abort("%s.yaml --> _type not defined, param_name=%s" %
                               (self.yaml_fname, param_name))
                if (param_meta.length_type == MetaData.LENGTH_TYPE_INT or
                    ...
                    if TypeInfo(param_meta.type).type == TypeInfo.STRUCT:
                        ...
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT,
                                         "%sfor (size_t i = 0; i < %s; i++) {" % (self.getIndentation(1), str(param_meta.length)))
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT,
                                         "%s%s;" % (self.getIndentation(2), t_name))
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT,
                    ...
                    if param_meta.value != None:
                        ...
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT,
                                         "%sfor (size_t i = 0; i < %s; i++) {" % (self.getIndentation(1), str(param_meta.length)))
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT, "%s%s = %s;" % (
                            self.getIndentation(2), t_name, MetaData.getFormattedValue(param_meta.value)))
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT,
                                         "%s}" % (self.getIndentation(1)))

                    if param_type_info.swap_needed:
                        t_name = "%s(%s[i])%s" % (("&" if not param_type_info.swap_is_func else ""),
                                                  param_name, ("." if param_type_info.swap_is_func else ""))
                        swap_func_lines.append(
                            "for (size_t i = 0; i < %s; i++){" % (str(param_meta.length)))
                        swap_func_lines.append("%s%s%s%s;" % (self.getIndentation(
                            1), param_type_info.swap_prefix, t_name, param_type_info.swap_suffix))
                        swap_func_lines.append("}")

                elif (param_meta.length_type == MetaData.LENGTH_TYPE_DYNAMIC or
                      param_meta.length_type == MetaData.LENGTH_TYPE_VAR):
                    line = "%s* %s; //TLVF_TODO: not supported yet" % (
                        param_meta.type, param_meta.name)
                else:
                    ...
                    if param_meta.value != None:
                        self.insertLineH(obj_meta.name, self.CODE_STRUCT_INIT_FUNC_INSERT, "%s = %s;" % (
                            param_name, MetaData.getFormattedValue(param_meta.value)))
                self.insertLineH(obj_meta.name, self.CODE_STRUCT_INSERT,
                                 self.getCommentLines(param_meta.comment))
        ...
        if param_meta != None and param_meta.value != None:
            ...
            if param_meta.length:
                if (param_meta.length_type == MetaData.LENGTH_TYPE_INT or
                    ...
                    lines_h.append("%s%s[i] = %s;" % (self.getIndentation(
                        1), param_name, MetaData.getFormattedValue(param_meta.value)))
                    lines_h.append("}")
                else:
                    self.abort(
                        "%s.yaml --> seeting a value to a dynamic length array is not supported." % self.yaml_fname)
            else:
                lines_h.append("%s = %s;" %
                               (param_name, MetaData.getFormattedValue(param_meta.value)))

        if len(swap_func_lines) > 0:
            self.insertLineH(obj_meta.name, self.CODE_STRUCT_SWAP_FUNC_INSERT, swap_func_lines)
        if bit_field_type:
    ...
    def overrideIsPostInitSucceeded(self, obj_meta, param_name, lines_h):
        ...
        if not obj_meta.marker_post_init_method_added:
            ...
            lines_cpp.append("%sTLVF_LOG(ERROR) << \"%s is not initialized\";" %
    ...
    def addClassParam(self, obj_meta, param_name, param_type, param_type_info, param_meta=None):
        ...
        if param_meta:
            if param_meta.type == None:
                self.abort("%s.yaml --> _type not defined, param_name=%s" %
                           (self.yaml_fname, param_name))
            if param_meta.type == MetaData.TYPE_STRUCT:
                self.abort("%s.yaml --> struct definition not suported inside class, param_name=%s" %
                           (self.yaml_fname, param_name))
            if param_meta.value != None and param_meta.value_const != None:
                self.abort("%s.yaml --> _value and _value_const are not supported together, param_name=%s" %
                           (self.yaml_fname, param_name))
            if param_meta.value_const != None:
                [param_val_const, err] = MetaData.getConstValue(
                    obj_meta.name, param_meta.value_const)
                if err:
                    self.abort("%s.yaml --> error in val_const=%s" %
                               (self.yaml_fname, param_meta.value_const))
            if param_meta.value != None:
                param_val = MetaData.getFormattedValue(param_meta.value)
            if param_meta.comment:
                param_comment_line = self.getCommentLines(param_meta.comment)
            ...
            if self.class_last_param_has_dynamic_length and param_has_dynamic_length:
                self.abort("%s.yaml --> only last parameter in class can have dynamic length, param_name=%s" %
                           (self.yaml_fname, param_name))
        ...
        for (marker, func) in obj_meta.alloc_list:
            if func == "add":
                ...
                length_str = "len"
            line = "%sm_%s = (%s *)((uint8_t *)(m_%s) + %s);" % (self.getIndentation(1),
                                                                 param_name, param_type, param_name, length_str)
        ...
        if param_length_type == None:
            if TypeInfo(param_type).type == TypeInfo.CLASS:
                ...
                swap_func_lines = [
                    "if (m_%s_ptr) { m_%s_ptr->class_swap(); }" % (param_name, param_name)]

                # Add allocation methods
                self.addClassVarLenMethods(obj_meta, param_type, param_name,
                                           param_meta, param_length, False, False)

                # Add function to return pointer
                lines_h.append("std::shared_ptr<%s> %s() { return m_%s_ptr; }" % (
                ...
                lines_cpp.append("%sreturn false;" % (self.getIndentation(2)))
                lines_cpp.append("%s}" % (self.getIndentation(1)))
                lines_cpp.append("%sif (!add_%s(%s)) {" % (
                    self.getIndentation(1), param_name, param_name))
                lines_cpp.append("%sTLVF_LOG(ERROR) << \"add_%s() failed\";" %
                                 (self.getIndentation(2), param_name))
                lines_cpp.append("%sreturn false;" % (self.getIndentation(2)))
                lines_cpp.append("%s}" % (self.getIndentation(1)))
                lines_cpp.append("%s// swap back since %s will be swapped as part of the whole class swap" %
                ...
                if param_class_const:
                    ...
                    if ret_err:
                        self.abort("%s.yaml --> can't get const val for param_name=%s" %
                                   (self.yaml_fname, param_name))
                    lines_h.append("static %s get_%s(){" % (param_type, param_name))
                    lines_h.append("%sreturn (%s)(%s);" %
                                   (self.getIndentation(1), param_type, ret_const_val))
                    lines_h.append("}")
                else:
                    ...
                    if obj_meta.is_tlv_class and param_name == MetaData.TLV_TYPE_LENGTH:
                        lines_cpp.append("if (!m_%s__) *m_%s = 0;" %
                        ...
                        lines_cpp.append("if (!m_%s__) *m_%s = %s;" %
                                         (self.MEMBER_PARSE, param_name, param_val_const))
                    elif param_val != None:
                        lines_cpp.append("if (!m_%s__) *m_%s = %s;" %
                                         (self.MEMBER_PARSE, param_name, param_val))
                    elif param_length_var:
                        lines_cpp.append("if (!m_%s__) *m_%s = 0;" %
                                         (self.MEMBER_PARSE, param_name))
                    ...
                    if obj_meta.is_tlv_class and param_name != MetaData.TLV_TYPE_TYPE and param_name != MetaData.TLV_TYPE_LENGTH:
                        lines_cpp.append(
                            "if(m_length && !m_%s__){ (*m_length) += sizeof(%s); }" % (self.MEMBER_PARSE, param_type))
                    ...
                    if obj_meta.is_tlv_class and param_name == MetaData.TLV_TYPE_TYPE and param_val_const:
                        lines_cpp.append("if (m_%s__) {" % (self.MEMBER_PARSE))
                        lines_cpp.append("%sif (*m_type != %s) {" %
                                         (self.getIndentation(1), param_val_const))
                        lines_cpp.append('%sTLVF_LOG(ERROR) << "TLV type mismatch. Expected value: " << int(%s) << ", received value: " << int(*m_type);' % (
                            self.getIndentation(2), param_val_const))
                        lines_cpp.append("%sreturn false;" % self.getIndentation(2))
                        lines_cpp.append("%s}" % self.getIndentation(1))
                        lines_cpp.append("}")
                        self.insertLineCpp(
                            obj_meta.name, self.CODE_CLASS_INIT_FUNC_SWAP_INSERT, lines_cpp)
                    ...
                    if TypeInfo(param_type).type == TypeInfo.CLASS:
                        lines_cpp.append("class_size += %s::get_initial_size(); // %s" %
                                         (param_type, param_name))
                    else:
                        lines_cpp.append("class_size += sizeof(%s); // %s" %
                                         (param_type, param_name))
            ...
            if (is_var_len or is_dynamic_len) and TypeInfo(param_type).type == TypeInfo.CLASS:
                ...
                var_lines.append("std::vector<std::shared_ptr<%s>> m_%s_vector;" %
                                 (param_type, param_name))
                if not obj_meta.lock_allocation_member_added:
            ...
            if is_var_len:
                ...
                lines_cpp.append("%s %s = *m_%s;" %
                                 (length_type.type_str, param_length, param_length))
                if length_type.swap_needed:
                    lines_cpp.append("if (m_%s__) {  %s&%s%s; }" % (
                        self.MEMBER_PARSE, length_type.swap_prefix, param_length, length_type.swap_suffix))
                if TypeInfo(param_type).type == TypeInfo.CLASS:
                    ...
                    lines_cpp.append("%s}" % (self.getIndentation(1)))
                    lines_cpp.append(
                        "%s// swap back since %s will be swapped as part of the whole class swap" % (self.getIndentation(1), param_name))
                    lines_cpp.append("%s%s->class_swap();" % (self.getIndentation(1), param_name))
                    lines_cpp.append("}")
                else:
            ...
            if is_int_len or is_const_len:
                ...
                if obj_meta.is_tlv_class:
                    lines_cpp.append("%sif (m_length) { (*m_length) += (sizeof(%s) * %s); }" %
                                     (self.getIndentation(1), param_type, param_length))
                if TypeInfo(param_type).type == TypeInfo.STRUCT:
                    lines_cpp.append("%sfor (size_t i = 0; i < %s; i++) { m_%s->struct_init(); }" % (
                        self.getIndentation(1), param_length, param_name))
                elif (param_val_const or param_val):
                    if not param_type_info.is_std_type:
                        self.abort(
                            "%s.yaml --> only std types are allowed in val / val_const" % self.yaml_fname)
                    lines_cpp += ["%sfor (size_t i = 0; i < %s; i++){" % (self.getIndentation(1), str(param_length)),
                                  "%sm_%s[i] = %s;" % (self.getIndentation(2), param_name, str(
                                      param_val) if param_val != None else str(param_val_const)),
                                  "%s}" % (self.getIndentation(1))]
                if obj_meta.is_tlv_class or TypeInfo(param_type).type == TypeInfo.STRUCT:
                    lines_cpp.append("}")
            ...
            if (param_val_const or param_val):
                if not (is_int_len or is_const_len):
                    self.abort(
                        "%s.yaml --> val / val_const are not supported for variable length types" % self.yaml_fname)
            ...
            if param_type_info.swap_needed:
                if (param_type_info.type == TypeInfo.CLASS):
                    t_name = ("&" if not param_type_info.swap_is_func else "") + ("std::get<1>(%s(i))" %
                                                                                  param_name) + ("." if param_type_info.swap_is_func else "")
                else:
                    t_name = ("&" if not param_type_info.swap_is_func else "") + \
                        ("m_%s[i]" % param_name) + ("." if param_type_info.swap_is_func else "")
            ...
            if is_int_len or is_const_len:
                lines_cpp.append("class_size += %s * sizeof(%s); // %s" %
                                 (param_length, param_type, param_name))
                self.insertLineCpp(obj_meta.name, self.CODE_CLASS_SIZE_FUNC_INSERT, lines_cpp)
            ...
            if is_dynamic_len:
                if param_type_info.type == TypeInfo.CLASS:
                    ...
                    lines_cpp.append(
                        "%sfor (size_t i = 0; i < m_%s_idx__; i++) {" % (self.getIndentation(1), param_name))
                    lines_cpp.append("%slen += m_%s_vector[i]->getLen();" %
                                     (self.getIndentation(2), param_name))
                    lines_cpp.append("%s}" % (self.getIndentation(1)))
                    ...
                    lines_h.append(
                        "size_t %s_length() { return m_%s_idx__ * sizeof(%s); }" % (param_name, param_name, param_type_full))
            ...
            if param_type_info.type == TypeInfo.CHAR:
                ...
                lines_cpp.append("%s* %s::%s(size_t length) {" %
                                 (param_type_full, obj_meta.name, param_name))
                lines_cpp.append("%sif( (m_%s_idx__ == 0) || (m_%s_idx__ < length) ) {" % (
                    self.getIndentation(1), param_name, param_name))
                lines_cpp.append('%sTLVF_LOG(ERROR) << "%s length is smaller than requested length";' % (
                    self.getIndentation(2), param_name))
                lines_cpp.append("%sreturn nullptr;" % self.getIndentation(2))
                lines_cpp.append("%s}" % self.getIndentation(1))
                if param_meta.length_max:
                    lines_cpp.append("%sif (m_%s_idx__ > %s )  {" % (
                        self.getIndentation(1), param_name, param_meta.length_max))
                    lines_cpp.append('%sTLVF_LOG(ERROR) << "Invalid length -  " << m_%s_idx__ << " elements (max length is " << %s << ")";' % (
                        self.getIndentation(2), param_name, param_meta.length_max))
                    lines_cpp.append("%sreturn nullptr;" % self.getIndentation(2))
                    lines_cpp.append("%s}" % self.getIndentation(1))
                ...
                lines_cpp.append("bool %s::set_%s(const std::string& str) { return set_%s(str.c_str(), str.size()); }" % (
                    obj_meta.name, param_name, param_name))

                lines_h.append("bool set_%s(const char buffer[], size_t size);" % (param_name))
                lines_cpp.append("bool %s::set_%s(const char str[], size_t size) {" % (
                    obj_meta.name, param_name))
                lines_cpp.append("%sif (str == nullptr) {" % self.getIndentation(1))
                lines_cpp.append('%sTLVF_LOG(WARNING) << "set_%s received a null pointer.";' % (
                    self.getIndentation(2), param_name))
                lines_cpp.append("%sreturn false;" % self.getIndentation(2))
                lines_cpp.append("%s}" % self.getIndentation(1))
                if is_const_len or is_int_len:
                    ...
                    lines_cpp.append(
                        '%sTLVF_LOG(ERROR) << "Received buffer size is smaller than string length";' % self.getIndentation(2))
                    lines_cpp.append("%sreturn false;" % self.getIndentation(2))
                    lines_cpp.append("%s}" % self.getIndentation(1))
                else:
                ...
                lines_cpp.append("%s* %s::%s(size_t idx) {" %
                                 (param_type_full, obj_meta.name, param_name))
                lines_cpp.append("%sif ( (m_%s_idx__ == 0) || (m_%s_idx__ <= idx) ) {" % (
                    self.getIndentation(1), param_name, param_name))
                lines_cpp.append(
                    '%sTLVF_LOG(ERROR) << "Requested index is greater than the number of available entries";' % self.getIndentation(2))
                lines_cpp.append('%sreturn nullptr;' % self.getIndentation(2))
                lines_cpp.append("%s}" % self.getIndentation(1))
                ...
                if param_type_info.type == TypeInfo.UINT8:
                    ...
                    lines_cpp.append(
                        "bool %s::set_%s(const void* buffer, size_t size) {" % (obj_meta.name, param_name))
                    ...
                    lines_cpp.append("%s}" % self.getIndentation(1))
                    if is_const_len or is_int_len:
                        lines_cpp.append("%sif (size > %s) {" %
                                         (self.getIndentation(1), param_length))
                        lines_cpp.append(
                            '%sTLVF_LOG(ERROR) << "Received buffer size is smaller than buffer length";' % self.getIndentation(2))
                        lines_cpp.append("%sreturn false;" % self.getIndentation(2))
                        lines_cpp.append("%s}" % self.getIndentation(1))
                    else:
                        lines_cpp.append("%sif (!alloc_%s(size)) { return false; }" % (
                            self.getIndentation(1), param_name))
                    lines_cpp.append(
                        "%sstd::copy_n(reinterpret_cast<const uint8_t *>(buffer), size, m_%s);" % (self.getIndentation(1), param_name))
                    lines_cpp.append("%sreturn true;" % self.getIndentation(1))
                    lines_cpp.append("}")
                ...
                lines_cpp.append("std::tuple<bool, %s&> %s::%s(size_t idx) {" % (
                    param_type_full, obj_meta.name, param_name))
                lines_cpp.append("%sbool ret_success = ( (m_%s_idx__ > 0) && (m_%s_idx__ > idx) );" % (
                    self.getIndentation(1), param_name, param_name))
                lines_cpp.append("%ssize_t ret_idx = ret_success ? idx : 0;" %
                                 (self.getIndentation(1)))
                ...
                if TypeInfo(param_type).type == TypeInfo.CLASS:  # TODO:only if it's the last member of the class
                    lines_cpp.append(
                        "%sreturn std::forward_as_tuple(ret_success, *(m_%s_vector[ret_idx]));" % (self.getIndentation(1), param_name))
                else:
                    lines_cpp.append("%sreturn std::forward_as_tuple(ret_success, m_%s[ret_idx]);" % (
                        self.getIndentation(1), param_name))
                lines_cpp.append("}")
                lines_cpp.append("")
            ...
            self.abort("%s.yaml --> unsupported length type: %r, param_name=%s" %
                       (self.yaml_fname, param_length_type, param_name))

        if len(swap_func_lines) > 0:
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_SWAP_FUNC_INSERT, swap_func_lines)
        self.insertLineH(obj_meta.name, self.CODE_CLASS_PUBLIC_FUNC_INSERT, lines_h)
        self.insertLineCpp(obj_meta.name, self.CODE_CLASS_PUBLIC_FUNC_INSERT, lines_cpp)
    ...
    def addClassVarLenMethods(self, obj_meta, param_type, param_name, param_meta, param_length, is_var_len, is_dynamic_len):
        ...
        lines_h = []
        if TypeInfo(param_type).type == TypeInfo.CLASS:
            # TODO: maybe change to 'create' and add '_entry' postfix
            lines_h.append("std::shared_ptr<%s> create_%s();" % (param_type, param_name))
            lines_cpp.append("std::shared_ptr<%s> %s::create_%s() {" % (
                param_type, obj_meta.name, param_name))
            lines_cpp.append("%sif (m_%s__ > %s) {" % (self.getIndentation(
                1), self.MEMBER_LOCK_ORDER_COUNTER, param_meta.list_index))
            lines_cpp.append("%sTLVF_LOG(ERROR) << \"Out of order allocation for variable length list %s, abort!\";" % (
                self.getIndentation(2), param_name))
            lines_cpp.append("%sreturn nullptr;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            ...
            lines_cpp.append("%sm_%s__ = %s;" % (self.getIndentation(
                1), self.MEMBER_LOCK_ORDER_COUNTER, param_meta.list_index))
            lines_cpp.append("%sm_%s__ = true;" %
                             (self.getIndentation(1), self.MEMBER_LOCK_ALLOCATION))
            lines_cpp.extend(self.addAllocationMarkersCreate(obj_meta, param_meta,
                                                             param_length, True))  # Variable length lists support
            if is_dynamic_len:
                lines_cpp.append("%sreturn std::make_shared<%s>(getBuffPtr(), getBuffRemainingBytes(), m_%s__);" % (
                    self.getIndentation(1), param_type, self.MEMBER_PARSE))
            else:
                lines_cpp.append("%sreturn std::make_shared<%s>(src, getBuffRemainingBytes(src), m_%s__);" % (
                    self.getIndentation(1), param_type, self.MEMBER_PARSE))
            lines_cpp.append("}")
            lines_cpp.append("")

            # TODO: maybe change to add and end '_entry' postfix
            lines_h.append("bool add_%s(std::shared_ptr<%s> ptr);" % (param_name, param_type))
            lines_cpp.append("bool %s::add_%s(std::shared_ptr<%s> ptr) {" % (
                obj_meta.name, param_name, param_type))

            lines_cpp.append("%sif (ptr == nullptr) {" % self.getIndentation(1))
            lines_cpp.append('%sTLVF_LOG(ERROR) << "Received entry is nullptr";' %
                             self.getIndentation(2))
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))

            lines_cpp.append("%sif (m_%s__ == false) {" % (
                self.getIndentation(1), self.MEMBER_LOCK_ALLOCATION))
            lines_cpp.append('%sTLVF_LOG(ERROR) << "No call to create_%s was called before add_%s";' % (
                self.getIndentation(2), param_name, param_name))
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            ...
            if param_length != None:
                ...
                ptr = "(uint8_t *)m_%s_vector[m_%s_idx__ - 1]->getBuffPtr()" % (
            ...
            lines_cpp.append(
                '%sTLVF_LOG(ERROR) << "Received entry pointer is different than expected (expecting the same pointer returned from add method)";' % self.getIndentation(2))
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))

            lines_cpp.append(
                "%sif (ptr->getLen() > getBuffRemainingBytes(ptr->getStartBuffPtr())) {;" % self.getIndentation(1))
            lines_cpp.append(
                '%sTLVF_LOG(ERROR) << "Not enough available space on buffer";' % self.getIndentation(2))
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            ...
            if param_length or is_dynamic_len:
                ...
                if is_var_len:
                    lines_cpp.append("%sif (!m_parse__) { (*m_%s)++; }" %
                                     (self.getIndentation(1), param_length))
            else:
            ...
            lines_cpp.extend(self.addAllocationMarkersAdd(obj_meta, param_meta,
                                                          param_length, False))  # Variable length lists support
            if param_length or is_dynamic_len:
                lines_cpp.append("%sm_%s_vector.push_back(ptr);" %
                                 (self.getIndentation(1), param_name))
            else:
            ...
            if obj_meta.is_tlv_class:
                lines_cpp.append(
                    "%sif(!m_parse__ && m_length){ (*m_length) += len; }" % (self.getIndentation(1)))
            lines_cpp.append("%sm_%s__ = false;" %
                             (self.getIndentation(1), self.MEMBER_LOCK_ALLOCATION))
            lines_cpp.append("%sreturn true;" % (self.getIndentation(1)))
            lines_cpp.append("}")
            lines_cpp.append("")
            ...
            lines_cpp.append("%sreturn false;" % self.getIndentation(2))
            lines_cpp.append("%s}" % self.getIndentation(1))
            lines_cpp.append("%ssize_t len = sizeof(%s) * count;" %
                             (self.getIndentation(1), param_type))
            ...
            if TypeInfo(param_type).type == TypeInfo.STRUCT:
                ...
                lines_cpp.append("%sfor (size_t i = m_%s_idx__ - count; i < m_%s_idx__; i++) { m_%s[i].struct_init(); }" % (
                    self.getIndentation(2), param_name, param_name, param_name))
                lines_cpp.append("%s}" % (self.getIndentation(1)))
            ...
            if memmove:
                ...
                if param_length != None:
                    lines_cpp.append("%sif (m_%s_idx__ > 0) {" % (
                        self.getIndentation(1), param_meta.name))
                    ptr = "(uint8_t *)m_%s_vector[m_%s_idx__ - 1]->getBuffPtr()" % (
                        param_meta.name, param_meta.name)
                    lines_cpp.append("%ssrc = %s;" % (self.getIndentation(2), ptr))
                    lines_cpp.append("%s}" % self.getIndentation(1))
                ...
                lines_cpp.append(
                    "%ssize_t move_length = getBuffRemainingBytes(src) - len;" % self.getIndentation(2))
                lines_cpp.append("%sstd::copy_n(src, move_length, dst);" % self.getIndentation(2))
                lines_cpp.append("%s}" % self.getIndentation(1))

            marker = "%s_%s_%s_%s" % (self.CODE_CLASS_ALLOC_INSERT,
                                      obj_meta.name, "create", param_meta.name)
    ...
    def addAllocationMarkersAlloc(self, obj_meta, param_meta, param_length, memmove):
        ...
        if param_meta.length_type == MetaData.LENGTH_TYPE_VAR or MetaData.LENGTH_TYPE_DYNAMIC:
            ...
            marker = "%s_%s_%s_%s" % (self.CODE_CLASS_ALLOC_INSERT,
                                      obj_meta.name, "alloc", param_meta.name)
    ...
    def addAllocationMarkersAdd(self, obj_meta, param_meta, param_length, memmove):
        ...
        if param_meta.length_type == MetaData.LENGTH_TYPE_VAR:
            marker = "%s_%s_%s_%s" % (self.CODE_CLASS_ALLOC_INSERT,
                                      obj_meta.name, "add", param_meta.name)
        ...
        for idx in range(0, len(template)):
            if marker == template[idx].strip():
                ...
                for l in lines:
                    ...
                    idx_insert += 1
                return
        self.abort("%s.yaml --> insertLine, can't find marker %s in code template" %
                   (self.yaml_fname, marker))
    ...
    def openFile(self, filename):
        ...
        self.appendLineH("#ifndef _%s_%s_H_" %
                         (self.yaml_path.upper().replace('/', '_'), self.yaml_fname.upper()))
        self.appendLineH("#define _%s_%s_H_" %
                         (self.yaml_path.upper().replace('/', '_'), self.yaml_fname.upper()))
        self.appendLineH("")
    ...
    def openObject(self, obj_meta, dict_value, root_obj_meta):
        ...
        if self.is_root_obj:
            ...
            if obj_meta.type == MetaData.TYPE_CLASS and obj_meta.name == root_obj_meta.name:
                ...
                self.insertLineH(
                    root_obj_meta.name, self.CODE_CLASS_PUBLIC_VARS_END_INSERT, insert_marker + "_" + insert_name)
            elif obj_meta.type == MetaData.TYPE_CLASS:
                self.insertLineH("", self.CODE_FORWARD_DECLARATION_INSERT,
                                 "class %s;" % obj_meta.name)
                insert_name = ""
                insert_marker = self.CODE_END_INSERT
            else:
                insert_marker = self.CODE_CLASS_PUBLIC_VARS_INSERT + "_" + root_obj_meta.name
                self.insertLineH(root_obj_meta.name, self.CODE_CLASS_PUBLIC_VARS_INSERT,
                                 insert_marker + "_" + insert_name)
        else:
            insert_name = ""
            insert_marker = self.CODE_END_INSERT
        ...
        if obj_meta.type == MetaData.TYPE_CLASS:
            ...
            if self.multi_class_auto_insert != None:
                for param_name, param_dict in self.multi_class_auto_insert.items():
                    ...
                    if param_meta.error:
                        self.abort(param_meta.error)
                    self.addClassParam(obj_meta, param_name, param_meta.type,
                                       param_meta.type_info, param_meta)

        elif obj_meta.type == MetaData.TYPE_STRUCT:
            self.addStructCode(insert_name, insert_marker, name)
            if obj_meta.bit_field:
                self.include_list.append("<asm/byteorder.h>")
                self.insertLineH(insert_name, self.CODE_STRUCT_INSERT,
                                 "#if defined(__LITTLE_ENDIAN_BITFIELD)")
                self.insertLineH(insert_name, self.CODE_STRUCT_BITFIELD_INSERT,
                                 "#elif defined(__BIG_ENDIAN_BITFIELD)")
                self.insertLineH(insert_name, self.CODE_STRUCT_REVERSED_BITFIELD_INSERT, "#else")
                self.insertLineH(insert_name, self.CODE_STRUCT_REVERSED_BITFIELD_INSERT,
    ...
    def addClassCode(self, insert_name, insert_marker, name, obj_meta):
        ...
        self.insertLineH(insert_name, insert_marker, "%sclass %s : public BaseClass" %
                         (self.getIndentation(0), name))
        self.insertLineH(insert_name, insert_marker, "{")
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(1), self.CODE_CLASS_START, name))
        ...
        # class end
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(1), self.CODE_CLASS_END, name))
        self.insertLineH(insert_name, insert_marker, "};")

    def addStructCode(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker, "typedef struct %s {" % (name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(1), self.CODE_STRUCT_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(1), self.CODE_STRUCT_BITFIELD_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(1), self.CODE_STRUCT_REVERSED_BITFIELD_INSERT, name))

        self.insertLineH(insert_name, insert_marker, "%svoid %s{" % (
            self.getIndentation(1), TypeInfo.STRUCT_SWAP_FUNCTION_NAME))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_STRUCT_SWAP_FUNC_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s}" % (self.getIndentation(1)))

        self.insertLineH(insert_name, insert_marker, "%svoid %s{" % (
            self.getIndentation(1), TypeInfo.STRUCT_INIT_FUNCTION_NAME))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_STRUCT_INIT_FUNC_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s}" % (self.getIndentation(1)))
    ...
    def addEnumCode(self, insert_name, insert_marker, name, enum_storage):
        ...
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(1), self.CODE_ENUM_INSERT, name))
    ...
    def addEnumClassCode(self, insert_name, insert_marker, name, enum_storage):
        ...
        self.insertLineH(insert_name, insert_marker, "%s};" % self.getIndentation(0))
        self.insertLineH(insert_name, insert_marker,
                         "%sclass %sValidate {" % (self.getIndentation(0), name))
        self.insertLineH(insert_name, insert_marker, "%spublic:" % (self.getIndentation(0)))
        self.insertLineH(insert_name, insert_marker, "%sstatic bool check(%s value) {" % (
            self.getIndentation(1), enum_storage))
        self.insertLineH(insert_name, insert_marker, "%sbool ret = false;" %
                         (self.getIndentation(2)))
        self.insertLineH(insert_name, insert_marker,
                         "%sswitch (value) {" % (self.getIndentation(2)))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_ENUM_VALIDATION_INSERT, name))
    ...
    def addClassConstructor(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_CONSTRACTOR, name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_DISTRACTOR, name))
        self.insertLineH(insert_name, insert_marker, "")

        self.insertLineCpp(insert_name, insert_marker, "%s_%s" %
                           (self.CODE_CLASS_CONSTRACTOR, name))
    ...
    def addClassPublicMembers(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_PUBLIC_VARS_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_PUBLIC_VARS_END_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_PUBLIC_FUNC_INSERT, name))

        self.insertLineCpp(insert_name, insert_marker, "%s_%s" %
                           (self.CODE_CLASS_PUBLIC_FUNC_INSERT, name))

    def addClassPrivateMembers(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_PRIVATE_FUNC_INSERT, name))
        self.insertLineH(insert_name, insert_marker, "%s%s_%s" %
                         (self.getIndentation(2), self.CODE_CLASS_PRIVATE_VARS_INSERT, name))

        self.insertLineCpp(insert_name, insert_marker, "%s_%s" %
                           (self.CODE_CLASS_PRIVATE_FUNC_INSERT, name))
    ...
    def addClassInitMethod(self, insert_name, insert_marker, name):
        ...
        self.insertLineCpp(insert_name, insert_marker,
                           "%sif (getBuffRemainingBytes() < get_initial_size()) {" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker,
                           '%sTLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";' % self.getIndentation(2))
        ...
        self.insertLineCpp(insert_name, insert_marker, "%s%s_%s" %
                           (self.getIndentation(1), self.CODE_CLASS_INIT_FUNC_INSERT, name))
        self.insertLineCpp(insert_name, insert_marker, "%sif (m_%s__) { class_swap(); }" % (
            self.getIndentation(1), self.MEMBER_PARSE))
        self.insertLineCpp(insert_name, insert_marker, "%s%s_%s" %
                           (self.getIndentation(1), self.CODE_CLASS_INIT_FUNC_SWAP_INSERT, name))
    ...
    def addClassFinalizeMethod(self, insert_name, insert_marker, name, obj_meta):
        self.insertLineH(insert_name, insert_marker, "%sbool finalize() override;" %
                         self.getIndentation(2))

        self.insertLineCpp(insert_name, insert_marker, "bool %s::finalize()" % (name))
        self.insertLineCpp(insert_name, insert_marker, "{")
        self.insertLineCpp(insert_name, insert_marker,
                           "%sif (m_parse__) {" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker,
                           '%sTLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";' % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker, "%sreturn true;" % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker, "%s}" % self.getIndentation(1))
        self.insertLineCpp(insert_name, insert_marker,
                           "%sif (m_finalized__) {" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker,
                           '%sTLVF_LOG(DEBUG) << "finalize() called for already finalized class";' % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker, "%sreturn true;" % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker, "%s}" % self.getIndentation(1))
        self.insertLineCpp(insert_name, insert_marker,
                           "%sif (!isPostInitSucceeded()) {" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker,
                           '%sTLVF_LOG(ERROR) << "post init check failed";' % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker, "%sreturn false;" % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker, "%s}" % self.getIndentation(1))
        self.insertLineCpp(insert_name, insert_marker,
                           "%sif (m_inner__) {" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker,
                           "%sif (!m_inner__->finalize()) {" % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker,
                           '%sTLVF_LOG(ERROR) << "m_inner__->finalize() failed";' % self.getIndentation(3))
        ...
        self.insertLineCpp(insert_name, insert_marker,
                           "%sauto tailroom = m_inner__->getMessageBuffLength() - m_inner__->getMessageLength();" % self.getIndentation(2))
        self.insertLineCpp(insert_name, insert_marker,
                           "%sm_buff_ptr__ -= tailroom;" % (self.getIndentation(2)))
        if obj_meta.is_tlv_class:
            self.insertLineCpp(insert_name, insert_marker,
                               "%s*m_length -= tailroom;" % (self.getIndentation(2)))
        self.insertLineCpp(insert_name, insert_marker, "%s}" % self.getIndentation(1))
        self.insertLineCpp(insert_name, insert_marker, "%sclass_swap();" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker, "%sm_finalized__ = true;" %
                           (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker, "%sreturn true;" % (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker, "}")
        self.insertLineCpp(insert_name, insert_marker, "")
    ...
    def addClassSwapMethod(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker,
                         "%svoid class_swap() override;" % self.getIndentation(2))

        self.insertLineCpp(insert_name, insert_marker, "void %s::class_swap()" % (name))
        self.insertLineCpp(insert_name, insert_marker, "{")
        self.insertLineCpp(insert_name, insert_marker, "%s%s_%s" %
                           (self.getIndentation(1), self.CODE_CLASS_SWAP_FUNC_INSERT, name))
        self.insertLineCpp(insert_name, insert_marker, "}")
        self.insertLineCpp(insert_name, insert_marker, "")

    def addClassSizeMethod(self, insert_name, insert_marker, name):
        self.insertLineH(insert_name, insert_marker,
                         "%sstatic size_t get_initial_size();" % self.getIndentation(2))

        self.insertLineCpp(insert_name, insert_marker, "size_t %s::get_initial_size()" % (name))
        self.insertLineCpp(insert_name, insert_marker, "{")
        self.insertLineCpp(insert_name, insert_marker, "%ssize_t class_size = 0;" %
                           (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker, "%s%s_%s" %
                           (self.getIndentation(1), self.CODE_CLASS_SIZE_FUNC_INSERT, name))
        self.insertLineCpp(insert_name, insert_marker, "%sreturn class_size;" %
                           (self.getIndentation(1)))
        self.insertLineCpp(insert_name, insert_marker, "}")
        self.insertLineCpp(insert_name, insert_marker, "")
    ...
    def closeObject(self, obj_meta):
        if obj_meta.type == MetaData.TYPE_CLASS:  # add class constractor
            # constractor 1
            self.insertLineH(obj_meta.name, self.CODE_CLASS_CONSTRACTOR,
                             "%s(uint8_t* buff, size_t buff_len, bool parse = false);" % (obj_meta.name))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_CONSTRACTOR, (
                "%s::%s(uint8_t* buff, size_t buff_len, bool parse) :" % (obj_meta.name, obj_meta.name)))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_CONSTRACTOR,
                               ("%sBaseClass(buff, buff_len, parse) {" % self.getIndentation(1)))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_CONSTRACTOR,
                               "%sm_init_succeeded = init();" % self.getIndentation(1))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_CONSTRACTOR, "}")

            # constractor 2
            self.insertLineH(obj_meta.name, self.CODE_CLASS_CONSTRACTOR,
                             "explicit %s(std::shared_ptr<BaseClass> base, bool parse = false);" % (obj_meta.name))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_CONSTRACTOR, (
                "%s::%s(std::shared_ptr<BaseClass> base, bool parse) :" % (obj_meta.name, obj_meta.name)))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_CONSTRACTOR,
            ...
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_DISTRACTOR,
                               "%s::~%s() {" % (obj_meta.name, obj_meta.name))
            self.insertLineCpp(obj_meta.name, self.CODE_CLASS_DISTRACTOR, "}")
    ...
    def generateCmake(self):
        ...
        code += [
            ...
            '',
            '# set_target_properties(${PROJET_NAME} PROPERTIES VERSION "%s" SOVERSION "%s")' % (
                self.CMAKE_PROPERTIES_VERSION, self.CMAKE_SO_VERSION),
    ...
    def writeFile(self, code_lines, file_suffix):
        ...
        if self.conf_source_license_header:
            ...
            try:
                fl = open(license_file_path, 'r')
            except:
                self.abort("can't open file %s" % license_file_path)
            for line in fl:
    ...
    def initLogger(self):
        if self.conf_log_level == "DEBUG":
            self.conf_log_level = logging.DEBUG
        if self.conf_log_level == "WARNING":
            self.conf_log_level = logging.WARNING
        if self.conf_log_level == "ERROR":
            self.conf_log_level = logging.ERROR
        if self.conf_log_level == "OFF":
            return

        file_path = os.path.join(self.conf_output_path, self.conf_log_file)
        self.mkdir_p(os.path.dirname(file_path))

        # datefmt=log_date_format --> log_date_format: '%d/%m/%Y %I:%M:%S %p'
        self.conf_log_format = logging.Formatter(fmt=self.conf_log_format, datefmt="")
    ...
    def loadConf(self):
        ...
        if yaml_conf == None:
            self.abort("can't read conf file")

        try:
            self.conf_include_yaml_path = yaml_conf["include_yaml_path"]
        except:
            self.conf_include_yaml_path = None

        try:
            self.conf_source_license_header = yaml_conf["source_license_header"]
        except:
            self.conf_source_license_header = None

        try:
            self.conf_log_file = yaml_conf["debug"]["log_file"]
        except:
            self.conf_log_file = ""
        try:
            self.conf_log_format = yaml_conf["debug"]["log_format"]
        except:
            self.conf_log_format = ""
        try:
            self.conf_log_level = yaml_conf["debug"]["log_level"]
        except:
            self.conf_log_level = ""
        try:
            self.conf_debug_dump_db = yaml_conf["debug"]["dump_db"]
        except:
            self.conf_debug_dump_db = False
        try:
            self.conf_debug_dump_include_files = yaml_conf["debug"]["dump_include_files"]
        except:
            self.conf_debug_dump_include_files = False
        try:
            self.conf_debug_dump_yaml = yaml_conf["debug"]["dump_yaml"]
        except:
            self.conf_debug_dump_yaml = False
        try:
            self.conf_debug_dump_stack = yaml_conf["debug"]["dump_stack"]
        except:
            self.conf_debug_dump_stack = False
        try:
            self.conf_debug_keep_source_marker = yaml_conf["debug"]["keep_source_marker"]
        except:
            self.conf_debug_keep_source_marker = False
        try:
            self.conf_output_path_include = os.path.abspath(os.environ["TLVF_PATH_INCLUDE"])
        except:
            logConsole("env TLVF_PATH_INCLUDE not set, using {}\n".format(
                self.conf_output_path + '/include'))
            self.conf_output_path_include = self.conf_output_path + '/include'
        try:
            self.conf_output_path_src = os.path.abspath(os.environ["TLVF_PATH_SRC"])
        except:
            logConsole("env TLVF_PATH_SRC not set, using {}\n".format(
                self.conf_output_path + '/src'))
...
def main():
    ...
    parser.add_argument('-c', '--conf', default=os.path.dirname(os.path.realpath(__file__)
                                                                ) + '/tlvf_conf.yaml', help="path to yaml config file")
    ...
    if args.test:
        ...
        tlvf = TlvF(args.src_path, args.yaml_path, args.out_path,
                    args.conf, args.print_dependencies, args.print_outputs)
