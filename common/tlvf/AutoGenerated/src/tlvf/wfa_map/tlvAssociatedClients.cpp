...
bool tlvAssociatedClients::init()
{
    ...
    m_bss_list_length = reinterpret_cast<uint8_t*>(m_buff_ptr__);
...
bool cBssInfo::init()
{
    ...
    m_clients_associated_list_length = reinterpret_cast<uint16_t*>(m_buff_ptr__);
...
bool cClientInfo::init()
{
    ...
    m_time_since_last_association_sec = reinterpret_cast<uint16_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint16_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint16_t) << ") Failed!";
        return false;
    }
