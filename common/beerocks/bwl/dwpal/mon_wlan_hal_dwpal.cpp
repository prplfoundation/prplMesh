                                               ...
                                               const std::string &sta_mac, SStaStats &sta_stats)
{
    ...
    LOG(DEBUG) << "GET_STA_MEASUREMENTS cmd: " << cmd << std::endl;
    if (!dwpal_send_cmd(cmd, &reply)) {
        LOG(ERROR) << __func__ << " failed";
        return false;
    }
    ...
    FieldsToParse fieldsToParse[]                                       = {
        ...
        {(void *)&Active, &numOfValidArgs[6], DWPAL_LONG_LONG_INT_PARAM, "Active=", 0},
        {(void *)&LastDataDownlinkRate, &numOfValidArgs[7], DWPAL_LONG_LONG_INT_PARAM,
         "LastDataDownlinkRate=", 0},
        {(void *)&LastDataUplinkRate, &numOfValidArgs[8], DWPAL_LONG_LONG_INT_PARAM,
         "LastDataUplinkRate=", 0},

        /* Must be at the end */
        {NULL, NULL, DWPAL_NUM_OF_PARSING_TYPES, NULL, 0}};
       LOG(DEBUG) << "About to call dwpal parse" << std::endl;
       if (dwpal_string_to_struct_parse(reply, replyLen, fieldsToParse, (7*sizeof(uint64_t)+ sizeof(sta_stats.retrans_count) + sizeof(ShortTermRSSIAverage))) ==
        DWPAL_FAILURE) {
        LOG(ERROR) << "DWPAL parse error ==> Abort";
        return false;
    }
