...
bool KeyValueParser::read_param_int(const std::string &key, parsed_line_t &obj, int64_t &value,
                                    bool ignore_unknown)
{
...
bool KeyValueParser::read_param_str(const std::string &key, parsed_line_t &obj, const char **value)
{
    auto val_iter = obj.find(key);
    if (val_iter == obj.end()) {
        LOG(ERROR) << "param :" << key << " does not exist";
