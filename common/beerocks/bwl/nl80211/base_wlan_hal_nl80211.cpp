...
static void map_obj_parser(
                std::stringstream &ss_in, std::list<char> delimiter_list,
...
static void listed_map_obj_parser(
                std::stringstream &ss_in, std::list<char> delimiter_list,
        ...
        m_wpa_ctrl_buffer_size(wpa_ctrl_buffer_size)
{
    ...
    if (m_wpa_ctrl_buffer_size) {
        m_wpa_ctrl_buffer = std::shared_ptr<char>(
            new char[m_wpa_ctrl_buffer_size], [](char *obj) {
                if (obj) {
                    delete[] obj;
                }
            }
...
bool base_wlan_hal_nl80211::wpa_ctrl_send_msg(const std::string &cmd, parsed_obj_listed_map_t &reply)
{
    if (!wpa_ctrl_send_msg(cmd)) {
        return false;
    }
    ...
    if (get_type() == HALType::Station) {
        ...
        if (ieee80211ac == 0) {
            ...
            switch (vht_oper_chwidth) {
                case 0:  m_radio_info.bandwidth = 40; break;
                case 1:  m_radio_info.bandwidth = 80; break;
                default: m_radio_info.bandwidth = 160;
            }

            m_radio_info.vht_center_freq = beerocks::string_utils::stoi(reply["vht_oper_centr_freq_seg0_idx"]);
        }
...
{
    // Netlink Message
    std::shared_ptr<nl_msg> nl_message = std::shared_ptr<struct nl_msg>(
        nlmsg_alloc(), [](struct nl_msg* obj) {
            if (obj) { nlmsg_free(obj); } 
        });
    LOG_IF(!nl_message, FATAL) << "Failed creating netlink message!";

    // Netlink Callback
    std::shared_ptr<nl_cb> nl_callback = std::shared_ptr<struct nl_cb>(
        nl_cb_alloc(NL_CB_DEFAULT), [](struct nl_cb* obj) {
            if (obj) { nl_cb_put(obj); } 
        });
    ...
    nl_cb_set(nl_callback.get(), NL_CB_VALID , NL_CB_CUSTOM, nl_handler_cb, nullptr); // response handler
