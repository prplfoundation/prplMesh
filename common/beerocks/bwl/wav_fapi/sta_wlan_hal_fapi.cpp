...
static int add_network(const std::string &iface, char *tmpBuff)
{
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint", 0, "X_LANTIQ_COM_Vendor_NetworkId",
                       tmpBuff) != UGW_SUCCESS) {
...
static bool set_network(const std::string &iface, const std::string &network_id,
                        const std::string &ssid, const std::string &bssid, WiFiSec sec,
                        const std::string &pass, bool hidden_ssid)
{
    ...
    if (bssid.length() > 0) {
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.EndPoint.Profile", "X_LANTIQ_COM_Vendor_BSSID",
                              bssid.c_str(), 0, 0);
    }

    if (hidden_ssid) {
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.EndPoint.Profile",
                              "X_LANTIQ_COM_Vendor_scan_ssid", "1", 0, 0);
    }

    // Security and Password
    UGW_OBJLIST_ADD(wlObj, "Device.WiFi.EndPoint.Profile.Security");
    if (sec != WiFiSec::Invalid) {
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.EndPoint.Profile.Security", "ModeEnabled",
                              fapi_security_val(sec).c_str(), 0, 0);
    }
...
static bool select_network(const std::string &iface, const std::string &network_id,
                           const std::string &freq)
{
    ...
    // Frequency
    UGW_OBJLIST_ADD(wlObj, "Device.WiFi.EndPoint.Profile");
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.EndPoint.Profile", "X_LANTIQ_COM_Vendor_Frequency",
                          freq.c_str(), 0, 0);
...
static bool read_status(const std::string &iface, ConnectionStatus &connection_status,
                        char *tmpBuff)
{
    LOG(TRACE) << __func__ << " entered for " << iface;
    ...
    if (fapi_wlan_client_mode_connection_status_get(iface.c_str(), wlObj.get(), 0) == UGW_FAILURE) {
        LOG(ERROR) << "read_status - fapi_wlan_client_mode_connection_status_get failed for "
                   << iface;
        return false;
    }

    if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint", 0, "X_LANTIQ_COM_Vendor_bssid", tmpBuff) ==
        UGW_FAILURE) {
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint", 0, "X_LANTIQ_COM_Vendor_freq", tmpBuff) ==
        UGW_FAILURE) {
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint", 0, "X_LANTIQ_COM_Vendor_ssid", tmpBuff) ==
        UGW_FAILURE) {
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint", 0, "X_LANTIQ_COM_Vendor_id", tmpBuff) ==
        UGW_FAILURE) {
...
sta_wlan_hal_fapi::~sta_wlan_hal_fapi() { detach(); }
...
int sta_wlan_hal_fapi::get_scan_results(const std::string &ssid, net::sScanResult *list, int size)
{
    ...
    if (fapi_wlan_client_mode_scan_results_get(get_iface_name().c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
        LOG(ERROR) << "get_scan_results - fapi_wlan_client_mode_scan_results_get failed for "
                   << get_iface_name();
        return 0;
    }
        ...
        FOR_EACH_PARAM(curObj, curParam)
        {
            ...
            */
            ...
            if (full_param_name.find("BSSID") != std::string::npos) {
                beerocks::net::network_utils::mac_from_string(list[num_of_results].bssid,
                                                              param_value);
...
bool sta_wlan_hal_fapi::connect(const std::string &ssid, const std::string &pass, WiFiSec sec,
                                const std::string &bssid, uint8_t channel, bool hidden_ssid)
{
    ...
    // Update network parameters
    if (!set_network(get_iface_name(), std::to_string(network_id), ssid, bssid, sec, pass,
                     hidden_ssid)) {
...
bool sta_wlan_hal_fapi::roam(const std::string &bssid, uint8_t channel)
{
    ...
    LOG(TRACE) << __func__ << ": iface = " << iface << " to bssid " << bssid << " on channel "
               << int(channel);
    ...
    if (!set_network(iface, std::to_string(m_active_network_id), "", bssid, WiFiSec::Invalid, "",
                     false)) {
...
bool sta_wlan_hal_fapi::get_4addr_mode()
{
    ...
    if (fapi_wlan_client_mode_data_routing_mode_get(get_iface_name().c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
    ...
    // Extract the network ID
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint.Profile", 0, "X_LANTIQ_COM_Vendor_RoutingMode",
                       tmpBuff) != UGW_SUCCESS) {
        LOG(ERROR) << "Failed parsing FAPI object!";
        return false;
    }
...
bool sta_wlan_hal_fapi::set_4addr_mode(bool enable)
{
    ...
    if (fapi_wlan_client_mode_data_routing_mode_set(get_iface_name().c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool sta_wlan_hal_fapi::unassoc_rssi_measurement(const std::string &mac, int chan, int bw,
                                                 int vht_center_frequency, int delay,
                                                 int window_size)
{
    LOG(TRACE) << __func__ << " mac: " << mac << ", channel: " << chan << ", bw: " << bw
               << ", vht_center_frequency: " << vht_center_frequency << ", delay: " << delay;
    ...
    if (fapi_wlan_unassociated_devices_info_req(get_iface_name().c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool sta_wlan_hal_fapi::is_connected()
{
    ...
    if (fapi_wlan_client_mode_connection_status_get(get_iface_name().c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
int sta_wlan_hal_fapi::get_rssi() { return beerocks::RSSI_INVALID; }

int sta_wlan_hal_fapi::get_channel() { return m_active_channel; }

std::string sta_wlan_hal_fapi::get_ssid() { return m_active_ssid; }

std::string sta_wlan_hal_fapi::get_bssid() { return m_active_bssid; }
...
bool sta_wlan_hal_fapi::parse_fapi_event(const std::string &opcode, std::shared_ptr<void> obj)
{
    ...
    case Event::Disconnected: {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.EndPoint", 0, "X_LANTIQ_COM_Vendor_Reason",
                           tmpBuff) != UGW_SUCCESS) {
    ...
    case Event::STA_Unassoc_RSSI: {
        ...
        // TODO: Change to HAL objects
        auto msg_buff =
            ALLOC_SMART_BUFFER(sizeof(sACTION_BACKHAUL_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE));
        auto msg = reinterpret_cast<sACTION_BACKHAUL_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE *>(
            msg_buff.get());
        if (!msg) {
            LOG(FATAL) << "Memory allocation failed!";
            return false;
