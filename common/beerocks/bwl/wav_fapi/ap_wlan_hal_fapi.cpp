...
static bool fapi_read_int(const std::shared_ptr<ObjList> wlObj, const char *objName,
                          const char *paramName, const char *tmpBuff, int &value)
{
...
static bool get_sta_caps(SRadioCapabilitiesStrings &caps_strings, sRadioCapabilities &sta_caps,
                         bool is_5ghz)
{
...
ap_wlan_hal_fapi::ap_wlan_hal_fapi(std::string iface_name, bool acs_enabled,
                                   hal_event_cb_t callback)
    : base_wlan_hal(bwl::HALType::AccessPoint, iface_name, IfaceType::Intel, acs_enabled, callback),
      base_wlan_hal_fapi(bwl::HALType::AccessPoint, iface_name, IfaceType::Intel, acs_enabled,
                         callback)
{
...
{
    LOG(TRACE) << __func__ << " mac: " << mac << " iface_name = " << get_radio_info().iface_name;
...
bool ap_wlan_hal_fapi::sta_disassoc(int8_t vap_id, const std::string &mac, uint32_t reason)
{
    ...
    // Build the object
    UGW_OBJLIST_ADD(wlObj, "Device.WiFi.AccessPoint.X_LANTIQ_COM_Vendor");
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.AccessPoint.X_LANTIQ_COM_Vendor", "MACAddress",
                          mac.c_str(), 0, 0);
...
bool ap_wlan_hal_fapi::sta_bss_steer(const std::string &mac, const std::string &bssid, int chan,
                                     int disassoc_timer, int valid_int)
{
    ...
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.SSID", "BSSID", bssid.c_str(), 0, 0);
    ...
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.SSID.X_LANTIQ_COM_Vendor", "0_neighbor",
                          std::string(bssid + ",0,0," + std::to_string(chan) + ",0,255").c_str(), 0,
                          0);
    ...
    // Execute the command
    if (fapi_wlan_bss_transition_management_req(get_radio_info().iface_name.c_str(), wlObj.get(),
                                                0) == UGW_FAILURE) {
...
bool ap_wlan_hal_fapi::sta_unassoc_rssi_measurement(const std::string &mac, int chan, int bw,
                                                    int vht_center_frequency, int delay,
                                                    int window_size)
{
    LOG(TRACE) << __func__ << " mac: " << mac << ", channel: " << chan << ", bw: " << bw
               << ", delay: " << delay;
    ...
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "CenterFrequency",
                          centerFreq.c_str(), 0, 0);
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "WaveVhtCenterFrequency",
                          waveVhtCenterFreq.c_str(), 0, 0);

    UGW_OBJLIST_ADD(wlObj, "Device.WiFi.Radio");
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio", "OperatingChannelBandwidth",
                          chanBandwidth.c_str(), 0, 0);
    ...
    if (fapi_wlan_unassociated_devices_info_req(get_radio_info().iface_name.c_str(), wlObj.get(),
                                                0) == UGW_FAILURE) {
...
bool ap_wlan_hal_fapi::switch_channel(int chan, int bw, int vht_center_frequency)
{
    ...
    if (chan == 0) {
        ...
        LOG(DEBUG) << __func__ << ": channel = " << int(chan) << ", bandwidth = " << bandwidth_str
                   << ", vht_center_frequency = " << vht_center_frequency;
        ...
        if (bw != 20) {
            UGW_OBJLIST_EDIT_NODE(
                wlObj, "Device.WiFi.Radio", "ExtensionChannel",
                ((freq < vht_center_frequency) ? "AboveControlChannel" : "BelowControlChannel"), 0,
                0);
        }
...
bool ap_wlan_hal_fapi::failsafe_channel_set(int chan, int bw, int vht_center_frequency)
{
    ...
    // Transition management parameters
    UGW_OBJLIST_ADD(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor");
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "Channel",
                          chan_str.c_str(), 0, 0);
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "CenterFrequency",
                          freq_str.c_str(), 0, 0);
    UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor",
                          "OperatingChannelBandwidth", bw_str.c_str(), 0, 0);

    if (fapi_wlan_failsafe_channel_set(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool ap_wlan_hal_fapi::failsafe_channel_get(int &chan, int &bw)
{
    ...
    if (fapi_wlan_failsafe_channel_get(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
    ...
    // Read the frequency
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "freq",
                       m_temp_fapi_value.get()) != UGW_SUCCESS) {
    ...
    chan = beerocks::utils::wifi_freq_to_channel(
        beerocks::string_utils::stoi(m_temp_fapi_value.get()));

    // Read the bandwidth
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "bandwidth",
                       m_temp_fapi_value.get()) != UGW_SUCCESS) {
...
bool ap_wlan_hal_fapi::restricted_channels_set(char *channel_list)
{
    ...
    if (!temp_channel_list.empty()) {
        UGW_OBJLIST_ADD(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor");
        UGW_OBJLIST_EDIT_NODE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", "ChannelList",
                              temp_channel_list.c_str(), 0, 0);
    }

    if (fapi_wlan_restricted_channels_set(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool ap_wlan_hal_fapi::restricted_channels_get(char *channel_list)
{
    ...
    if (fapi_wlan_restricted_channels_get(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
    ...
    if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "restricted_channels",
                       tmpBuff) != UGW_SUCCESS) {
...
bool ap_wlan_hal_fapi::read_acs_report()
{
    ...
    if (fapi_wlan_acs_report_get(get_radio_info().iface_name.c_str(), wlObj.get(), 0) ==
        UGW_FAILURE) {
...
bool ap_wlan_hal_fapi::parse_fapi_event(const std::string &opcode, std::shared_ptr<void> obj)
{
    ...
    case Event::CSA_Finished: {
        ...
        if (!fapi_read_int(wlObj, "Device.WiFi.Radio", "ExtensionChannel", tmpBuff,
        ...
        if (m_radio_info.channel == chan && m_radio_info.bandwidth == bw &&
            event == Event::CSA_Finished && chanSwReason != ChanSwReason::Radar) {
            LOG(DEBUG) << "CSA_Finished for the same channel and bw ignore" << std::endl
                       << " m_radio_info.channel = " << int(m_radio_info.channel)
                       << " chan = " << int(chan)
                       << "m_radio_info.bandwidth = " << int(m_radio_info.bandwidth)
                       << " bw = " << int(bw);
            return false;
        }
    ...
    case Event::DFS_RADAR_Detected: {
        ...
        LOG(DEBUG) << "DFS-RADAR-Detected " << std::endl
                   << " wifi_ctrl_enabled = " << int(m_radio_info.wifi_ctrl_enabled)
                   << " channel = " << int(m_radio_info.channel)
                   << " bandwidth = " << int(m_radio_info.bandwidth)
                   << " wireless_utils::is_dfs_channel(m_radio_info.channel) "
                   << int(son::wireless_utils::is_dfs_channel(m_radio_info.channel));
        ...
        if (m_radio_info.wifi_ctrl_enabled == 0 && //ap value on first cac interupted by radar.
            ...
            m_radio_info.vht_center_freq =
                beerocks::utils::wifi_channel_to_vht_center_freq(ch, bw, true);
    ...
    case Event::STA_Connected: {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.X_LANTIQ_COM_Vendor.AssociatedDevice", 0,
                           "SupportedRates", tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.X_LANTIQ_COM_Vendor.AssociatedDevice", 0,
                           "btm_supported", tmpBuff) != UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.X_LANTIQ_COM_Vendor.AssociatedDevice", 0,
                           "nr_enabled", tmpBuff) != UGW_SUCCESS) {
    ...
    case Event::STA_Unassoc_RSSI: {
        ...
        // Read the RSSI value
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.AssociatedDevice", 0, "SignalStrength",
                           tmpBuff) != UGW_SUCCESS) {
        ...
        if ((rssi_cnt > 0) && (msg->params.rx_packets > 1)) {
            ...
            if (temp_sum_delay > m_unassoc_measure_window_size) {
                ...
                msg->params.rx_packets =
                    -2; // means the actual measurement started later then aspected
            }
        }
    ...
    case Event::BSS_TM_Response: {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.AccessPoint.X_LANTIQ_COM_Vendor.AssociatedDevice", 0,
                           "status_code", tmpBuff) != UGW_SUCCESS) {
    ...
    case Event::DFS_CAC_Completed: {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "success", tmpBuff) !=
            UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "freq", tmpBuff) !=
            UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "timeout", tmpBuff) !=
            UGW_SUCCESS) {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "chan_width",
                           tmpBuff) != UGW_SUCCESS) {
    ...
    case Event::DFS_NOP_Finished: {
        ...
        if (UGW_READ_VALUE(wlObj, "Device.WiFi.Radio.X_LANTIQ_COM_Vendor", 0, "cf1", tmpBuff) !=
            UGW_SUCCESS) {
