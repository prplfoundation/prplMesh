...
{
    switch (value) {
    case wav_fsm_event::Attach:
        out << "Attach";
        break;
    case wav_fsm_event::Detach:
        out << "Detach";
        break;
    }
    return out;
...
static void listed_map_obj_parser(std::stringstream &ss_in, std::list<char> delimiter_list,
                                  parsed_obj_listed_map_t &listed_map_obj)
{
    if (delimiter_list.empty())
        return;
...
base_wlan_hal_wav::base_wlan_hal_wav(HALType type, std::string iface_name, bool acs_enabled,
...
bool base_wlan_hal_wav::wav_obj_read_int(const std::string &key, parsed_obj_map_t &obj,
...
bool base_wlan_hal_wav::wav_obj_read_str(const std::string &key, parsed_obj_map_t &obj,
                                         char **value)
{
    auto val_iter = obj.find(key);
    if (val_iter == obj.end()) {
...
base_wlan_hal_wav::~base_wlan_hal_wav() { detach(); }
...
HALState base_wlan_hal_wav::attach(bool block)
{
    while (true) {
        ...
        if (m_last_attach_state != attach_state) {
            LOG(DEBUG) << "WAV FSM " << m_radio_info.iface_name
...
bool base_wlan_hal_wav::process_ext_events()
{
    ...
    buffer[buff_size_copy] =
        0; // the wpa_ctrl does not put null termintaor at the and of the string

    /* Silencing unhandled multiple events */
    if (strstr(buffer, "WPS-ENROLLEE-SEEN")) {
        LOG_EVERY_N(UNHANDLED_EVENTS_LOGS, DEBUG)
            << "DWPAL unhandled event WPS-ENROLLEE-SEEN recieved";
        return true;
    } else if (strstr(buffer, "AP-PROBE-REQ-RECEIVED")) {
        LOG_EVERY_N(UNHANDLED_EVENTS_LOGS, DEBUG)
            << "DWPAL unhandled event AP-PROBE-REQ-RECEIVED recieved";
        return true;
    } else if (strstr(buffer, "RRM-BEACON-REP-RECEIVED")) {
        LOG_EVERY_N(UNHANDLED_EVENTS_LOGS, DEBUG)
            << "DWPAL unhandled event RRM-BEACON-REP-RECEIVED recieved";
        return true;
    }
