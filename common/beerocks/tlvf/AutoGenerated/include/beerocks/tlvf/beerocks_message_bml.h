...
class cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY : public BaseClass
{
    public:
        cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY(uint8_t* buff, size_t buff_len, bool parse = false, bool swap_needed = false);
        cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY(std::shared_ptr<BaseClass> base, bool parse = false, bool swap_needed = false);
        ~cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY();

        static eActionOp_BML get_action_op(){
            return (eActionOp_BML)(ACTION_BML_TRIGGER_AP_CAPABILITY_QUERY);
        }
        sMacAddr& al_mac();
        void class_swap();
        static size_t get_initial_size();
