...
message_com::parse_intel_vs_message(ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    ieee1905_1::TlvList vs_contents(tlv_header->payload(), tlv_header->payload_length());//TODO: this probably causes the errors with buffer size
...
std::string message_com::print_cmdu_types(const message::sUdsHeader *uds_header,
                                          sCmduInfo *cmdu_info)
{
    ieee1905_1::CmduMessageRx cmdu_rx = ieee1905_1::CmduMessageRx((uint8_t *)uds_header + sizeof(message::sUdsHeader),
                                     uds_header->length);
