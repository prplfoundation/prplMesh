...
bool cACTION_PLATFORM_ONBOARD_QUERY_RESPONSE::init()
{
    ...
    m_params = reinterpret_cast<sOnboarding*>(m_buff_ptr__);
...
bool cACTION_PLATFORM_WIFI_CREDENTIALS_GET_REQUEST::init()
{
    ...
    m_vap_id = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
...
bool cACTION_PLATFORM_WIFI_CREDENTIALS_GET_RESPONSE::init()
{
    ...
    m_front_params = reinterpret_cast<sWifiCredentials*>(m_buff_ptr__);
    ...
    m_back_params = reinterpret_cast<sWifiCredentials*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sWifiCredentials))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sWifiCredentials) << ") Failed!";
        return false;
    }
...
bool cACTION_PLATFORM_ADMIN_CREDENTIALS_GET_RESPONSE::init()
{
    ...
    m_params = reinterpret_cast<sAdminCredentials*>(m_buff_ptr__);
...
bool cACTION_PLATFORM_VERSION_MISMATCH_NOTIFICATION::init()
{
    ...
    m_versions = reinterpret_cast<sVersions*>(m_buff_ptr__);
...
bool cACTION_PLATFORM_ERROR_NOTIFICATION::init()
{
    ...
    m_code = reinterpret_cast<uint32_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint32_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint32_t) << ") Failed!";
        return false;
    }
...
cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::~cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST() {
}
sMacAddr& cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::params() {
    return (sMacAddr&)(*m_params);
}

void cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_PLATFORM), reinterpret_cast<uint8_t*>(m_action_op));
    m_params->struct_swap();
}

bool cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // params
    return class_size;
}

bool cACTION_PLATFORM_MESSAGE_TO_RADIO_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_params = reinterpret_cast<sMacAddr*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sMacAddr))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sMacAddr) << ") Failed!";
        return false;
    }
...
cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::~cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE() {
}
uint8_t& cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::result() {
    return (uint8_t&)(*m_result);
}

void cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_PLATFORM), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_PLATFORM_MESSAGE_TO_RADIO_RESPONSE::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
