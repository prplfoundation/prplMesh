...
cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::~cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY() {
}
sMacAddr& cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::mac() {
    return (sMacAddr&)(*m_mac);
}

void cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_mac->struct_swap();
}

bool cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // mac
    return class_size;
...
bool cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::~cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE() {
}
sMacAddr& cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::sta_mac() {
    return (sMacAddr&)(*m_sta_mac);
}

uint8_t& cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::bss_num() {
    return (uint8_t&)(*m_bss_num);
}

std::tuple<bool, sPerBssAssociatedStaLinkMetrics&> cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::per_bss_associated_sta_link_metrics(size_t idx) {
...
bool cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::alloc_per_bss_associated_sta_link_metrics(size_t count) {
...
void cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
...
bool cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_BACKHAUL_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
