...
cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::~cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST() {
}
sDcsScanRequestParams& cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::scan_params() {
    return (sDcsScanRequestParams&)(*m_scan_params);
}

void cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::class_swap()
{
    m_scan_params->struct_swap();
}

size_t cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sDcsScanRequestParams); // scan_params
    return class_size;
}

bool cACTION_MONITOR_DCS_TRIGGER_SCAN_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_scan_params = (sDcsScanRequestParams*)m_buff_ptr__;
...
cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::~cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE() {
}
uint8_t& cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::success() {
    return (uint8_t&)(*m_success);
}

void cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::class_swap()
{
}

size_t cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_MONITOR_DCS_TRIGGER_SCAN_RESPONSE::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION::~cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION() {
}
void cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION::class_swap()
{
}

size_t cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_MONITOR_DCS_SCAN_TRIGGERED_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::~cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION() {
}
sDcsChannelScanResults& cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::scan_results() {
    return (sDcsChannelScanResults&)(*m_scan_results);
}

void cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::class_swap()
{
    m_scan_results->struct_swap();
}

size_t cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_MONITOR_DCS_SCAN_RESULTS_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::~cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION() {
}
uint8_t& cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::reason() {
    return (uint8_t&)(*m_reason);
}

void cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::class_swap()
{
}

size_t cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_MONITOR_DCS_SCAN_ABORT_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION::cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION::~cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION() {
}
void cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION::class_swap()
{
}

size_t cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_MONITOR_DCS_SCAN_FINISHED_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
