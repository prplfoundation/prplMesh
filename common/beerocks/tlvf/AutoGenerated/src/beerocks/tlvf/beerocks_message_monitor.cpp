...
cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::~cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION() {
}
sMacAddr& cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::sta_mac() {
    return (sMacAddr&)(*m_sta_mac);
}

int8_t& cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::vap_id() {
    return (int8_t&)(*m_vap_id);
}

void cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_MONITOR), reinterpret_cast<uint8_t*>(m_action_op));
    m_sta_mac->struct_swap();
}

bool cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // sta_mac
    class_size += sizeof(int8_t); // vap_id
...
bool cACTION_MONITOR_CLIENT_ASSOCIATED_NOTIFICATION::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
bool cACTION_MONITOR_HOSTAP_STATUS_CHANGED_NOTIFICATION::init()
{
    ...
    m_new_tx_state = reinterpret_cast<int8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(int8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(int8_t) << ") Failed!";
        return false;
    }
    m_new_hostap_enabled_state = reinterpret_cast<int8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(int8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(int8_t) << ") Failed!";
        return false;
    }
