...
cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY(uint8_t* buff, size_t buff_len, bool parse, bool swap_needed) :
    BaseClass(buff, buff_len, parse, swap_needed) {
    m_init_succeeded = init();
}
cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY(std::shared_ptr<BaseClass> base, bool parse, bool swap_needed) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse, swap_needed){
    m_init_succeeded = init();
}
cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::~cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY() {
}
sMacAddr& cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::al_mac() {
    return (sMacAddr&)(*m_al_mac);
}

void cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::class_swap()
{
    m_al_mac->struct_swap();
}

size_t cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // al_mac
    return class_size;
...
bool cACTION_BML_TRIGGER_AP_CAPABILITY_QUERY::init()
{
    if (getBuffRemainingBytes() < kMinimumLength) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
