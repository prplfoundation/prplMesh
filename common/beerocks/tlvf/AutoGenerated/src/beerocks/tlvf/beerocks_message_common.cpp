...
sLinkMetricQuery::sLinkMetricQuery(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
sLinkMetricQuery::sLinkMetricQuery(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
sLinkMetricQuery::~sLinkMetricQuery() {
}
eNeighborType& sLinkMetricQuery::neighbor_type() {
    return (eNeighborType&)(*m_neighbor_type);
}

sMacAddr& sLinkMetricQuery::mac_al_1905_device() {
    return (sMacAddr&)(*m_mac_al_1905_device);
}

eLinkMetricsType& sLinkMetricQuery::link_metrics() {
    return (eLinkMetricsType&)(*m_link_metrics);
}

void sLinkMetricQuery::class_swap()
{
...
bool sLinkMetricQuery::finalize()
{
    if (m_parse__) {
...
size_t sLinkMetricQuery::get_initial_size()
{
    size_t class_size = 0;
