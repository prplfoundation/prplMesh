...
{
	std::ostringstream s;
	s << topic << std::setfill(PubSocket::topic_pad_char) << std::setw(Message::kMaxTopicSize - topic.length()) << '\0';
	return s.str();
}
...
bool SubSocket::Receive(Message& msg, int flags)
{
	...
	Message::Frame frame(size, (uint8_t*)buf + Message::kMaxTopicSize); //The constructor copies the data into the frame internal buffer
	msg.Add(frame);
	nng_free(buf, size);
...
std::unique_ptr<Message> SubSocket::Receive(int flags)
{
	...
	Message::Frame frame(nbytes, (uint8_t*)buf + Message::kMaxTopicSize); //The constructor copies the data into the frame internal buffer
	nng_free(buf, size);
	DBG("message received");
