...
struct msglib_pollitems {
	...
	std::vector<pollfd>::iterator Find(const Socket& socket)
	{
		return Find(socket.fd());
	}
...
{
	return Modify(socket.fd(), events);
}
...
void Poller::PrintItems() const
{
	...
	for (auto item:items_->items_)
		MAPF_INFO("socket. fd:" << item.fd << ", events: " << item.events << ", revents: " << item.revents);
}
