...
const std::string DEVICE_BEEROCKS_FRONT_WIFI_OBJECT_NAME("Device.X_INTEL_COM_BEEROCKS.WiFi.Front.Ap.1");
...
class cal_query_wifi
{
  public:
    ...
    {
        std::string object_name = DEVICE_WIFI_ACCESSPOINT_OBJECT_NAME + "." + std::to_string(ap_number);
    ...
    bool get_wifi_ssid_and_security_entry(int index, cal_settings::SAPSettings& ap_settings)
    {
        ...
        if (index_ap > 0) {
            ...
            std::string object_name_accesspoint = DEVICE_WIFI_ACCESSPOINT_OBJECT_NAME + "." + index_suffix;
    ...
    bool get_wifi_radio_entry(std::string iface_name, cal_settings::SAPSettings& ap_settings)
    {
        ...
        for (int i = 1; i < ip_interface_count; i++)
        {
            ...
            if (0 == rv) {
                ...
                ap_settings.enabled = (obj_it->get_param("Enable") == "1") || (obj_it->get_param("Enable") == "true");
    ...
    bool get_backhaul_params(int& max_vaps, int& network_enabled, int& prefered_radio_band) {
        ...
        prefered_radio_band = (obj_it->get_param("BackhaulPreferedRadioBand") == "5G" ? BPL_RADIO_BAND_5G : BPL_RADIO_BAND_2G);
    ...
    int get_backhaul_vaps(char* backhaul_vaps_buf, const int buf_len)
    {
        ...
        for (uint8_t i = 1; i <= BPL_BACK_VAPS_GROUPS; i++)
        {
            ...
            backhaul_vaps_buf += BPL_MAC_ADDR_OCTETS_LEN;
            utils::mac_from_string((uint8_t*)backhaul_vaps_buf, obj_it->get_param("BSSID_5G_Secondary"));
            backhaul_vaps_buf += BPL_MAC_ADDR_OCTETS_LEN;
        }
    ...
    int beerocks_get_enable()
    { return (beerocks_read_bool_value("Enable")); }

    int beerocks_is_onboarding()
    { return (beerocks_read_bool_value("Onboarding")); }

    int beerocks_get_band_steering()
    { return (beerocks_read_bool_value("BandSteering")); }
    
    int beerocks_get_client_roaming()
    { return (beerocks_read_bool_value("ClientRoaming")); }
        ...
        for (auto obj : obj_list) {
            ...
            if (name == "Device.DeviceInfo.") {
                ...
                if (obj.get_param("X_LANTIQ_COM_UpStream") == "bridge") {
                    ...
                    lan_iface_ip_obj = "Device.IP.Interface." + name.substr(name.find_last_of('.') + 1) + ".IPv4Address.1";
                ...
                if (obj.get_param("X_LANTIQ_COM_DefaultGateway") == "true") {
                    ...
                    wan_iface_ip_obj = "Device.IP.Interface." + name.substr(name.find_last_of('.') + 1) + ".IPv4Address.1";
    ...
    int beerocks_get_wifi_credentials(const int radio_dir, std::string& ssid, std::string& pass, std::string& sec)
    {
        ...
        msg.add_get_object((radio_dir ? DEVICE_BEEROCKS_BACK_WIFI_OBJECT_NAME : DEVICE_BEEROCKS_FRONT_WIFI_OBJECT_NAME))
            .add_get_param("ModeEnabled")
            .add_get_param("SSID")
            .add_get_param("WEPKey")
            .add_get_param("KeyPassphrase");
    ...
    {
        cal_message msg(MOPT_SET, SOPT_OBJVALUE, OWN_WEB); // set OWN_WEB will make the platform to onboard 
        auto cal_obj1 = msg.add_set_object((radio_dir ? DEVICE_BEEROCKS_BACK_WIFI_OBJECT_NAME : DEVICE_BEEROCKS_FRONT_WIFI_OBJECT_NAME), OBJOPT_MODIFY);
    ...
    int beerocks_set_wifi_credentials(std::string& iface, std::string& ssid, std::string& pass, std::string& sec)
    {
        ...
        for (int i = 1; i < ip_interface_count; i++)
        {
            ...
            msg1.add_get_object(object_name)
                .add_get_param("Name");
            ...
            if (0 == rv) {
                ...
                cal_obj1.add_set_param("SSID", ssid.c_str());
                std::string object_name2 = DEVICE_WIFI_ACCESSPOINT_OBJECT_NAME + "." + index_suffix + ".Security";
    ...
    {
        cal_message msg(MOPT_GET, SOPT_OBJVALUE | GET_PWD_VAL);
        msg.add_get_object(DEVICE_BEEROCKS_USER_PASSWORD_OBJECT_NAME)
            .add_get_param("Password");
    ...
    {
        return (beerocks_read_bool_value("WiredBackhaul")); 
    } 
...
{ cal_query_wifi cal(m_cal); return (cal.beerocks_get_enable()); }

int cal_settings::beerocks_is_master()
{ cal_query_wifi cal(m_cal); return (cal.beerocks_is_master()); }

int cal_settings::beerocks_get_operating_mode()
{ cal_query_wifi cal(m_cal); return (cal.beerocks_get_operating_mode()); }

int cal_settings::beerocks_is_onboarding()
{ cal_query_wifi cal(m_cal); return (cal.beerocks_is_onboarding()); }

int cal_settings::beerocks_get_band_steering()
{ cal_query_wifi cal(m_cal); return (cal.beerocks_get_band_steering()); }

int cal_settings::beerocks_get_client_roaming()
{ cal_query_wifi cal(m_cal); return (cal.beerocks_get_client_roaming()); }
...
int cal_settings::beerocks_get_wifi_params(std::string iface_name, cal_settings::SAPSettings &ap_settings)
{
...
int cal_settings::beerocks_get_backhaul_params(int& max_vaps, int& network_enabled, int& prefered_radio_band)
{
    cal_query_wifi cal_query(m_cal);
...
int cal_settings::beerocks_get_wifi_credentials(int radio_dir, std::string& ssid, std::string& pass, std::string& sec)
{
    cal_query_wifi cal_query(m_cal);    
    return (cal_query.beerocks_get_wifi_credentials(radio_dir, ssid, pass, sec));
}

int cal_settings::beerocks_set_beerocks_credentials(const int radio_dir, std::string ssid, std::string pass, std::string sec)
{
    cal_query_wifi cal_query(m_cal);    
...
int cal_settings::beerocks_set_wifi_credentials(std::string iface, std::string ssid, std::string pass, std::string sec)
{
    cal_query_wifi cal_query(m_cal);    
...
{ cal_query_wifi cal(m_cal); return (cal.beerocks_is_wired_backhaul()); }
