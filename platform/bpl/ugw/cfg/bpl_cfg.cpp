...
const char* s_error_strings[] = {
    FOREACH_ERROR_CODE(GENERATE_ERROR_STRING)
};
...
int bpl_cfg_get_device_info(BPL_DEVICE_INFO* device_info)
{
    ...
    // Copy the value
    utils::copy_string(device_info->manufacturer,   cal_device_info.manufacturer.c_str(),  BPL_DEV_INFO_LEN);
    utils::copy_string(device_info->model_name,     cal_device_info.model_name.c_str(),    BPL_DEV_INFO_LEN);
    utils::copy_string(device_info->serial_number,  cal_device_info.serial_number.c_str(), BPL_DEV_INFO_LEN);

    // LAN
    if (!cal_device_info.lan_interface_name.empty()) {
        utils::copy_string(device_info->lan_iface_name, cal_device_info.lan_interface_name.c_str(), BPL_IFNAME_LEN);
        device_info->lan_ip_address = utils::uint_ipv4_from_string(cal_device_info.lan_interface_ip);
        device_info->lan_network_mask = utils::uint_ipv4_from_string(cal_device_info.lan_interface_netmask);
    }

    // WAN
    if (!cal_device_info.wan_interface_name.empty()) {
        utils::copy_string(device_info->wan_iface_name, cal_device_info.wan_interface_name.c_str(), BPL_IFNAME_LEN);
        device_info->wan_ip_address = utils::uint_ipv4_from_string(cal_device_info.wan_interface_ip);
        device_info->wan_network_mask = utils::uint_ipv4_from_string(cal_device_info.wan_interface_netmask);
    }
...
int bpl_cfg_get_wifi_params(const char* iface, struct BPL_WLAN_PARAMS* wlan_params)
{
    ...
    wlan_params->advertise_ssid = ap_settings.advertise_ssid?1:0;
...
{
    return (sl_beerocks_set_wifi_advertise_ssid(iface, advertise_ssid) ? 0 : -1);
}
...
{
    return (sl_beerocks_notify_fw_mismatch() ? 0 : -1);
}

int bpl_cfg_notify_error(int code, const char data[BPL_ERROR_STRING_LEN])
{   
    if(code >= BPL_ERR_LAST) return -1;
...
int bpl_cfg_get_administrator_credentials(char pass[BPL_PASS_LEN])
{
    ...
    // TODO: Use constants for sizes
    if (pass) utils::copy_string(pass, str_pass.c_str(), BPL_USER_PASS_LEN);
