...
tlvNon1905neighborDeviceList::tlvNon1905neighborDeviceList(uint8_t *buff, size_t buff_len,
                                                           bool parse, bool swap_needed)
    : BaseClass(buff, buff_len, parse, swap_needed)
{
    m_init_succeeded = init();
}
tlvNon1905neighborDeviceList::tlvNon1905neighborDeviceList(std::shared_ptr<BaseClass> base,
                                                           bool parse, bool swap_needed)
...
const eTlvType &tlvNon1905neighborDeviceList::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvNon1905neighborDeviceList::length() { return (const uint16_t &)(*m_length); }
...
bool tlvNon1905neighborDeviceList::alloc_mac_non_1905_device(size_t count)
{
    ...
    if (!m_parse__) {
        for (size_t i = m_mac_non_1905_device_idx__ - count; i < m_mac_non_1905_device_idx__; i++) {
            m_mac_non_1905_device[i].struct_init();
        }
    }
    return true;
}
...
bool tlvNon1905neighborDeviceList::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_NON_1905_NEIGHBOR_DEVICE_LIST;
    else {
        ...
        if (swapped_type != eTlvType::TLV_NON_1905_NEIGHBOR_DEVICE_LIST) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_NON_1905_NEIGHBOR_DEVICE_LIST)
