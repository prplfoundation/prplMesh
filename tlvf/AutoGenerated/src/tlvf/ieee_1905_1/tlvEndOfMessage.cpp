...
const eTlvType &tlvEndOfMessage::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvEndOfMessage::length() { return (const uint16_t &)(*m_length); }
...
bool tlvEndOfMessage::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_END_OF_MESSAGE;
    else {
        ...
        if (swapped_type != eTlvType::TLV_END_OF_MESSAGE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_END_OF_MESSAGE)
