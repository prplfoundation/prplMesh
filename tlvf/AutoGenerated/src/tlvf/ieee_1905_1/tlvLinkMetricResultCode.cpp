...
const eTlvType &tlvLinkMetricResultCode::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvLinkMetricResultCode::length() { return (const uint16_t &)(*m_length); }

tlvLinkMetricResultCode::eValue &tlvLinkMetricResultCode::value() { return (eValue &)(*m_value); }
...
bool tlvLinkMetricResultCode::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_LINK_METRIC_RESULT_CODE;
    else {
        ...
        if (swapped_type != eTlvType::TLV_LINK_METRIC_RESULT_CODE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_LINK_METRIC_RESULT_CODE)
