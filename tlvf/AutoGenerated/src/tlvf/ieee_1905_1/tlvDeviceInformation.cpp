...
const eTlvType &tlvDeviceInformation::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvDeviceInformation::length() { return (const uint16_t &)(*m_length); }

sMacAddress &tlvDeviceInformation::mac() { return (sMacAddress &)(*m_mac); }

uint8_t &tlvDeviceInformation::info_length() { return (uint8_t &)(*m_info_length); }
...
bool tlvDeviceInformation::alloc_info(size_t count)
{
    ...
    *m_info_length += count;
    ...
    if (!m_parse__) {
        for (size_t i = m_info_idx__ - count; i < m_info_idx__; i++) {
            m_info[i].struct_init();
        }
    }
    return true;
}
...
bool tlvDeviceInformation::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_DEVICE_INFORMATION;
    else {
        ...
        if (swapped_type != eTlvType::TLV_DEVICE_INFORMATION) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_DEVICE_INFORMATION)
