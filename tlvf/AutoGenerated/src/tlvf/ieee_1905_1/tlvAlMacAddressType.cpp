...
const eTlvType &tlvAlMacAddressType::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvAlMacAddressType::length() { return (const uint16_t &)(*m_length); }

sMacAddress &tlvAlMacAddressType::mac() { return (sMacAddress &)(*m_mac); }
...
bool tlvAlMacAddressType::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_AL_MAC_ADDRESS_TYPE;
    else {
        ...
        if (swapped_type != eTlvType::TLV_AL_MAC_ADDRESS_TYPE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_AL_MAC_ADDRESS_TYPE)
