...
const eTlvType &tlvSupportedRole::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvSupportedRole::length() { return (const uint16_t &)(*m_length); }

tlvSupportedRole::eValue &tlvSupportedRole::value() { return (eValue &)(*m_value); }
...
bool tlvSupportedRole::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_SUPPORTED_ROLE;
    else {
        ...
        if (swapped_type != eTlvType::TLV_SUPPORTED_ROLE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_SUPPORTED_ROLE)
