...
const eTlvType &tlvAutoconfigFreqBand::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvAutoconfigFreqBand::length() { return (const uint16_t &)(*m_length); }

tlvAutoconfigFreqBand::eValue &tlvAutoconfigFreqBand::value() { return (eValue &)(*m_value); }
...
bool tlvAutoconfigFreqBand::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_AUTOCONFIG_FREQ_BAND;
    else {
        ...
        if (swapped_type != eTlvType::TLV_AUTOCONFIG_FREQ_BAND) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_AUTOCONFIG_FREQ_BAND)
