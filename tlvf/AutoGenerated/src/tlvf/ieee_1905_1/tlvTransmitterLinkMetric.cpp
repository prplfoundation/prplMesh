...
const eTlvType &tlvTransmitterLinkMetric::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvTransmitterLinkMetric::length() { return (const uint16_t &)(*m_length); }
...
sMacAddress &
tlvTransmitterLinkMetric::al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv()
{
    return (sMacAddress &)(*m_al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv);
}

std::tuple<bool, tlvTransmitterLinkMetric::sInterfacePairInfo &>
tlvTransmitterLinkMetric::interface_pair_info(size_t idx)
{
...
bool tlvTransmitterLinkMetric::alloc_interface_pair_info(size_t count)
{
    ...
    if (!m_parse__) {
        for (size_t i = m_interface_pair_info_idx__ - count; i < m_interface_pair_info_idx__; i++) {
            m_interface_pair_info[i].struct_init();
        }
    }
    return true;
}
...
size_t tlvTransmitterLinkMetric::get_initial_size()
{
    ...
    class_size +=
        sizeof(sMacAddress); // al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv
    return class_size;
}
...
bool tlvTransmitterLinkMetric::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_TRANSMITTER_LINK_METRIC;
    else {
        ...
        if (swapped_type != eTlvType::TLV_TRANSMITTER_LINK_METRIC) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_TRANSMITTER_LINK_METRIC)
    ...
    if (!m_parse__) {
        m_al_mac_of_the_device_that_transmits->struct_init();
    }
    m_al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv =
        (sMacAddress *)m_buff_ptr__;
    m_buff_ptr__ += sizeof(sMacAddress) * 1;
    if (m_length && !m_parse__) {
        (*m_length) += sizeof(sMacAddress);
    }
    if (!m_parse__) {
        m_al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv->struct_init();
    }
