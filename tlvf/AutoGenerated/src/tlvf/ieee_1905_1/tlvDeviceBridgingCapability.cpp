...
tlvDeviceBridgingCapability::tlvDeviceBridgingCapability(std::shared_ptr<BaseClass> base,
...
const eTlvType &tlvDeviceBridgingCapability::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvDeviceBridgingCapability::length() { return (const uint16_t &)(*m_length); }
...
bool tlvDeviceBridgingCapability::add_bridging_tuples_list(std::shared_ptr<cMacList> ptr)
{
    ...
    if (m_lock_allocation__ == false) {
        TLVF_LOG(ERROR)
            << "No call to create_bridging_tuples_list was called before add_bridging_tuples_list";
        return false;
    }
...
bool tlvDeviceBridgingCapability::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_DEVICE_BRIDGING_CAPABILITY;
    else {
        ...
        if (swapped_type != eTlvType::TLV_DEVICE_BRIDGING_CAPABILITY) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_DEVICE_BRIDGING_CAPABILITY)
    ...
    if (!m_parse__)
        *m_bridging_tuples_list_length = 0;
...
cMacList::~cMacList() {}
uint8_t &cMacList::mac_list_length() { return (uint8_t &)(*m_mac_list_length); }
...
bool cMacList::alloc_mac_list(size_t count)
{
    ...
    *m_mac_list_length += count;
    m_buff_ptr__ += len;
    if (!m_parse__) {
        for (size_t i = m_mac_list_idx__ - count; i < m_mac_list_idx__; i++) {
            m_mac_list[i].struct_init();
        }
    }
    return true;
}
