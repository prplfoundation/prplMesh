...
tlvPushButtonEventNotification::tlvPushButtonEventNotification(uint8_t *buff, size_t buff_len,
                                                               bool parse, bool swap_needed)
    : BaseClass(buff, buff_len, parse, swap_needed)
{
    m_init_succeeded = init();
}
tlvPushButtonEventNotification::tlvPushButtonEventNotification(std::shared_ptr<BaseClass> base,
                                                               bool parse, bool swap_needed)
...
const eTlvType &tlvPushButtonEventNotification::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvPushButtonEventNotification::length() { return (const uint16_t &)(*m_length); }
...
std::tuple<bool, tlvPushButtonEventNotification::sMediaType &>
tlvPushButtonEventNotification::media_type_list(size_t idx)
{
...
bool tlvPushButtonEventNotification::alloc_media_type_list(size_t count)
{
    ...
    *m_media_type_list_length += count;
    ...
    if (!m_parse__) {
        for (size_t i = m_media_type_list_idx__ - count; i < m_media_type_list_idx__; i++) {
            m_media_type_list[i].struct_init();
        }
    }
    return true;
}
...
bool tlvPushButtonEventNotification::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_PUSH_BUTTON_EVENT_NOTIFICATION;
    else {
        ...
        if (swapped_type != eTlvType::TLV_PUSH_BUTTON_EVENT_NOTIFICATION) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_PUSH_BUTTON_EVENT_NOTIFICATION)
