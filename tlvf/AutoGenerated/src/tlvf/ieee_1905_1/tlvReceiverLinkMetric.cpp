...
const eTlvType &tlvReceiverLinkMetric::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvReceiverLinkMetric::length() { return (const uint16_t &)(*m_length); }
...
sMacAddress &
tlvReceiverLinkMetric::al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv()
{
    return (sMacAddress &)(*m_al_mac_of_the_neighbor_whose_link_metric_is_reported_in_this_tlv);
}

std::tuple<bool, tlvReceiverLinkMetric::sInterfacePairInfo &>
tlvReceiverLinkMetric::interface_pair_info(size_t idx)
{
...
bool tlvReceiverLinkMetric::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_RECEIVER_LINK_METRIC;
    else {
        ...
        if (swapped_type != eTlvType::TLV_RECEIVER_LINK_METRIC) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_RECEIVER_LINK_METRIC)
