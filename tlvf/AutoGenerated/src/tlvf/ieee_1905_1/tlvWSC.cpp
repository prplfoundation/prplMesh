...
tlvWSC::~tlvWSC() {}
const eTlvType &tlvWSC::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvWSC::length() { return (const uint16_t &)(*m_length); }
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_WSC;
        ...
        if (swapped_type != eTlvType::TLV_WSC) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: " << int(eTlvType::TLV_WSC)
