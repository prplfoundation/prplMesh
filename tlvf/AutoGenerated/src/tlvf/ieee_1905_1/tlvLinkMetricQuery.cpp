...
const eTlvType &tlvLinkMetricQuery::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvLinkMetricQuery::length() { return (const uint16_t &)(*m_length); }
...
bool tlvLinkMetricQuery::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_LINK_METRIC_QUERY;
    else {
        ...
        if (swapped_type != eTlvType::TLV_LINK_METRIC_QUERY) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_LINK_METRIC_QUERY)
