...
const eTlvType &tlvSearchedRole::type() { return (const eTlvType &)(*m_type); }

const uint16_t &tlvSearchedRole::length() { return (const uint16_t &)(*m_length); }

tlvSearchedRole::eValue &tlvSearchedRole::value() { return (eValue &)(*m_value); }
...
bool tlvSearchedRole::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvType::TLV_SEARCHED_ROLE;
    else {
        ...
        if (swapped_type != eTlvType::TLV_SEARCHED_ROLE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvType::TLV_SEARCHED_ROLE)
