...
const eTlvTypeMap &tlvSupportedService::type() { return (const eTlvTypeMap &)(*m_type); }

const uint16_t &tlvSupportedService::length() { return (const uint16_t &)(*m_length); }
...
std::tuple<bool, tlvSupportedService::eSupportedService &>
tlvSupportedService::supported_service_list(size_t idx)
{
    bool ret_success =
        ((m_supported_service_list_idx__ > 0) && (m_supported_service_list_idx__ > idx));
...
bool tlvSupportedService::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvTypeMap::TLV_SUPPORTED_SERVICE;
    else {
        ...
        if (swapped_type != eTlvTypeMap::TLV_SUPPORTED_SERVICE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvTypeMap::TLV_SUPPORTED_SERVICE)
    ...
    if (!m_parse__)
        *m_supported_service_list_length = 0;
