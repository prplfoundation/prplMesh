...
const eTlvTypeMap &tlvChannelPreference::type() { return (const eTlvTypeMap &)(*m_type); }

const uint16_t &tlvChannelPreference::length() { return (const uint16_t &)(*m_length); }

sMacAddress &tlvChannelPreference::radio_uid() { return (sMacAddress &)(*m_radio_uid); }
...
{
    bool ret_success =
        ((m_operating_classes_list_idx__ > 0) && (m_operating_classes_list_idx__ > idx));
...
std::shared_ptr<cOperatingClasses> tlvChannelPreference::create_operating_classes_list()
{
    ...
    return std::make_shared<cOperatingClasses>(getBuffPtr(), getBuffRemainingBytes(), m_parse__,
                                               m_swap__);
}
...
bool tlvChannelPreference::add_operating_classes_list(std::shared_ptr<cOperatingClasses> ptr)
{
    ...
    if (m_lock_allocation__ == false) {
        TLVF_LOG(ERROR) << "No call to create_operating_classes_list was called before "
                           "add_operating_classes_list";
        return false;
    }
    if (ptr->getStartBuffPtr() != getBuffPtr()) {
        TLVF_LOG(ERROR) << "Received to entry pointer is different than expected (excepting the "
                           "same pointer returned from add method)";
        return false;
    }
...
bool tlvChannelPreference::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvTypeMap::TLV_CHANNEL_PREFERENCE;
    else {
        ...
        if (swapped_type != eTlvTypeMap::TLV_CHANNEL_PREFERENCE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvTypeMap::TLV_CHANNEL_PREFERENCE)
    ...
    if (!m_parse__)
        *m_operating_classes_list_length = 0;
...
uint8_t &cOperatingClasses::operating_class() { return (uint8_t &)(*m_operating_class); }

uint8_t &cOperatingClasses::channel_list_length() { return (uint8_t &)(*m_channel_list_length); }
...
cOperatingClasses::sFlags &cOperatingClasses::flags() { return (sFlags &)(*m_flags); }

void cOperatingClasses::class_swap() { m_flags->struct_swap(); }
