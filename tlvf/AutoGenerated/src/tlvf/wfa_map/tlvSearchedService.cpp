...
const eTlvTypeMap &tlvSearchedService::type() { return (const eTlvTypeMap &)(*m_type); }

const uint16_t &tlvSearchedService::length() { return (const uint16_t &)(*m_length); }
...
std::tuple<bool, tlvSearchedService::eSearchedService &>
tlvSearchedService::searched_service_list(size_t idx)
{
    bool ret_success =
        ((m_searched_service_list_idx__ > 0) && (m_searched_service_list_idx__ > idx));
...
bool tlvSearchedService::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvTypeMap::TLV_SEARCHED_SERVICE;
    else {
        ...
        if (swapped_type != eTlvTypeMap::TLV_SEARCHED_SERVICE) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvTypeMap::TLV_SEARCHED_SERVICE)
    ...
    if (!m_parse__)
        *m_searched_service_list_length = 0;
