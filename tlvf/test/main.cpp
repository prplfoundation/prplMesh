...
{
    size_t hlen = cCmduHeader::get_initial_size() + tlvVendorSpecific::get_initial_size() + beerocks_message::cACTION_HEADER::get_initial_size();
    ...
    cmdu_tx_msg.metadata()->length = 256; //buffer size, needs to be bigger for longer messages, otherwise segfault or memory leak might occur
    ...
    auto header = msg.create(cmdu_tx_msg.data(), cmdu_tx_msg.metadata()->length, 0, eMessageType::BACKHAUL_STEERING_REQUEST_MESSAGE);
    ...
    bool allocation_succeed = firstTlv->alloc_mac_non_1905_device(3); //3 mac addresses for the example, can be any number, only limited by the buffer size
    mapf_assert (allocation_succeed == true); //false is returned in case that there isn't enough space on the buffer for the allocation   

    auto first_mac = firstTlv->mac_non_1905_device(0); //get the first mac address struct in this tlv. returns a <bool,sMacAddress&> collection.
    bool isExist = std::get<0>(first_mac); //checking the first parameter, boolean, if the address in this index exists
    if (isExist)
    {
        auto& address = std::get<1>(first_mac); //knowing that the address exists, get the address struct
        address.mac[0] = 0x00;
        address.mac[1] = 0x01;
        address.mac[2] = 0x02;
        address.mac[3] = 0x03;
    ...
    auto second_mac = firstTlv->mac_non_1905_device(1); //get the second mac address struct in this tlv. returns a bool-sMacAddress& collection.
    isExist = std::get<0>(second_mac); //checking the first parameter, boolean, if the address in this index exists
    if (isExist)
    {
        auto& address = std::get<1>(second_mac); //knowing that the address exists, get the address struct
        address.mac[0] = 0x05;
        address.mac[1] = 0x05;
        address.mac[2] = 0x05;
        address.mac[3] = 0x05;
    ...
    auto third_mac = firstTlv->mac_non_1905_device(2); //get the third mac address struct in this tlv. returns a bool-sMacAddress& collection.
    isExist = std::get<0>(third_mac); //checking the first parameter, boolean, if the address in this index exists
    if (isExist)
    {
        auto& address = std::get<1>(third_mac); //knowing that the address exists, get the address struct
        address.mac[0] = 0xFF;
        address.mac[1] = 0xFF;
        address.mac[2] = 0xFF;
        address.mac[3] = 0xFF;
    ...
    eTlvType type;
    if (received_message.getNextTlvType(type) && type == eTlvType::TLV_NON_1905_NEIGHBOR_DEVICE_LIST)
    {
        MAPF_ERR("SUCCESS");
    }
    ...
    if (tlv1 != nullptr)
    {
        ...
        auto mac2 = tlv1->mac_non_1905_device(2); //get the second mac address struct in this tlv. returns a bool-sMacAddress& collection.
        isExist = std::get<0>(mac2); //checking the first parameter, boolean, if the address in this index exists
        if (isExist)
        {
            auto address = std::get<1>(mac2); //knowing that the address exists, get the address struct
     /*       address.mac[0] = 0x05;
            address.mac[1] = 0x05;
            address.mac[2] = 0x05;
            address.mac[3] = 0x05;
