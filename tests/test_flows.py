...
class TestFlows:
    ...
    def check_topology_notification(self, eth_src: str, neighbors: list,
        ...
        """
        multicasts = self.check_cmdu_type("topology notification", 0x1, eth_src)

        # check correctness of topology notifications
        for multicast in multicasts:
            # relay indication should be set
            if not multicast.ieee1905_relay_indicator:
                self.fail("Multicast topology notification should be relayed")
                return False

            mid = multicast.ieee1905_mid
            for eth_dst in neighbors:
                unicasts = self.check_cmdu_type("topology notification", 0x1, eth_src, eth_dst)
                matches = [unicast for unicast in unicasts if unicast.ieee1905_mid == mid]
                if len(matches) != 1:
                    self.fail("Recieved {} topology notifications to {} with mid {} (should be one)"
                              .format(len(matches), eth_dst, mid))
                    return False
                unicast = matches[0]
                if unicast.ieee1905_relay_indicator:
                    self.fail("Unicast topology notification should not be relayed")
                    return False

        # search for requested event
        debug("Search for event: sta mac={}, bssid={}, event={}".format(sta.mac, bssid, event))
        matches = [m for m in multicasts if m.ieee1905_tlvs[0].assoc_event_client_mac == sta.mac
                   and m.ieee1905_tlvs[0].assoc_event_agent_bssid == bssid
                   and int(m.ieee1905_tlvs[0].assoc_event_flags, 16) == event.value]
        if len(matches) != 1:
            self.fail("Found {} matches for event, should be exactly 1".format(len(matches)))
            return False
