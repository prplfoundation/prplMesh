...
void optimal_path_task::work()
{
    ...
    case FILL_POTENTIAL_AP_LIST_11K: {
        ...
        if (database.get_client_stay_on_initial_radio(client) == eTriStateBool::ENABLE) {
            ...
            if (client_initial_hostap == tlvf::mac_from_string(current_hostap)) {
                TASK_LOG(INFO) << "Client is already on initial radio " << client_initial_hostap;
                finish();
                return;
            }
            // Client is not on initial radio, lets try to find it in the steering potential candidate list
            if (potential_11k_aps.find(tlvf::mac_to_string(client_initial_hostap)) !=
                ...
                chosen_bssid = database.get_hostap_vap_with_ssid(
                    tlvf::mac_to_string(client_initial_hostap), current_hostap_ssid);
                state          = SEND_STEER_ACTION;
                is_force_steer = true;
                TASK_LOG(INFO) << "Steer client imminently to initial radio " << it->first;
                break;
            }
            TASK_LOG(WARNING) << "Client's initial radio " << client_initial_hostap
        ...
        if ((selected_bands != PARAMETER_NOT_CONFIGURED) &&
            ...
            if (!database.is_hostap_on_selected_bands(selected_bands,
                ...
                if (it != current_hostap_siblings.end()) {
                    ...
                    state          = SEND_STEER_ACTION;
                    is_force_steer = true;
                    TASK_LOG(INFO) << "Found local radio " << it->data()
            ...
            remove_all_non_selected_band_radios(potential_11k_aps, selected_bands);
        }
    ...
    case FIND_AND_PICK_HOSTAP_CROSS: {
        ...
        // TODO: need to create preliminary state that prepares candidate hostap list
        // regardless of client's 11k support  https://jira.prplfoundation.org/browse/PPM-102
        auto client = tlvf::mac_from_string(sta_mac);
        if (database.get_client_stay_on_initial_radio(client) == eTriStateBool::ENABLE) {
            TASK_LOG(INFO) << "Client stay on initial radio enabled";
            auto client_initial_hostap = database.get_client_initial_radio(client);
            ...
            auto it =
                std::find_if(hostap_candidates.begin(), hostap_candidates.end(),
                             [&](std::pair<std::string, bool> &hostap) {
                                 return hostap.first == tlvf::mac_to_string(client_initial_hostap);
                             });

            if (it != hostap_candidates.end()) {
                // initial client hostap is on the candidate list, force steer the client there.
                chosen_bssid   = database.get_hostap_vap_with_ssid(it->first, current_hostap_ssid);
                state          = SEND_STEER_ACTION;
                is_force_steer = true;
                TASK_LOG(INFO) << "Steer client imminently to initial radio " << it->first;
                break;
        ...
        if ((selected_bands != PARAMETER_NOT_CONFIGURED) &&
            ...
            // continue with optimal path task but remove all non selected band hostaps
            // from steering candidate list.
            remove_all_non_selected_band_radios(hostap_candidates, selected_bands);
        }
...
void optimal_path_task::remove_all_non_selected_band_radios(C &radios, const int8_t selected_bands)
{
    ...
    TASK_LOG(INFO) << "Stay on selected band enabled";
